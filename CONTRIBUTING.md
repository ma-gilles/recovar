...
each paired run. The loader never builds or rewrites an explicit
`RECOVAR_CUDA_LIB` path: it loads it if it exists and exports the required
symbols, and otherwise stops with an error naming the file. Unpinned cache
builds are rebuilt only when the sha256 of their sources differs from the one
recorded beside the library (`<library>.sources.sha256`), not when source
mtimes are newer, and every build renames a temporary file over its target.
A library built directly with `make` records no digest; that is fine for a
pinned path. For qualification, place a freshly copied
