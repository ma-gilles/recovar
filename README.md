...
## Command: `outlier_detection`

### Overview

The outlier detection command identifies and filters out anomalous particles from cryo-EM datasets. It implements multiple detection methods including contrast-based outlier detection and optional junk particle detection based on reconstruction quality. This tool is particularly useful for cleaning datasets before downstream analysis and improving reconstruction quality.

The command supports two main detection approaches:

1. **Contrast-based (amplitude scale) outlier detection**: Identifies particles with unusual contrast characteristics that may indicate poor quality or contamination.
2. **Junk particle detection**: Uses reconstruction quality metrics (FSC scores) to identify particles that produce poor reconstructions when clustered together.

### Usage

```bash
recovar outlier_detection [pipeline_output_dir] --zdim [dimension] [options]
```

**Positional Arguments:**

- `pipeline_output_dir`: Directory containing RECOVAR pipeline results (output directory from `pipeline` command).

**Required Arguments:**

- `--zdim ZDIM`: Dimension of the latent variable to use for analysis (must be a single integer). 4 is often a good choice for this.

<details><summary>Optional Arguments</summary>

- `-o OUTDIR, --outdir OUTDIR`: Output directory for results. Default is `pipeline_output_dir/outlier_detection/`.

- `--use-junk-detection`: Enable junk particle detection in addition to contrast-based detection.

- `--junk-detection-params JUNK_DETECTION_PARAMS`: Path to JSON file with junk detection parameters.

- `--n-clusters N_CLUSTERS`: Number of clusters for junk detection (default: 100).

- `--n-images-per-cluster N_IMAGES_PER_CLUSTER`: Number of images per cluster for junk detection (default: 100).

- `--junk-detection-method JUNK_DETECTION_METHOD`: Method for junk detection: 'adaptive_threshold', 'percentile', 'std_based', or 'consensus' (default: 'adaptive_threshold').

- `--percentile-threshold PERCENTILE_THRESHOLD`: Percentile threshold for junk detection (default: 25).

- `--std-threshold STD_THRESHOLD`: Standard deviation threshold for junk detection (default: 2.0).

- `--min-junk-fraction MIN_JUNK_FRACTION`: Minimum fraction of clusters that can be classified as junk (default: 0.1).

- `--max-junk-fraction MAX_JUNK_FRACTION`: Maximum fraction of clusters that can be classified as junk (default: 0.8).

- `--contrast-methods CONTRAST_METHODS`: Comma-separated list of contrast detection methods: 'individual', 'particle_based', 'tilt_series_based' (default: 'individual,particle_based').

- `--save-intermediate`: Save intermediate results for debugging.

- `--lazy`: Use lazy loading for large datasets.

</details>

### Example Commands

1. **Basic Contrast-based Outlier Detection:**

   ```bash
   recovar outlier_detection /path/to/pipeline_output --zdim 10
   ```

2. **Outlier Detection with Junk Detection Enabled:**

   ```bash
   recovar outlier_detection /path/to/pipeline_output --zdim 10 \
       --use-junk-detection \
       --n-clusters 200 \
       --n-images-per-cluster 50
   ```

3. **Advanced Configuration with Custom Parameters:**

   ```bash
   recovar outlier_detection /path/to/pipeline_output --zdim 10 \
       --use-junk-detection \
       --junk-detection-method consensus \
       --percentile-threshold 30 \
       --std-threshold 1.5 \
       --contrast-methods individual,particle_based,tilt_series_based \
       --save-intermediate
   ```
...
This method identifies particles with unusual contrast characteristics using multiple approaches:

- **Individual particle analysis**: Detects outliers based on individual particle contrast scores.
- **Particle-based grouping**: Groups particles by their latent space coordinates and detects outliers within each group.
- **Tilt series-based grouping**: For tilt series data, groups particles by their tilt series and detects outliers within each series.

The method uses multiple outlier detection algorithms:
- **Isolation Forest**: Effective for detecting global outliers
- **Local Outlier Factor (LOF)**: Identifies local density-based outliers
- **Robust Covariance**: Detects outliers based on Mahalanobis distance
- **Consensus approach**: Combines results from multiple methods for robust detection

#### Junk Particle Detection

This optional method identifies particles that produce poor reconstructions when clustered together:

1. **Clustering**: Groups particles into clusters based on latent space coordinates
2. **Reconstruction**: Generates reconstructions for each cluster
3. **Quality Assessment**: Computes FSC scores (half-map and vs-mean) for each reconstruction
4. **Outlier Detection**: Identifies clusters with poor FSC scores as junk

**Supported Methods:**
- **Adaptive Threshold**: Automatically determines threshold based on score distribution
- **Percentile-based**: Uses percentile threshold (e.g., bottom 25%)
- **Standard Deviation**: Uses mean ± N*std threshold
- **Consensus**: Combines multiple methods for robust detection

### Output Structure

The command creates an `outlier_detection/` directory (or specified output directory) with the following structure:

```
outlier_detection/
├── contrast_based/
│   ├── anomaly_detection_results.png      # Visualization of contrast-based detection
│   ├── contrast_outlier_detection.pkl     # Detailed results and statistics
│   ├── contrast_outlier_detection.png     # Contrast analysis plots
│   ├── inlier_indices.pkl                 # Indices of inlier particles
│   ├── outlier_indices.pkl                # Indices of outlier particles
│   ├── inliers_*.pkl                      # Inlier indices for each method
│   └── outliers_*.pkl                     # Outlier indices for each method
├── junk_detection/                        # Only if --use-junk-detection is enabled
│   ├── all_fsc_curves_*.pkl              # FSC curves for all clusters
│   ├── all_halfmap_fsc_curves_*.png      # Half-map FSC visualization
│   ├── all_vs_mean_fsc_curves_*.png      # vs-mean FSC visualization
│   ├── individual_mrc_files/              # Individual cluster reconstructions
│   ├── reconstructions/                   # Cluster reconstruction volumes
│   ├── junk_clusters.pkl                  # Identified junk clusters
│   ├── junk_particles.pkl                 # Particles from junk clusters
│   └── junk_detection_results.png         # Junk detection visualization
├── combined_results/
│   ├── combined_outliers.pkl              # Union of all outlier indices
│   ├── combined_inliers.pkl               # Intersection of all inlier indices
│   ├── overlap_analysis.txt               # Detailed overlap statistics
│   └── overlap_matrix.png                 # Visualization of method overlaps
└── run.log                                # Execution log
```

### Output Files Description

#### Contrast-based Detection Results

- **`anomaly_detection_results.png`**: Comprehensive visualization showing:
  - UMAP embedding with outlier/inlier coloring
  - Contrast score distributions
  - Method comparison plots
  - Overlap analysis between detection methods

- **`contrast_outlier_detection.pkl`**: Contains detailed results including:
  - Outlier/inlier indices for each method
  - Contrast scores and statistics
  - Detection parameters and thresholds
  - Performance metrics

- **`contrast_outlier_detection.png`**: Detailed contrast analysis including:
  - Histograms of contrast scores per particle and per tilt series
  - Statistical summaries
  - Method comparison visualizations

#### Junk Detection Results (if enabled)

- **`junk_detection_results.png`**: Visualization of junk detection including:
  - FSC score distributions
  - Cluster quality assessment
  - Junk cluster identification
  - UMAP embedding with junk/non-junk coloring

- **`junk_clusters.pkl`**: Contains:
  - Identified junk cluster indices
  - FSC scores for all clusters
  - Detection method parameters
  - Quality metrics

#### Combined Results

- **`combined_outliers.pkl`**: Union of outliers from all detection methods
- **`combined_inliers.pkl`**: Intersection of inliers from all detection methods
- **`overlap_analysis.txt`**: Detailed text analysis of overlaps between methods
- **`overlap_matrix.png`**: Heatmap visualization of method overlaps

### Integration with Pipeline

The outlier detection results can be used to filter the dataset for downstream analysis:

1. **Filter particles for re-analysis**:
   ```bash
   recovar pipeline particles.mrcs --ctf ctf.pkl --poses poses.pkl \
       --ind outlier_detection/combined_inliers.pkl -o filtered_output
   ```

2. **Extract outlier particles for inspection**:
   ```bash
   recovar extract_image_subset_from_kmeans centers.pkl outliers.pkl \
       --ind outlier_detection/combined_outliers.pkl
   ```

### Notes

- **Memory Requirements**: Junk detection requires significant memory for reconstruction. Use `--lazy` for large datasets.
- **Runtime**: Junk detection can be time-consuming due to reconstruction requirements. Consider using fewer clusters or images per cluster for faster execution.
- **Tilt Series Data**: When using `--tilt-series` data, the command automatically adapts to use tilt series-based grouping.
- **Parameter Tuning**: The default parameters work well for most datasets, but you may need to adjust thresholds based on your specific data characteristics.
- **Validation**: Always inspect the generated visualizations to ensure the detection is working as expected for your dataset.
