...
def _nonempty_per_class_support(significant_sample_indices_by_class):
    """Make per-class sparse kernels tolerate zero posterior-support images.

    The joint class x pose support can legitimately contain no samples for a
    class on an image.  The sparse per-class replay kernels are single-class
    utilities and require at least one coarse sample per image, so use one
    deterministic negligible sample for those per-class diagnostic calls while
    leaving the reported joint support counts unchanged.
    """

    fixed = []
    n_empty = 0
    for class_entries in significant_sample_indices_by_class:
        fixed_class = []
        for samples in class_entries:
            if samples is None:
                fixed_class.append(None)
                continue
            samples = np.asarray(samples, dtype=np.int32).reshape(-1)
            if samples.size == 0:
                fixed_class.append(np.asarray([0], dtype=np.int32))
                n_empty += 1
            else:
                fixed_class.append(samples)
        fixed.append(fixed_class)
...
def _relion_bpref_maps_from_sparse_support(
    ...
    *,
    ...
    for class_index in range(n_classes):
        ...
        row_dump_dir = os.environ.get("RECOVAR_BPREF_ROW_DUMP_DIR")
        if row_dump_dir:
            row_dump_path = Path(row_dump_dir)
            row_dump_path.mkdir(parents=True, exist_ok=True)
            np.savez(
                row_dump_path / f"class{class_index + 1:03d}_rows.npz",
                images=images,
                weights=weights,
                rotations=row_rotations,
            )
        print(
        ...
        vol_relion = np.asarray(
            bind.backproject_and_reconstruct(
                ...
                True,
                int(current_size),
                float(class_normalise[class_index]),
                float(minres_map),
            )
        )
...
def _replay_bpref_row_dump(
    ...
    *,
    ...
    """Reconstruct BPref maps from dumped BackProjector rows."""

    from recovar.relion_bind import _relion_bind_core as bind
    from recovar.utils import helpers

    class_normalise = _class_distributions(relion_model)
    maps = []
    for class_index in range(n_classes):
        row_path = row_dump_dir / f"class{class_index + 1:03d}_rows.npz"
        ...
        vol_relion = np.asarray(
            bind.backproject_and_reconstruct(
                ...
                rows["rotations"],
                rows["weights"],
                _tau_spectrum(relion_model, class_index),
                int(volume_shape[0]),
                int(reconstruction_padding_factor),
                1,
                True,
...
def main() -> None:
    ...
    if need_significant_support:
        ...
        per_class_sparse_support, n_empty_per_class_support = _nonempty_per_class_support(significant_sample_indices)
        ...
        if args.significant_mstep:
            ...
            for class_index in range(n_classes):
                class_Ft_y, class_Ft_ctf = compute_pass2_stats_sparse(
                    ...
                    noise_variance,
                    translations.astype(np.float32),
                    per_class_sparse_support[class_index],
                    healpix_order,
                    args.disc_type,
                    oversampling_order=0,
                    current_size=current_size,
        ...
        print(
            ...
            f"empty per-class supports patched={n_empty_per_class_support}"
        )
    ...
    if args.relion_bpref_mstep:
        ...
        bpref_maps, relion_bpref_summary = _relion_bpref_maps_from_sparse_support(
            ...
            class_normalise=_class_distributions(target_model),
        )
        ...
        row_dump_dir = os.environ.get("RECOVAR_BPREF_ROW_DUMP_DIR")
        if row_dump_dir:
            replay_maps = _replay_bpref_row_dump(
                Path(row_dump_dir),
                target_model,
                n_classes=n_classes,
            ...
            replay_maps = [np.asarray(class_map) * np.asarray(solvent_mask) for class_map in replay_maps]
            variant_maps["relion_bpref_row_replay_solvent"] = replay_maps
            variant_results["relion_bpref_row_replay_solvent"] = _best_class_permutation(replay_maps, relion_real)
            row_replay_diff = [
                float(np.max(np.abs(np.asarray(bpref_maps[k]) - np.asarray(replay_maps[k]))))
                for k in range(n_classes)
            ]
            for class_index, class_real in enumerate(bpref_maps):
                write_relion_mrc(
                    output_dir / f"relion_bpref_sparse_class{class_index + 1:03d}.mrc",
                    class_real,
                    voxel_size=ds.voxel_size,
                )
            for class_index, class_real in enumerate(replay_maps):
                write_relion_mrc(
                    output_dir / f"relion_bpref_row_replay_class{class_index + 1:03d}.mrc",
                    class_real,
                    voxel_size=ds.voxel_size,
                )
            print(
                "  RELION BPref row replay: "
                f"mean_corr={variant_results['relion_bpref_row_replay_solvent']['mean_corr']:.6f}, "
                f"max_abs_diff_vs_inline={np.round(row_replay_diff, 6).tolist()}"
            )
