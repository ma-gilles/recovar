...
def evaluate_stage_1c_strict(records):
    """Strict spec Section 11.5 criterion. Listed for traceability;
    not gated at v0 toy size. Several clauses (HeterogeneousEMState
    baseline, family C subspace tolerance, family D contrast-overlap)
    are noted as `unimplemented_in_v0` and the check returns
    `passed = False` until they are wired up."""
    by_family = {f: [r for r in records if r["family"] == f] for f in ("A", "B", "C", "D")}

    b_proj_strict = all(r["proj_improvement"] > 0 for r in by_family["B"])
    return {
        "criterion": "strict",
        "passed": False,
        "family_B_proj_improves": b_proj_strict,
        "family_C_subspace_tolerance_check": "unimplemented_in_v0",
        "family_D_contrast_overlap_check": "unimplemented_in_v0",
        "heterogeneous_em_state_baseline_check": "unimplemented_in_v0",
    }
