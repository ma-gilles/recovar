#!/usr/bin/env python3
"""Submit one corrected-sign spike 3DFlex K=1 sanity pipeline through CryoSPARC."""
...
def main() -> None:
    ...
    parser.add_argument("--workspace-title", default="spike fullatom noise1 b80 corrected 3DFlex sanity")
    parser.add_argument(
        "--consensus-source",
        choices=["import_map", "homo_reconstruct"],
        default="homo_reconstruct",
    ...
    if args.source_run == DEFAULT_SOURCE_RUN:
        label = f"n{args.n_images:08d}"
        source_run = (
            Path("/scratch/gpfs/CRYOEM/gilleslab/tmp/spike_fullatom_consistency_grid256_noise1_b80_true_oracle_sweep_20260527")
            / label
            / "runs"
            / f"{label}_seed0000"
        )
    ...
    jobs["import_particles"] = cs.create_job(
        ...
        title=f"Import spike noise1/B80 {args.n_images} particles sign=1",
    )
    ...
    if args.consensus_source == "homo_reconstruct":
        jobs["homo_reconstruct"] = cs.create_job(
            ...
            title=f"Homogeneous reconstruction only spike noise1/B80 n={args.n_images}",
        )
        if args.submit:
    ...
    jobs["flex_prep"] = cs.create_job(
        ...
        title=f"3DFlex data prep spike noise1/B80 n={args.n_images}",
    )
    ...
    jobs["flex_meshprep"] = cs.create_job(
        ...
        title=f"3DFlex mesh prep spike noise1/B80 n={args.n_images}",
    )
    for k in (1,):
        jobs[f"flex_train_k{k}"] = cs.create_job(
            ...
            params={"flex_K": k},
            title=f"3DFlex train spike noise1/B80 n={args.n_images} K={k}",
        )
        if args.run_flex_highres:
            jobs[f"flex_highres_k{k}"] = cs.create_job(
                ...
                title=f"3DFlex reconstruction spike noise1/B80 n={args.n_images} K={k}",
            )
    ...
    # K=1 is the primary corrected branch. K=2 can be submitted later once K=1 is healthy.
    if args.run_flex_highres:
