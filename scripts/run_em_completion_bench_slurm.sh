...
Environment overrides:
  ...
  RECOVAR_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE
                             ...
                             defaults to 0 for autonomous and 1 for relion-replay
...
$(write_optional_env_exports "${optional_env_names[@]}")
for env_name in ${optional_env_names[*]} RECOVAR_SPARSE_KCLASS_FUSED; do
  env_value="\${!env_name-}"
  if [[ -n "\${env_value}" ]]; then
