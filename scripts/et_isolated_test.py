...
def main():
    ...
    def F(v):
        return float(np.asarray(v)) if v is not None else None
    ...
    _, var_fsc = plot_utils.plot_fsc_new(
        gt_fourier_var, variance_est["combined"], np.array(volume_shape), voxel_size, threshold=0.5, name="Var FSC"
    )
    ...
    logger.info(
        "  svd_4=%.6f  svd_10=%.6f  s0=%.2f  s1=%.2f",
        scores.get("svd_relative_variance_4", 0),
        scores.get("svd_relative_variance_10", 0),
        scores["eigenvalue_0"],
        scores.get("eigenvalue_1", 0),
    )
