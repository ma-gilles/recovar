...
def load_relion_half_sets(relion_star_path, our_star_path):
    ...
    our_subsets = np.array([
        idx_to_subset[image_name_to_stack_idx(n)] for n in our_names
    ])
...
def main():
        ...
        if r_h1_ft is not None and r_h2_ft is not None:
            ...
            if result1 is not None:
                our_h1_ft = jnp.asarray(
                    result1["means"][0] if isinstance(result1, dict) and "means" in result1
            ...
            if result2 is not None:
                our_h1_ft = jnp.asarray(
                    result2["means"][0] if isinstance(result2, dict) and "means" in result2
    ...
    _print_report(
        ...
        relion_iters, relion_current_sizes, relion_elapsed, relion_res_A,
        cross_fsc_run1, cross_fsc_run2, relion_internal_fsc,
        volume_shape, voxel_size, args.max_iter,
    )
...
def _print_report(
    result1, result2, time1, time2,
    relion_iters, relion_cs, relion_elapsed, relion_res_A,
    cross_fsc_run1, cross_fsc_run2, relion_internal_fsc,
    volume_shape, voxel_size, max_iter,
):
    ...
    print(f"Same half-set split (RELION's rlnRandomSubset)")
    print()

    # ---- Table 1: Per-iteration comparison ----
    print("## Per-Iteration Comparison")
    print()
    print("| Iter | RELION cs | Our cs (own) | Our cs (oracle) | "
    ...
    print()
    print("| Iter | Cross-FSC 0.5 (own) | Cross-FSC 0.5 (oracle) | "
          "RELION internal 0.143 | Our internal 0.143 (own) |")
    print("|------|---------------------|------------------------|-"
          "----------------------|--------------------------|")
    ...
    if result1 and result1.get("means"):
        ...
        relion_merged_ft = load_mrc_to_ft(
            os.path.join(RELION_REF, "run_class001.mrc"), volume_shape
        )
