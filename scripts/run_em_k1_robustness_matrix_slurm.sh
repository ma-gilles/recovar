...
RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID="${RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID:-}"
RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM="${RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM:-}"
RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS="${RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS:-}"
RECOVAR_K1_RELION_LIVE_INITIAL_NOISE="${RECOVAR_K1_RELION_LIVE_INITIAL_NOISE:-}"
RECOVAR_INITIAL_PROJECTOR_USE_REAL_REFERENCE="${RECOVAR_INITIAL_PROJECTOR_USE_REAL_REFERENCE:-}"
RECOVAR_FIRSTITER_CC_TREE_TOP2_RESCORE_MAX_MARGIN="${RECOVAR_FIRSTITER_CC_TREE_TOP2_RESCORE_MAX_MARGIN:-}"
...
RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR="${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR:-}"
RECOVAR_BPREF_CONTRIBUTION_DUMP_ITERATION="${RECOVAR_BPREF_CONTRIBUTION_DUMP_ITERATION:-}"
RECOVAR_BPREF_CONTRIBUTION_DUMP_HALF="${RECOVAR_BPREF_CONTRIBUTION_DUMP_HALF:-}"
RECOVAR_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE="${RECOVAR_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE:-}"
RECOVAR_BPREF_CONTRIBUTION_DUMP_ORIGINAL_INDICES="${RECOVAR_BPREF_CONTRIBUTION_DUMP_ORIGINAL_INDICES:-}"
RECOVAR_BPREF_CONTRIBUTION_DUMP_RUN_ID="${RECOVAR_BPREF_CONTRIBUTION_DUMP_RUN_ID:-}"
RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY="${RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY:-}"
RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256="${RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256:-}"
...
RECOVAR_PASS2_DUMP_ITERATION="${RECOVAR_PASS2_DUMP_ITERATION:-}"
RECOVAR_SIGNIFICANCE_DUMP_DIR="${RECOVAR_SIGNIFICANCE_DUMP_DIR:-}"
RECOVAR_SIGNIFICANCE_DUMP_ORIGINAL_INDICES="${RECOVAR_SIGNIFICANCE_DUMP_ORIGINAL_INDICES:-}"
RECOVAR_SIGNIFICANCE_DUMP_CURRENT_SIZE="${RECOVAR_SIGNIFICANCE_DUMP_CURRENT_SIZE:-}"
RECOVAR_SIGNIFICANCE_DUMP_ITERATION="${RECOVAR_SIGNIFICANCE_DUMP_ITERATION:-}"
...
RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS="${RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS:-}"
...
RELION_ACC_DUMP_VERBOSE="${RELION_ACC_DUMP_VERBOSE:-}"
RELION_ACC_DUMP_PASS1_DIFF2="${RELION_ACC_DUMP_PASS1_DIFF2:-}"
WATCH=0
DRY_RUN=0
SELECTED_CASES=()
...
Environment overrides:
  ...
  RECOVAR_K1_BPREF_EXECUTION_ORDER_CHUNK_SIZE
                                    Diagnostic only: pad consecutive physical-order particles into
                                    memory-bounded mixed-support BPref chunks of at most this size.
  RECOVAR_K1_COARSE_GAUSSIAN_FFI    Diagnostic only: use the RELION CUDA contraction for K=1
                                    coarse Gaussian scoring (default: unset/off).
  RECOVAR_K1_COARSE_GAUSSIAN_NATIVE_TEXTURE
                                    Diagnostic only: fuse RELION texture projection, translation,
                                    and coarse score reduction in the source launch topology.
  RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID
                                    Diagnostic only: set 0 to restore the legacy floor-bounded K=1
                                    translation grid. Default unset/on uses RELION's ceil-bounded grid.
  RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM
                                    Explicit fresh-K=1 powerClass spectrum override.
  RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS
                                    Explicit exact BPref operand override.
  RECOVAR_K1_RELION_LIVE_INITIAL_NOISE
                                    Explicit fresh-K=1 live initial-noise override. Unset defaults on
                                    only for a strict one-optics RELION cold start; 0 disables it.
  ...
  RECOVAR_RELION_X_HALF_MSTEP_DOUBLE
                                    Diagnostic only: accumulate the RELION x-half K=1 M-step in
                                    complex128/float64 instead of production complex64/float32.
  RECOVAR_RELION_FINE_DIFF2_FUSED_FFI
                                    Diagnostic only: use the exact RELION CUDA fine-diff2 FFI.
                                    The launcher records and replays this value explicitly.
  ...
  RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR
                                    Diagnostic only: exact-local pre-scatter contribution root. Requires
                                    explicit iteration, half, and current-size targets. Particle identities
                                    and stack SHA256 are frozen automatically after preparation when unset.
  ...
  RECOVAR_PASS2_DUMP_ITERATION      Numbered-iteration filter for RECOVAR_PASS2_DUMP_DIR
                                    (default: ${RECOVAR_PASS2_DUMP_ITERATION:-<unset>})
  RECOVAR_SIGNIFICANCE_DUMP_DIR     Diagnostic only: base directory for per-case RECOVAR pass-1
                                    significance/posterior dumps
                                    (default: ${RECOVAR_SIGNIFICANCE_DUMP_DIR:-<unset>})
  RECOVAR_SIGNIFICANCE_DUMP_ORIGINAL_INDICES
                                    Comma-separated original particle indices for significance dumps
                                    (default: ${RECOVAR_SIGNIFICANCE_DUMP_ORIGINAL_INDICES:-<unset>})
  RECOVAR_SIGNIFICANCE_DUMP_CURRENT_SIZE
                                    Current size filter for significance dumps
                                    (default: ${RECOVAR_SIGNIFICANCE_DUMP_CURRENT_SIZE:-<unset>})
  RECOVAR_SIGNIFICANCE_DUMP_ITERATION
                                    One-based iteration filter for significance dumps
                                    (default: ${RECOVAR_SIGNIFICANCE_DUMP_ITERATION:-<unset>})
  ...
  RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS
                                    Memory guard for fused exact-local x-half projection rows.
                                    Lower this for OOM diagnostics (default: RECOVAR internal default).
  ...
  RECOVAR_RELION_CURRENT_SIZES      Diagnostic only: comma-separated per-iteration current-size
                                    oracle forwarded to --relion_current_sizes (default: unset)
  ...
  RELION_ACC_DUMP_VERBOSE           Patched RELION accelerated candidate/posterior dump gate
                                    (default: ${RELION_ACC_DUMP_VERBOSE:-<unset>})
  RELION_ACC_DUMP_PASS1_DIFF2       Patched RELION part-scoped pass-1 diff2/operand dump gate
                                    (default: ${RELION_ACC_DUMP_PASS1_DIFF2:-<unset>})
USAGE
}
...
if [[ "${SCORECARD_MODE}" == "1" ]]; then
  ...
  if [[ "${RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID}" =~ ^(0|false|FALSE|no|NO|off|OFF)$ ]]; then
    echo "--scorecard requires RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID unset/on" >&2
    exit 2
  fi
...
unset RECOVAR_RELION_FINE_DIFF2_FUSED_FFI
...
# Detached worktrees intentionally reuse the dependency packages from the
# base pixi environment.  Make non-Python build dependencies (notably FFTW)
# discoverable without creating a worktree-local .pixi symlink.
...
$(write_optional_env_exports TF_GPU_ALLOCATOR RECOVAR_RELION_EM_BATCH_PROJECTION_FRACTION RECOVAR_RELION_FIRSTITER_RECON_COMPLEX_BUDGET RECOVAR_PASS1_FUSED RECOVAR_DISABLE_LOCAL_BIG_JIT RECOVAR_LOCAL_BUCKET_QUANTUM RECOVAR_SPARSE_PASS2_TAIL_BUCKET_COALESCE_MAX_IMAGES RECOVAR_SPARSE_PASS2_TAIL_BUCKET_COALESCE_MAX_INFLATION RECOVAR_SPARSE_PASS2_TAIL_BUCKET_COALESCE_MIN_BUCKET_SIZE RECOVAR_SPARSE_PASS2_MAX_TRANSLATION_TILE_BYTES RECOVAR_SPARSE_PASS2_MAX_PROJECTION_GATHER_BYTES RECOVAR_SPARSE_PASS2_MAX_NOISE_BLOCK_BYTES RECOVAR_SPARSE_PASS2_MAX_ADJOINT_BLOCK_BYTES RECOVAR_SPARSE_PASS2_PROJECTION_CACHE_MAX_BYTES RECOVAR_SPARSE_PASS2_MAX_PROJECTED_ROTATIONS RECOVAR_K1_DENSE_PASS2 RECOVAR_K1_SKIP_SIGNIFICANCE_PRUNING RECOVAR_K1_COARSE_GAUSSIAN_FFI RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS RECOVAR_K1_RELION_LIVE_INITIAL_NOISE RECOVAR_K1_RELION_X_HALF_MSTEP RECOVAR_RELION_X_HALF_MSTEP_DOUBLE RECOVAR_RELION_FINE_DIFF2_FUSED_FFI RECOVAR_EM_LOW_PMAX_REFINE_GUARD RECOVAR_EM_LOW_PMAX_REFINE_MAX_AVE_PMAX RECOVAR_EM_LOW_PMAX_REFINE_REQUIRE_LOCAL RECOVAR_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE RECOVAR_FINAL_ALL_DATA_USE_MERGED_REFERENCE RECOVAR_FINAL_ALL_DATA_DISABLE_REPLAY_LAST_NUMBERED_STATE RECOVAR_FINAL_ALL_DATA_GRID_CORRECT RECOVAR_INITIAL_PROJECTOR_USE_REAL_REFERENCE RECOVAR_FIRSTITER_CC_TREE_TOP2_RESCORE_MAX_MARGIN RECOVAR_LOCAL_ADAPTIVE_PASS2_FULL_PARENT RECOVAR_LOCAL_ADAPTIVE_PASS2_ROTATION_ONLY RECOVAR_LOCAL_ADAPTIVE_PASS2_DENOMINATOR_SUPPORT RECOVAR_BPREF_ACCUM_DUMP_DIR RECOVAR_PASS2_DUMP_DIR RECOVAR_PASS2_DUMP_ORIGINAL_INDICES RECOVAR_PASS2_DUMP_CURRENT_SIZE RECOVAR_PASS2_DUMP_ITERATION RECOVAR_SIGNIFICANCE_DUMP_DIR RECOVAR_SIGNIFICANCE_DUMP_ORIGINAL_INDICES RECOVAR_SIGNIFICANCE_DUMP_CURRENT_SIZE RECOVAR_SIGNIFICANCE_DUMP_ITERATION RECOVAR_LOCAL_FUSED_POSTERIOR_DUMP_DIR RECOVAR_LOCAL_FUSED_POSTERIOR_DUMP_GLOBAL_INDICES RECOVAR_LOCAL_FUSED_POSTERIOR_DUMP_CURRENT_SIZE RECOVAR_LOCAL_FUSED_POSTERIOR_DUMP_ITERATION RECOVAR_LOCAL_FUSED_POSTERIOR_DUMP_LABEL RECOVAR_LOCAL_SCORE_DUMP_DIR RECOVAR_LOCAL_SCORE_DUMP_GLOBAL_INDICES RECOVAR_LOCAL_SCORE_DUMP_CURRENT_SIZE RECOVAR_LOCAL_SCORE_DUMP_ITERATION RECOVAR_LOCAL_SCORE_DUMP_LABEL RECOVAR_LOCAL_SCORE_DUMP_OPERANDS RECOVAR_EXACT_LOCAL_AUTO_MICROBATCH_BOOST RECOVAR_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS RECOVAR_EXACT_LOCAL_PROGRESS_CHUNKS RECOVAR_EXACT_LOCAL_PROGRESS_SECONDS RECOVAR_RELION_PROJECTOR_DUMP_DIR RECOVAR_MSTEP_DUMP_DIR RECOVAR_MSTEP_DUMP_MAX_CALLS RECOVAR_MSTEP_DUMP_RAW RECOVAR_SAVE_INTERMEDIATES_DIR RECOVAR_RELION_CURRENT_SIZES RELION_DUMP_DIR RELION_DUMP_STACK_INDEX RELION_DUMP_PART_ID RELION_DUMP_PART RELION_DUMP_ITER)
$(write_optional_env_exports RECOVAR_K1_COARSE_GAUSSIAN_NATIVE_TEXTURE RECOVAR_K1_BPREF_EXECUTION_ORDER_CHUNK_SIZE)
$(write_optional_env_exports RELION_ACC_DUMP_VERBOSE RELION_ACC_DUMP_PASS1_DIFF2)
...
$(write_optional_env_exports RECOVAR_EXACT_LOCAL_TARGET_ROW_PIXELS RECOVAR_EXACT_LOCAL_BIG_JIT_MATMUL_MAX_GB RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR RECOVAR_BPREF_CONTRIBUTION_DUMP_ITERATION RECOVAR_BPREF_CONTRIBUTION_DUMP_HALF RECOVAR_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE RECOVAR_BPREF_CONTRIBUTION_DUMP_ORIGINAL_INDICES RECOVAR_BPREF_CONTRIBUTION_DUMP_RUN_ID RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256)
$(write_optional_env_exports RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS)
if [[ -n "\${RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS:-}" ]]; then
  echo "RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS=\${RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS}"
fi
...
for env_name in RECOVAR_K1_COARSE_GAUSSIAN_NATIVE_TEXTURE RECOVAR_K1_BPREF_EXECUTION_ORDER_CHUNK_SIZE; do
  env_value="\${!env_name-}"
  if [[ -n "\${env_value}" ]]; then
    echo "\${env_name}=\${env_value}"
  fi
done
for env_name in RELION_ACC_DUMP_VERBOSE RELION_ACC_DUMP_PASS1_DIFF2; do
  env_value="\${!env_name-}"
  if [[ -n "\${env_value}" ]]; then
    echo "\${env_name}=\${env_value}"
  fi
done
for env_name in RECOVAR_EXACT_LOCAL_TARGET_ROW_PIXELS RECOVAR_EXACT_LOCAL_BIG_JIT_MATMUL_MAX_GB RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR RECOVAR_BPREF_CONTRIBUTION_DUMP_ITERATION RECOVAR_BPREF_CONTRIBUTION_DUMP_HALF RECOVAR_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE RECOVAR_BPREF_CONTRIBUTION_DUMP_ORIGINAL_INDICES RECOVAR_BPREF_CONTRIBUTION_DUMP_RUN_ID RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256; do
  env_value="\${!env_name-}"
  if [[ -n "\${env_value}" ]]; then
    echo "\${env_name}=\${env_value}"
  fi
...
if [[ -n "\${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR:-}" ]]; then
  export RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR="\${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR%/}/${idx}_${name}"
  mkdir -p "\${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR}"
  echo "RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR=\${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR}"
fi
...
if [[ -n "\${RECOVAR_SIGNIFICANCE_DUMP_DIR:-}" ]]; then
  export RECOVAR_SIGNIFICANCE_DUMP_DIR="\${RECOVAR_SIGNIFICANCE_DUMP_DIR%/}/${idx}_${name}"
  mkdir -p "\${RECOVAR_SIGNIFICANCE_DUMP_DIR}"
  echo "RECOVAR_SIGNIFICANCE_DUMP_DIR=\${RECOVAR_SIGNIFICANCE_DUMP_DIR}"
fi
...
RECOVAR_EXTRA_ARGS=()
if [[ -n "\${RECOVAR_RELION_CURRENT_SIZES:-}" ]]; then
  RECOVAR_EXTRA_ARGS+=(--relion_current_sizes "\${RECOVAR_RELION_CURRENT_SIZES}")
fi
...
if [[ -n "\${RECOVAR_BPREF_CONTRIBUTION_DUMP_DIR:-}" ]]; then
  if [[ -z "\${RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY:-}" || -z "\${RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256:-}" ]]; then
    BPREF_METADATA_DIR="\${CASE_ROOT}/bpref_contribution_metadata"
    BPREF_METADATA_MANIFEST="\${BPREF_METADATA_DIR}/manifest.json"
    mkdir -p "\${BPREF_METADATA_DIR}"
    "\${PIXI_PY}" -m scripts.prepare_bpref_contribution_metadata \
      --star "\${DATA_DIR}/particles.star" \
      --image-names-npy "\${BPREF_METADATA_DIR}/image_names.npy" \
      --manifest "\${BPREF_METADATA_MANIFEST}"
    export RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY="\${BPREF_METADATA_DIR}/image_names.npy"
    export RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256="\$("\${PIXI_PY}" -c 'import json,sys; print(json.load(open(sys.argv[1]))["source_stack_sha256"])' "\${BPREF_METADATA_MANIFEST}")"
  fi
  echo "RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY=\${RECOVAR_BPREF_CONTRIBUTION_IMAGE_NAMES_NPY}"
  echo "RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256=\${RECOVAR_BPREF_CONTRIBUTION_STACK_SHA256}"
fi
...
unset RECOVAR_RELION_FINE_DIFF2_FUSED_FFI
unset RECOVAR_USE_FLOAT64_SCORING RECOVAR_USE_FLOAT64_PROJECTIONS
$(write_optional_env_exports RECOVAR_RELION_FINE_DIFF2_FUSED_FFI)
export PYTHONNOUSERSITE=1
export RECOVAR_DISABLE_CUDA=1
export CUDA_VISIBLE_DEVICES=""
...
RECOVAR_RELION_X_HALF_MSTEP_DOUBLE=${RECOVAR_RELION_X_HALF_MSTEP_DOUBLE:-}
RECOVAR_RELION_FINE_DIFF2_FUSED_FFI=${RECOVAR_RELION_FINE_DIFF2_FUSED_FFI:-}
RECOVAR_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE=${RECOVAR_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE:-}
RECOVAR_FINAL_ALL_DATA_USE_MERGED_REFERENCE=${RECOVAR_FINAL_ALL_DATA_USE_MERGED_REFERENCE:-}
RECOVAR_FINAL_ALL_DATA_DISABLE_REPLAY_LAST_NUMBERED_STATE=${RECOVAR_FINAL_ALL_DATA_DISABLE_REPLAY_LAST_NUMBERED_STATE:-}
RECOVAR_FINAL_ALL_DATA_GRID_CORRECT=${RECOVAR_FINAL_ALL_DATA_GRID_CORRECT:-}
...
RECOVAR_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS=${RECOVAR_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS:-}
RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS=${RECOVAR_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS:-}
RECOVAR_K1_COARSE_GAUSSIAN_FFI=${RECOVAR_K1_COARSE_GAUSSIAN_FFI:-}
RECOVAR_K1_COARSE_GAUSSIAN_NATIVE_TEXTURE=${RECOVAR_K1_COARSE_GAUSSIAN_NATIVE_TEXTURE:-}
RECOVAR_K1_BPREF_EXECUTION_ORDER_CHUNK_SIZE=${RECOVAR_K1_BPREF_EXECUTION_ORDER_CHUNK_SIZE:-}
RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID=${RECOVAR_K1_RELION_EXACT_TRANSLATION_GRID:-}
RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM=${RECOVAR_K1_RELION_POWERCLASS_SPECTRUM_NORM:-}
RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS=${RECOVAR_K1_RELION_EXACT_BPREF_OPERANDS:-}
RECOVAR_K1_RELION_LIVE_INITIAL_NOISE=${RECOVAR_K1_RELION_LIVE_INITIAL_NOISE:-}
