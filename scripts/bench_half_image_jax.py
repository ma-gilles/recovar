...
def main():
    ...
    for grid_size in [32, 64, 128]:
        ...
        vol_half = jnp.array(np.asarray(
            ftu.full_volume_to_half_volume(jnp.array(vol_full_np), volume_shape)
        ).ravel())
        ...
        full_imgs = jnp.array(
            (rng.standard_normal((n_images, grid_size * grid_size)) +
             1j * rng.standard_normal((n_images, grid_size * grid_size))).astype(np.complex64)
        )
        ...
        print(f"Full volume size: {full_vol_size}  |  Half volume size: {half_vol_size}  ({half_vol_size/full_vol_size:.1%})")
        print(f"{'─' * 70}")
        ...
        for order_name, order in [("nearest", 0), ("linear", 1)]:
            ...
            # ── Forward benchmarks ──
            t_full, _ = bench(
                core_slicing._jax_slice, vol_full, rots, image_shape, volume_shape, order
            )
            t_half_img, _ = bench(
                core_slicing._jax_slice_half_image, vol_full, rots, image_shape, volume_shape, order
            )
            ...
            def adj_fi_fv(imgs):
                return core_slicing.adjoint_slice_volume(
                    imgs, rots, image_shape, volume_shape, dt,
        ...
        coeffs = cubic_interpolation.calculate_spline_coefficients(
            jnp.array(vol_full_np)
        )
        ...
        for hi_label, hi_flag, imgs in [("full_img", False, full_imgs), ("half_img", True, half_imgs)]:
            for hv_label, hv_flag in [("full_vol", False), ("half_vol", True)]:
                def adj_cubic(imgs_arg, _hi=hi_flag, _hv=hv_flag):
                    return core_slicing.adjoint_slice_volume(
                        imgs_arg, rots, image_shape, volume_shape, dt_cubic,
                        half_image=_hi, half_volume=_hv,
                    )
    ...
    for grid_size in [32, 64, 128, 256]:
        ...
        print(f"  Grid {grid_size:3d}:  full_img={full_img_bytes/1e6:7.1f} MB  half_img={half_img_bytes/1e6:7.1f} MB  "
