...
def compute_little_H_b(
    ...
    b = compute_bLambdainvPU_terms(
        mean_projections, u_projections, batch, translations, CTF, noise_variance, image_shape
    )
...
def compute_bLambdainvPU_terms(
    mean_projections, u_projections, invnoise_CTFed_images, translations, CTF, noise_variance, image_shape
):
    ...
    shifted_images = core.batch_trans_translate_images(
        invnoise_CTFed_images, jnp.repeat(translations[None], invnoise_CTFed_images.shape[0], axis=0), image_shape
    )
...
def reduce_covariance_est_inner_eqx(
    ...
    b = compute_bLambdainvPU_terms(
        mean_projections, u_projections, batch, translations, CTF, jnp.ones_like(noise_variance), config.image_shape
    )
    b = b.swapaxes(-1, -2)
...
def compute_H_B(
    ...
    batch_size = utils.safe_batch_size(
        utils.get_image_batch_size(experiment_dataset.grid_size, gpu_memory - utils.get_size_in_gb(mean_projections))
        / translations.shape[0]
    )
    logger.info(
        "Starting H_B, batch size %s. Remaining memory %s",
        batch_size,
        gpu_memory - utils.get_size_in_gb(mean_projections),
    )
...
def sum_up_images_fixed_rots_covariance_with_precompute(
    shifted_CTFed_images,
    mean_projections,
    CTF,
    gridpoints,
...
def compute_projected_covariance_rhs_lhs(
    ...
    batch_size = utils.safe_batch_size(
        utils.get_image_batch_size(experiment_dataset.grid_size, utils.get_gpu_memory_total())
    )
    ...
    mean_volume = (
        core.VolumeRepr(mean, disc_type=disc_type_mean, prefiltered=True) if disc_type_mean == "cubic" else mean
    )
    basis_volume = (
        core.VolumeRepr(basis, disc_type=disc_type_u, prefiltered=True) if disc_type_u == "cubic" else basis
    )
    for rot_indices in utils.index_batch_iter(n_rotations, batch_size):
        mean_projections[rot_indices] = core.slice_volume(
            mean_volume,
    ...
    # Divide by translations to account for per-translation memory in inner loop
    batch_size = utils.safe_batch_size(
        utils.get_image_batch_size(experiment_dataset.grid_size, memory_left_over_after_kron_allocate)
        / translations.shape[0]
    )
...
def estimate_principal_components_simple(
    experiment_dataset,
    mean,
    ...
    basis, s = principal_components.get_cov_svds(
        cov,
        picked_frequency_indices,
        volume_mask,
        experiment_dataset.volume_shape,
    ...
    covariance = compute_projected_covariance(
        [experiment_dataset],
        mean,
        basis,
        rotations,
        ...
        batch_size,
        disc_type_mean,
        covariance_options["disc_type_u"],
        image_indices=None,
    )
...
def estimate_principal_components_halfset(
    cryos,
    means,
    mean_signal_variance,
    cov_prior,
    ...
    for cryo_idx, cryo in enumerate(cryos):
        Hs[cryo_idx], Bs[cryo_idx] = compute_H_B(
            cryo,
            means[cryo_idx],
            probabilities[cryo_idx],
            rotations,
            translations,
            noise_variance,
            volume_mask,
    ...
    volume_mask = None
    _, covariance_prior, covariance_fscs = principal_components.compute_covariance_regularization_relion_style(
        Hs,
        Bs,
        mean_signal_variance,
        picked_frequency_indices,
    ...
    for cryo_idx, cryo in enumerate(cryos):
        cov_cols[cryo_idx] = relion_functions.post_process_from_filter_v2(
            ...
            tau=covariance_prior,
            kernel=covariance_options["left_kernel"],
            use_spherical_mask=covariance_options["use_spherical_mask"],
            grid_correct=covariance_options["grid_correct"],
        ...
        orthog_cov_cols, _ = principal_components.get_cov_svds(
            cov_cols[cryo_idx],
            picked_frequency_indices,
            volume_mask,
            volume_shape,
