...
def _get_relion_grid_metadata(
    ...
    if symmetry == "C1":
        ...
        from recovar.relion_bind._relion_bind_core import get_healpix_sampling_metadata
...
def _split_rotation_indices(
    ...
    *,
    ...
    symmetry = canonicalize_rotational_symmetry(symmetry)
    n_pixels = (
        hp.nside2npix(2**healpix_order) if symmetry == "C1"
        else int(_get_relion_grid_metadata(int(healpix_order), symmetry)["n_pixels"])
    )
...
def get_oversampled_rotation_grid_from_samples(
    ...
    *,
    ...
    coarse_metadata = (
        None if symmetry == "C1"
        else _get_relion_grid_metadata(int(parent_nside_level), symmetry)
    )
    coarse_n_pixels = (
        hp.nside2npix(2**parent_nside_level) if coarse_metadata is None
        else int(coarse_metadata["n_pixels"])
    )
    ...
    parent_pixels = (
        parent_directions if coarse_metadata is None
