...
def _as_centered_bpref_source(
    ...
    *,
    ...
    """Return the centered cube, center, and radius used for BPref slicing.

    Dense EM historically returned an original-box full cube. The shared
    RELION x-half M-step instead returns its current-size odd BackProjector
    cube after conversion to the public full layout. Both encode the same
    centered support and must feed the same BPref slab conversion.
    """
    ...
    raise ValueError(
        ...
        f"{full_size**3} or a current-size BackProjector cube of size "
        f"{compact_size**3}; got shape {arr.shape}",
    )
