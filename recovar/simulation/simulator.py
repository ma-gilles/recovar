...
def get_dataset_params(n_images, grid_size, ctf_file, poses_file):
    ctf_params = np.array(load_utils.load_ctf_params(grid_size, ctf_file))
    ...
    for mrc_name in mrc_names:
        ...
        if mrc_grid_size == grid_size:
            ...
            if mrc_grid_size > grid_size:
                ...
                X_new[
                    half_diff_size:-half_diff_size, half_diff_size:-half_diff_size, half_diff_size:-half_diff_size
                ] = X
                X = jnp.asarray(X_new)
            ...
            half_pad = padding // 2
            X_padded[
                half_pad : X.shape[0] + half_pad, half_pad : X.shape[1] + half_pad, half_pad : X.shape[2] + half_pad
            ] = X
...
def generate_synthetic_dataset(
    ...
    outlier_volume = (
        fourier_transform_utils.get_dft3(utils.load_mrc(outlier_file_input)).reshape(-1)
    ...
    if streaming_mmap:
        # The mmap'd MRC has been written incrementally throughout
        # simulation + post-processing. Just close it cleanly to flush
        # the header / final pages.
        mrc_file.flush()
        mrc_file.close()
        # Sanity check: file already exists at the expected path.
        if streaming_particles_path != particles_file:
            raise RuntimeError(
                f"streaming MRC was created at {streaming_particles_path} "
                f"but expected {particles_file}",
            )
    else:
...
def generate_simulated_dataset(
    ...
    if n_tilts > 0:
        ...
        tilt_series_assignment = np.random.choice(
            np.arange(volumes.shape[0]), size=n_tilt_groups, p=volume_distribution
        )
        ...
        for i in range(n_tilt_groups):
            ...
            ctf_params[tilt_groups == i, core.CTFParamIndex.CONTRAST] = np.cos(
                x_angles[tilt_numbers[tilt_groups == i]] / 180 * np.pi
            )
    ...
    main_dataset = cryoem_dataset.CryoEMDataset(
        None,
        voxel_size,
        cryoem_dataset.ImageMetadata(rots, trans, ctf_params),
    ...
    if put_extra_particles:
        ...
        other_particles_dataset = cryoem_dataset.CryoEMDataset(
            None,
            voxel_size,
            cryoem_dataset.ImageMetadata(rots_2, trans_2, ctf_params),
            ctf_evaluator=ctf_evaluator,
            grid_size=grid_size,
        )
        # No noise in this stack.
        extra_particles_image_stack = simulate_data(
            other_particles_dataset,
            volumes,
            noise_variance * 0,
            batch_size,
            image_assignments,
            per_image_noise_scale_2,
            per_image_noise_scale,
            seed=0,
            disc_type=disc_type,
            mrc_file=None,
    ...
    if percent_outliers > 0:
        ...
        outlier_contrast, outlier_noise_scale = generate_contrast_params(
            n_outlier_images, noise_scale_std, contrast_std
        )

        outlier_particle_dataset = cryoem_dataset.CryoEMDataset(
            None,
            voxel_size,
            cryoem_dataset.ImageMetadata(rots_3, trans_3, ctf_params_3),
            ctf_evaluator=ctf_evaluator,
            grid_size=grid_size,
        )

        outlier_particle_image_stack = simulate_data(
            outlier_particle_dataset,
            outlier_volume[None],
            noise_variance,
            batch_size,
            np.zeros(n_outlier_images, dtype=int),
            outlier_noise_scale,
            outlier_contrast,
            seed=1,
            disc_type=disc_type,
    ...
    if n_tilts > 0 and percent_tilt_series_outliers > 0:
        ...
        ctf_params_tilt_outliers, rots_tilt_outliers, trans_tilt_outliers = dataset_param_generator(
            n_images_outliers, grid_size
        )
        tilt_outlier_contrast, tilt_outlier_noise_scale = generate_contrast_params(
            n_images_outliers, noise_scale_std, contrast_std
        )

        # Create dataset for tilt outliers
        tilt_outlier_dataset = cryoem_dataset.CryoEMDataset(
            None,
            voxel_size,
        ...
        tilt_outlier_image_stack = simulate_data(
            tilt_outlier_dataset,
            outlier_volume[None],
            noise_variance,
            batch_size,
            np.zeros(n_images_outliers, dtype=int),
            tilt_outlier_noise_scale,
            tilt_outlier_contrast,
            seed=2,
            disc_type=disc_type,
            mrc_file=None,
            premultiplied_ctf=premultiplied_ctf,
...
def simulate_nufft_data_batch_from_pdb(
    atom_group,
    rotation_matrices,
    translations,
    CTF_params,
...
def compute_projections_with_nufft(atom_group, plane_coords, voxel_size):
    ...
    X_ims = np.array(
        gsm.generate_potential_at_freqs_from_atoms(atom_group, voxel_size, plane_coords_vec).astype(np.complex64)
    )
...
def compute_volume_projections_with_nufft(volume, plane_coords, voxel_size):
    ...
    plane_coords_vec = np.array(plane_coords.reshape(-1, 3)).astype(np.float64)
    X_ims = gsm.get_fourier_transform_of_volume_at_freq_coords(
        np.array(volume).astype(np.complex128), plane_coords_vec, voxel_size
    )
    X_ims = X_ims.reshape(plane_coords.shape[:-1])
