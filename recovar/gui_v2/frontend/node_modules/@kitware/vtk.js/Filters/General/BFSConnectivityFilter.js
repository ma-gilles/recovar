...
import Constants from './BFSConnectivityFilter/Constants.js';
...
function vtkBFSConnectivityFilter(publicAPI, model) {
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    } else if (model.extractionMode === ExtractionMode.ExtractionMode_SMALLEST) {
      cells = sortRegions[sortRegions.length - 1];
    } else if (model.extractionMode === ExtractionMode.ExtractionMode_CUSTOM) {
      if (model.extractionIndex >= 0 && model.extractionIndex < model.regionsCount) {
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      for (let j = 0; j < n; j++) {
        ...
        if (!pointMap.has(oldPtId)) {
          ...
          newPointsData.push(oldPoints[pIdx], oldPoints[pIdx + 1], oldPoints[pIdx + 2]);
          pointCounter++;
        }
...
function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.setGet(publicAPI, model, ['extractionMode', 'extractionIndex', 'regionsCount']);
  vtkBFSConnectivityFilter(publicAPI, model);
}
const newInstance = macro.newInstance(extend, 'vtkBFSConnectivityFilter');
var vtkBFSConnectivityFilter$1 = {
  newInstance,
  extend
};

export { vtkBFSConnectivityFilter$1 as default, extend, newInstance };
