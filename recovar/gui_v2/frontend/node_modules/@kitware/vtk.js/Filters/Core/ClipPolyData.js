...
function vtkClipPolyData(publicAPI, model) {
  ...
  function addPoint(pointRef, clipState) {
    ...
    if (inserted) {
      if (pointRef.type === 'original') {
        clipState.outputPointData.passData(clipState.inputPointData, pointRef.pointId, id);
      } else {
        clipState.outputPointData.interpolateData(clipState.inputPointData, pointRef.pointId1, pointRef.pointId2, id, pointRef.t);
      }
  ...
  function makeOriginalRef(pointId, scalar, clipState) {
    ...
    return {
      type: 'original',
      pointId,
      point: [clipState.inputPointsData[offset], clipState.inputPointsData[offset + 1], clipState.inputPointsData[offset + 2]],
      scalar
    };
  }
  ...
  function makeEdgeRef(ref1, ref2) {
    ...
    return {
      ...
      point: [ref1.point[0] + t * (ref2.point[0] - ref1.point[0]), ref1.point[1] + t * (ref2.point[1] - ref1.point[1]), ref1.point[2] + t * (ref2.point[2] - ref1.point[2])],
      scalar: model.value
    };
  }
  ...
  function clipPolygon(values, offset, npts, insideOut, clipState) {
    ...
    for (let i = 0; i < npts; i++) {
      ...
      polygon.push(makeOriginalRef(pointId, clipState.inputScalars[pointId], clipState));
    }
    const result = [];
    ...
    polygon = result;
    const polygonPointIds = polygon.map(pointRef => addPoint(pointRef, clipState));
    const cleanedIds = [];
    for (let i = 0; i < polygonPointIds.length; i++) {
      if (cleanedIds.length === 0 || cleanedIds[cleanedIds.length - 1] !== polygonPointIds[i]) {
        cleanedIds.push(polygonPointIds[i]);
      }
    }
    if (cleanedIds.length > 1 && cleanedIds[0] === cleanedIds[cleanedIds.length - 1]) {
      cleanedIds.pop();
    }
    return cleanedIds;
  }
   ...
   * @param {object} clipState
   */
  function addTriangulatedPolygon(pointIds, polys, inputCellId, outputCellData, clipState) {
    ...
    for (let i = 1; i < pointIds.length - 1; i++) {
      const newCellId = polys.insertNextCell([pointIds[0], pointIds[i], pointIds[i + 1]]);
      outputCellData.passData(clipState.inputCellData, inputCellId, newCellId);
    }
  }
   ...
   */
  function processVerts(cellArray, verts, insideOut, outputCellData, clipState, cellOffset) {
    iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
      for (let i = 0; i < npts; i++) {
        ...
        if (isInside(scalar, insideOut, model.value)) {
          const newPointId = addPoint(makeOriginalRef(pointId, scalar, clipState), clipState);
          const newCellId = verts.insertNextCell([newPointId]);
          outputCellData.passData(clipState.inputCellData, cellOffset + localCellId, newCellId);
        }
      }
    });
  }
   ...
   * @param {number} cellOffset
   */
  function processLines(cellArray, lines, insideOut, outputCellData, clipState, cellOffset) {
    iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
   ...
   * @param {number} cellOffset
   */
  function processPolys(cellArray, polys, insideOut, outputCellData, clipState, cellOffset) {
    const clipTriangle = (id0, id1, id2, inputCellId) => {
      ...
      clipState.triangleCell.clip(model.value, clipState.triangleScalars, model.locator, polys, clipState.inputPointData, clipState.outputPointData, clipState.inputCellData, inputCellId, outputCellData, insideOut);
    };
    iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
      if (npts === 3) {
        clipTriangle(values[offset], values[offset + 1], values[offset + 2], cellOffset + localCellId);
        return;
      }
      const polygonIds = clipPolygon(values, offset, npts, insideOut, clipState);
      addTriangulatedPolygon(polygonIds, polys, cellOffset + localCellId, outputCellData, clipState);
    });
  }
   ...
   * @param {number} cellOffset
   */
  function processStrips(cellArray, polys, insideOut, outputCellData, clipState, cellOffset) {
    const clipTriangle = (id0, id1, id2, inputCellId) => {
      const pointIds = clipState.trianglePointIds;
      pointIds[0] = id0;
      pointIds[1] = id1;
    ...
    iterateCellArray(cellArray, (values, offset, npts, localCellId) => {
      for (let i = 0; i < npts - 2; i++) {
        if (i % 2 === 0) {
          clipTriangle(values[offset + i], values[offset + i + 1], values[offset + i + 2], cellOffset + localCellId);
        } else {
          clipTriangle(values[offset + i + 1], values[offset + i], values[offset + i + 2], cellOffset + localCellId);
        }
      }
    });
  }
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    const inputScalars = evaluateImplicitFunction(input.getPoints(), model.clipFunction);
    if (!inputScalars) {
    ...
    let cellOffset = 0;
    processVerts(input.getVerts(), mainVerts, model.insideOut, output.getCellData(), clipState, cellOffset);
    if (model.generateClippedOutput) {
      processVerts(input.getVerts(), clippedVerts, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
    }
    cellOffset += countCells(input.getVerts());
    processLines(input.getLines(), mainLines, model.insideOut, output.getCellData(), clipState, cellOffset);
    if (model.generateClippedOutput) {
      processLines(input.getLines(), clippedLines, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
    }
    cellOffset += countCells(input.getLines());
    processPolys(input.getPolys(), mainPolys, model.insideOut, output.getCellData(), clipState, cellOffset);
    if (model.generateClippedOutput) {
      processPolys(input.getPolys(), clippedPolys, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
    }
    cellOffset += countCells(input.getPolys());
    processStrips(input.getStrips(), mainPolys, model.insideOut, output.getCellData(), clipState, cellOffset);
    if (model.generateClippedOutput) {
      processStrips(input.getStrips(), clippedPolys, !model.insideOut, clippedOutput.getCellData(), clipState, cellOffset);
    }
    ...
    if (model.generateClipScalars) {
      const clipScalarValues = new Float32Array(clipState.newPoints.getNumberOfPoints());
      for (let i = 0; i < clipScalarValues.length; i++) {
        clipScalarValues[i] = Number.isFinite(clipState.clipScalarValues[i]) ? clipState.clipScalarValues[i] : 0;
      }
    ...
    vtkDebugMacro(`Created: ${clipState.newPoints.getNumberOfPoints()} points, ` + `${mainVerts.getNumberOfCells()} verts, ` + `${mainLines.getNumberOfCells()} lines, ` + `${mainPolys.getNumberOfCells()} polys`);
  };
}
...
function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.setGet(publicAPI, model, ['clipFunction', 'insideOut', 'value', 'generateClipScalars', 'generateClippedOutput', 'outputPointsPrecision', 'locator']);
  vtkClipPolyData(publicAPI, model);
}
const newInstance = macro.newInstance(extend, 'vtkClipPolyData');
var vtkClipPolyData$1 = {
  newInstance,
  extend
};

export { vtkClipPolyData$1 as default, extend, newInstance };
