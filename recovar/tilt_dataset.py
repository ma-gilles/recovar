...
        critical_exp = jnp.mul(critical_exp, scale_factor * 0.245)
        return torch.add(critical_exp, 2.81)

    def get_dose_filters(self, tilt_index, frequencies, Apix, grid_size, dose_per_tilt, tilt_numbers):
        D = grid_size

        N = len(tilt_index)
        # 
        freqs = frequencies / Apix  # D/A
        x = freqs[..., 0]
        y = freqs[..., 1]
        s2 = x**2 + y**2
        s = jnp.sqrt(s2)
...
        ce = self.critical_exposure(s)
        ce_tile = jnp.repeat(ce, N, axis=0) #ce.repeat(N, 1)

        oe_tile = ce_tile * 2.51284  # Optimal exposure
        oe_mask = (cd_tile < oe_tile)#.long()

        freq_correction = jnp.exp(-0.5 * cd_tile / ce_tile)
        freq_correction = jnp.mul(freq_correction, oe_mask)
        angle_correction = jnp.cos(self.tilt_angles[tilt_index] * np.pi / 180)
