...
def add_args(parser: argparse.ArgumentParser):
    ...
    mk.add_argument(
        ...
        type=int,
        default=0,
        dest="mask_dilate_iter",
        help="Iterations to dilate solvent and focus mask",
    )
    mk.add_argument(
        "--dilated-mask-dilation-iters",
        type=int,
        default=None,
...
def _peek_image_size(particles_file: str, datadir: str = "", strip_prefix=None) -> int:
    ...
    loader = load_images(particles_file, lazy=True, datadir=datadir or "", strip_prefix=strip_prefix)
...
def _check_uninvert_data(means, dataset, args):
    ...
    if args.uninvert_data == "automatic":
        if uninvert_check:
            ...
            logger.warning(
                "sum(mean) < 0! Swapping sign of data (uninvert-data = true). If this is not what you want, explicitely set uninvert-data=False "
            )
        else:
...
def _estimate_noise(dataset, means, dilated_volume_mask, batch_size, args, noise_model, gpu_budget_gb=None):
    ...
    radial_noise_var_ubed = np.where(
        radial_noise_var_outside_mask > radial_ub_noise_var, radial_ub_noise_var, radial_noise_var_outside_mask
    )
...
def _compute_embeddings(means, u, s, dataset, volume_mask, options, gpu_memory, focus_masks, zdim_for_rest, args):
    ...
    n_pcs_list = [(num_foc_masks - 1) * zdim_for_rest + zdim for zdim in options.zs_dim_to_test]
...
def _prepare_downsampled_input(args):
    """Reuse or create a downsampled stack, then point ``args`` at its STAR file.

    The caller resolves the target and sets the memory budget first. Preserve
    the original particle path and requested size for output metadata; clear
    input-directory and downsampling overrides once the cached STAR owns them.
    """
...
def standard_recovar_pipeline(args):
    ...
    if getattr(args, "tomograms", None) is not None:
        ...
        logger.info("Converting RELION5 tomo data: %s + %s -> %s", args.tomograms, args.particles, converted_star)
    ...
    if args.tilt_series and args.dose_per_tilt is not None:
        logger.warning(
            "dose_per_tilt is provided, but tilt_series_ctf is set to using starfile = -B_fac/4 (by default). Thus, dose_per_tilt will not be used."
        )

    if (args.tilt_series_ctf == "v2_scale_from_star") and (args.angle_per_tilt is not None):
        logger.warning(
            "angle_per_tilt is provided, but tilt_series_ctf is set to using scale from inputfile (by default). Thus, angle_per_tilt will not be used."
        )
    ...
    _prepare_downsampled_input(args)
    ...
    from recovar.utils.parser_args import apply_memory_planning_args

    log_backend(logger)
    plan, mem_trace = apply_memory_planning_args(
        args,
        command="pipeline",
    ...
    gpu_memory = plan.budget.effective_budget_gb
    volume_shape = ds.volume_shape
    batch_size = plan.image_batch_size
    logger.info(
        "image batch size: %s, volume batch size: %s, column batch size: %s",
        plan.image_batch_size,
        plan.volume_batch_size,
        plan.column_batch_size,
    )
    ...
    for repeat in range(n_repeats):
        ...
        output.save_volume(
            means.combined, os.path.splitext(paths.mean_volume)[0], volume_shape, from_ft=True, voxel_size=ds.voxel_size
        )
        output.save_volume(
            means.corrected0,
            os.path.splitext(paths.mean_half1_unfil)[0],
            volume_shape,
            from_ft=True,
        ...
        output.save_volume(
            means.corrected1,
            os.path.splitext(paths.mean_half2_unfil)[0],
            volume_shape,
            from_ft=True,
            voxel_size=ds.voxel_size,
        )
        output.save_volume(
            volume_mask, os.path.splitext(paths.mask_volume)[0], volume_shape, from_ft=False, voxel_size=ds.voxel_size
        )
        ...
        output.save_volume(
            best_filtered_nob,
            os.path.splitext(paths.mean_filtered)[0],
            volume_shape,
            from_ft=False,
            voxel_size=ds.voxel_size,
        )
        ...
        # Override with the centralized memory plan's n_pcs choice. When
        # adaptive-n-pcs is set the planner already consulted the
        # calibration table; otherwise this is a no-op (200).
        if plan.n_pcs_to_compute != covariance_options.get("n_pcs_to_compute"):
            logger.info(
                "Memory plan overriding covariance n_pcs: %s -> %s (calibration_status=%s)",
    ...
    if mem_trace is not None:
        mem_trace.record("after_embedding")

    peak_gpu_gb = utils.get_peak_gpu_memory_used(device=0)
    logger.info("peak GPU memory used: %s", peak_gpu_gb)

    # End-of-run memory budget check.
    # Production: warn-and-continue.
    # Test mode (--fail-on-memory-exceed): exit non-zero with structured
    # summary if peak > requested * 1.05.
    if args.gpu_memory is not None and peak_gpu_gb is not None:
        try:
            peak_val = float(peak_gpu_gb)
        except (TypeError, ValueError):
            peak_val = None

        # Honor RECOVAR_DEBUG_FORCE_PEAK_GB so unit tests can deterministically
        # trigger the over-budget code path.
        from recovar.utils.memory_planner import _debug_force_peak_gb as _force

        forced = _force()
        if forced is not None:
            peak_val = float(forced)
            logger.info("RECOVAR_DEBUG_FORCE_PEAK_GB=%.2f overriding observed peak", forced)

        if peak_val is not None and peak_val > args.gpu_memory:
    ...
    output.save_covar_output_volumes(
        paths.output_dir, means.combined, u["rescaled"], s, volume_mask, volume_shape, voxel_size=ds.voxel_size
    )
    output.save_volume(
        volume_mask, os.path.splitext(paths.mask_volume)[0], volume_shape, from_ft=False, voxel_size=ds.voxel_size
    )
    output.save_volume(
        dilated_volume_mask,
        os.path.splitext(paths.dilated_mask_volume)[0],
        volume_shape,
        from_ft=False,
        voxel_size=ds.voxel_size,
    )
    ...
    output.save_volume(
        focus_mask, os.path.splitext(paths.focus_mask_volume)[0], volume_shape, from_ft=False, voxel_size=ds.voxel_size
    )
    if args.use_complement_mask:
        output.save_volume(
            focus_masks[0],
            os.path.splitext(paths.complement_mask_volume)[0],
            volume_shape,
            from_ft=False,
            voxel_size=ds.voxel_size,
        )
    ...
    if args.tilt_series:
        particles_ind_split = [ds.halfset_original_group_indices(halfset_id) for halfset_id in range(2)]
    else:
