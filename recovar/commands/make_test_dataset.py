...
def make_test_dataset(
    ...
    *,
    ...
    if tilt_series:
        ...
        logger.info("Generating tilt series with %d particles and %d tilts per particle", n_particles, n_tilts)
    ...
    if tilt_series and percent_tilt_series_outliers > 0:
        logger.info("Generated tilt series dataset with %.1f%% tilt outliers", percent_tilt_series_outliers * 100)
...
def main():
    ...
    parser.add_argument(
        "--grid-size", type=int, default=None, help="Alias for --image-size (takes precedence when set)"
    )
    parser.add_argument(
        "--volume-input", default=None, help="Optional input volume prefix (e.g. /path/to/vol for vol0000.mrc, ...)"
    )
    parser.add_argument(
        "--n-tilts", type=int, default=None, help="Number of tilts when --tilt-series is enabled (default: 27)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--nested-prefix", default="Extract/job193", help="Prefix path for nested structure (default: Extract/job193)"
    )
    parser.add_argument(
        "--tilt-series", action="store_true", help="Generate tilt series dataset instead of single particle dataset"
    )
    ...
    make_test_dataset(
        ...
        args.noise_level,
        args.n_images,
        args.create_nested_structure,
        args.nested_prefix,
        args.tilt_series,
        args.outlier_file_input,
        args.percent_outliers,
        args.percent_tilt_series_outliers,
        args.seed,
