...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="List of alphas for regularization (default (1e-9, 1e-8, ..., 1e1)",
    )
    parser.add_argument(
        "--percentile_bound",
        type=int,
...
def estimate_conformational_density(
    recovar_result_dir,
    output_dir=None,
    pca_dim=4,
    z_dim_used=4,
    ...
    lbfgsb_sols, alphas, cost, reg_cost, density, total_covar, grids, bounds = (
        deconvolve_density.get_deconvolved_density(
            pipeline_output,
            zdim=z_dim_used,
            noreg=True,
            pca_dim_max=pca_dim,
