...
def add_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "input", type=str, help="Path to first halfmap (.mrc file) OR directory containing volume subdirectories"
    )

    parser.add_argument(
        "--halfmap2", type=str, default=None, help="Path to second halfmap (if not provided, will auto-detect)"
    )
    ...
    parser.add_argument(
        "--B-factor", type=float, default=None, help="B-factor for sharpening in Angstroms^2 (default: no B-factor)"
    )

    parser.add_argument(
        "--mask-radius", type=float, default=None, help="Radius of spherical mask in Angstroms (default: no mask)"
    )

    parser.add_argument(
        "--fsc-mask", type=str, default=None, help="Path to a mask .mrc file to use for FSC calculation (optional)"
    )

    parser.add_argument(
        "--batch", action="store_true", help="Process all volumes in a directory (input should be volumes directory)"
    )
    ...
    parser.add_argument(
        "--local", action="store_true", help="Use local resolution filtering instead of global filtering"
    )
    ...
    parser.add_argument(
        "--locres-minres", type=float, default=50.0, help="Minimum local resolution in Angstroms (default: 50.0)"
    )

    parser.add_argument(
        "--fsc-threshold", type=float, default=1 / 7, help="FSC threshold for resolution determination (default: 1/7)"
    )

    parser.add_argument(
        "--filter-edgewidth", type=int, default=2, help="Edge width for FSC filtering in pixels (default: 2)"
    )
...
def estimate_bfactor_from_halfmaps(halfmap1, voxel_size, plot_path=None):
    ...
    if any(s < min_size for s in halfmap1.shape):
        raise ValueError(
            f"Volume too small for reliable B-factor estimation. Minimum size: {min_size}, got: {halfmap1.shape}"
        )
    ...
    try:
        ...
        valid_freq = valid_basic & (freq_shells >= min_freq) & (freq_shells <= max_freq)
        ...
        logger.info(
            "Resolution range: %.1f to %.1f Å", 1 / freq_shells[valid_freq].max(), 1 / freq_shells[valid_freq].min()
        )
        ...
        if plot_path is not None:
            try:
                _create_bfactor_plot_robust(
                    freq_shells, log_power, valid_freq, x, y, bfactor, log_p0, fit_method, r_squared, plot_path
                )
...
def _create_bfactor_plot_robust(
    freq_shells, log_power, valid_mask, x_fit, y_fit, bfactor, log_p0, fit_method, r_squared, plot_path
):
    ...
    # Add summary text
    fig.suptitle(f"B-factor Estimation: {bfactor:.1f} Å² (R²={r_squared:.3f})", fontsize=14, fontweight="bold")
...
def estimate_bfactor_from_fsc_weighted_halfmaps(
    ...
    for idx, (label, fsc_external, mask) in enumerate(fscs_to_plot):
        ...
        # Prepare for fit
        valid = np.isfinite(amp_weighted) & (amp_weighted > 0) & (bin_counts >= 3) & (freq_shells > 0)
        ...
        if fit_method == "linear_regression":
            ax2.plot(
                ...
                color="tab:red",
                lw=2,
                label=f"Fit (B={bfactor:.1f} Å², R²={r_squared:.3f})",
            )
            annotation = (
                f"B-factor = {bfactor:.1f} Å²\nR² = {r_squared:.3f}\nFit region: {fit_range[0]:.3f}-{fit_range[1]:.3f}"
            )
            ax2.annotate(
                annotation,
                xy=(0.05, 0.95),
...
def local_filter_halfmaps(
    ...
    apply_mask_path=None,
    locres_sampling=25.0,
    locres_maskrad=None,
    ...
    if estimate_B_factor:
        ...
        if B_factor is None:
            ...
            logger.warning(
                "Using user-supplied B-factor %s, ignoring estimated value %.2f", B_factor, estimated_bfactor
            )
    ...
    filtered_combined, local_resol_map, local_auc_map, fscs, local_resols = locres.local_resolution(
        halfmap1,
        halfmap2,
        B_factor if B_factor is not None else 0,
        voxel_size,
...
def postprocess_halfmaps(
    ...
    filtered_combined, fsc, global_resol = locres.filter_maps_with_global_fsc(
        halfmap1, halfmap2, voxel_size, mask_radius=mask_radius, B_factor=B_factor, fsc_mask=fsc_mask, mask=apply_mask
    )
...
def batch_process_volumes_local(
    volumes_dir,
    output_dir,
    voxel_size,
    B_factor=None,
    ...
    for vol_dir in vol_dirs:
        ...
        try:
            median_resol = local_filter_halfmaps(
                ...
                fsc_mask_path=fsc_mask_path,
                estimate_B_factor=estimate_B_factor,
                apply_mask_path=apply_mask_path,
                locres_sampling=locres_sampling,
                locres_maskrad=locres_maskrad,
                locres_edgwidth=locres_edgwidth,
                locres_minres=locres_minres,
                fsc_threshold=fsc_threshold,
                filter_edgewidth=filter_edgewidth,
            )
            ...
            results[vol_name] = {"error": str(e), "success": False}
            continue
...
def batch_process_volumes(
    ...
    for vol_dir in vol_dirs:
        ...
        try:
            ...
            results[vol_name] = {"global_resolution": global_resol, "filtered_path": output_path, "success": True}
            ...
            try:
                if args.local:
                    results = batch_process_volumes_local(
                        args.input,
                        args.output,
                        args.voxel_size if hasattr(args, "voxel_size") else None,
                        B_factor=args.B_factor,
                        mask_radius=args.mask_radius,
                        fsc_mask_path=args.fsc_mask,
                        estimate_B_factor=args.estimate_B_factor,
                        apply_mask_path=args.apply_mask,
                        locres_sampling=args.locres_sampling,
                        locres_maskrad=args.locres_maskrad,
                        locres_edgwidth=args.locres_edgwidth,
                        locres_minres=args.locres_minres,
                        fsc_threshold=args.fsc_threshold,
                        filter_edgewidth=args.filter_edgewidth,
                    )
                else:
