...
def run_pipeline_with_outlier_removal():
    ...
    parser.add_argument(
        "--delete-rounds", action="store_true", help="Delete results of all rounds except the inliers/outliers"
    )

    # Add comprehensive outlier detection arguments
    parser.add_argument("--use-contrast-detection", action="store_true", help="Use contrast-based outlier detection")
    parser.add_argument(
...
def _run_pipeline_with_outlier_removal_impl(args):
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[_RobustFileHandler(os.path.join(args.outdir, "run.log")), _RobustStreamHandler()],
    )
    ...
    for k in range(args.k_rounds):
        ...
        if current_image_indices is not None:
            ...
            if args.tilt_series and current_particle_indices is not None:
                particle_indices_filename = os.path.join(args.outdir, f"particle_inliers_round_{k}.pkl")
        ...
        outlier_argv = [
            ...
            str(zdim_key),
            "--output-dir",
            os.path.join(args.outdir, "outlier_detection"),
        ]
        ...
        if args.use_contrast_detection:
            outlier_argv.extend(
                [
                    "--low-contrast-threshold",
                    str(args.low_contrast_threshold),
                    "--high-contrast-threshold",
                    str(args.high_contrast_threshold),
                    "--max-contrast",
                    str(args.max_contrast),
                    "--particle-bad-fraction-threshold",
                    str(args.particle_bad_fraction_threshold),
                    "--micrograph-bad-fraction-threshold",
                    str(args.micrograph_bad_fraction_threshold),
                ]
            )
        ...
        combined_inliers_file = os.path.join(
            outlier_output_dir, "data", "combined_results", f"combined_image_inliers_{zdim_key}.pkl"
        )
        ...
        current_particle_indices = None
        if args.tilt_series:
            combined_particle_inliers_file = os.path.join(
                outlier_output_dir, "data", "combined_results", f"combined_particle_inliers_{zdim_key}.pkl"
            )
        ...
        combined_outliers_file = os.path.join(
            outlier_output_dir, "data", "combined_results", f"combined_image_outliers_{zdim_key}.pkl"
        )
        ...
        if args.tilt_series and current_particle_indices is not None:
            ...
            combined_particle_outliers_file = os.path.join(
                outlier_output_dir, "data", "combined_results", f"combined_particle_outliers_{zdim_key}.pkl"
            )
                ...
                try:
                    ...
                    for item in os.listdir(dir_path):
                        ...
                        if os.path.isdir(item_path):
                            ...
                            if not item.endswith(".pkl") or (
                                not item.startswith("inliers_round_") and not item.startswith("outliers_round_")
                            ):
                                os.remove(item_path)
