...
does NOT import any trajectory-generation / 5nrl machinery — it is built
around an arbitrary stack of pre-rendered volumes (one MRC per
conformational state) sitting at ``<output_dir>/01_raw_volumes/vol####.mrc``.

Pipeline stages
---------------

1. ``01_raw_volumes/`` — the input stack. Must exist before running.
   Use ``recovar.commands.render_spike_morph_volumes`` (or any other
   renderer) to produce these. Voxel size is read from the MRC header.
...
def run_walkthrough(args, out: Path) -> dict:
    ...
    if n_raw == 0:
        raise SystemExit(
            f"No volumes found in {raw_dir}. Render them first, e.g.:\n"
            f"  python -m recovar.commands.render_spike_morph_volumes {out} --pdb-dir <DIR>"
        )
...
1. `01_raw_volumes/` — user-supplied volumes (must be pre-rendered).
...
def add_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument(
        ...
        required=True,
        help="Run dir. Must already contain 01_raw_volumes/vol####.mrc.",
    )
