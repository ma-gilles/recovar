...
def plot_anomaly_detection_results(zs, original_indices, folder_name, save_all_plots=False):
    ...
    n_cols = n_algorithms
    fig, axes = plt.subplots(nrows=n_rows, ncols=n_cols, figsize=(n_cols * 6, n_rows * 5), squeeze=False)
    plt.subplots_adjust(left=0.05, right=0.95, bottom=0.07, top=0.90, wspace=0.3, hspace=0.4)
    ...
    for i_algo, (name, y_pred) in enumerate(zip(algorithm_names, predictions)):
        ...
        if zs.shape[1] >= 2:
            ...
            if np.sum(inliers) > 1:
                ...
                hb = ax.hexbin(zs[inliers, 0], zs[inliers, 1], gridsize=50, cmap="Blues", bins="log", mincnt=1)
                # Add colorbar
                ...
                ax.text(
                    0.5,
                    0.5,
                    "Insufficient inliers for hexbin",
        ...
        if zs.shape[1] >= 4:
            ...
            if np.sum(inliers) > 1:
                ...
                hb = ax.hexbin(zs[inliers, 2], zs[inliers, 3], gridsize=50, cmap="Blues", bins="log", mincnt=1)
                # Add colorbar
                cb = fig.colorbar(hb, ax=ax)
                cb.set_label("log$_{10}$(N)")
            else:
        ...
        if umap_valid.shape[1] >= 2:
            ...
            if np.sum(inliers) > 1:
                hb = ax.hexbin(
                    umap_valid[inliers, 0], umap_valid[inliers, 1], gridsize=50, cmap="Blues", bins="log", mincnt=1
                )
                # Add colorbar
                cb = fig.colorbar(hb, ax=ax)
                cb.set_label("log$_{10}$(N)")
...
def outlier_detection_from_contrast(
    pipeline_output,
    zdim_key=4,
    ...
    original_image_indices = np.concatenate(pipeline_output.get("halfsets")).astype(np.int32, copy=False)
    original_particle_indices = np.concatenate(pipeline_output.get("particles_halfsets")).astype(np.int32, copy=False)
    ...
    logger.info(
        "  Low contrast outliers (< %s): %d (%.1f%%)",
        low_contrast_threshold,
        n_low_contrast,
        n_low_contrast / n_images * 100,
    )
    logger.info(
        "  High contrast outliers (> %s): %d (%.1f%%)",
        high_contrast_threshold,
        n_high_contrast,
        n_high_contrast / n_images * 100,
    )
    logger.info(
        "  Total individual outliers: %d (%.1f%%)", n_individual_outliers, n_individual_outliers / n_images * 100
    )
    ...
    if starfile is not None and starfile.endswith(".star"):
        ...
        micrographtilt_to_tilts, tilts_to_micrographtilt = (
            image_backends.TiltSeriesDataset.parse_micrograph_tilt_mapping(starfile)
        )
    else:
        logger.info(
            "Starfile %s is not a .star file, skipping particle and micrograph-based outlier detection", starfile
        )
    ...
    if particle_to_tilts is not None:
        logger.info("\nParticle-based outlier detection:")
        logger.info(
            "Total of particles: %s, of which %s were used in pipeline",
            len(particle_to_tilts),
            original_particle_indices.size,
        )
        ...
        logger.info(
            "  Total particle-based outliers: %s (%.1f%% of particles)",
            len(particle_outliers),
            len(particle_outliers) / (len(particle_outliers) + len(particle_inliers)) * 100,
        )
        logger.info(
            "  Corresponding to: %s (%.1f%% of images)",
            np.sum(outliers_image_identified_by_particle),
            np.sum(outliers_image_identified_by_particle) / n_images * 100,
        )
    ...
    if micrographtilt_to_tilts is not None:
        ...
        if len(micrographtilt_to_tilts) > 0:
            ...
            logger.info(
                "Number of micrographs with less than 10 images: %s", np.sum([size < 10 for size in micrograph_sizes])
            )
        ...
        logger.info(
            "  Total micrograph-based outliers: %s (%.1f%%) of micrographs",
            len(micrograph_outliers),
            len(micrograph_outliers) / (len(micrograph_outliers) + len(micrograph_inliers)) * 100,
        )
        logger.info(
            "  Corresponding to: %s (%.1f%% of images)",
            np.sum(outliers_image_identified_by_micrograph),
            np.sum(outliers_image_identified_by_micrograph) / n_images * 100,
        )
    ...
    if (particle_to_tilts is not None) or (micrographtilt_to_tilts is not None):
        ...
        if individual_count > 0 and particle_count > 0:
            ...
            logger.info(
                "  Individual-Particle overlap: %s (%.1f%% of individual)", overlap, overlap / individual_count * 100
            )
        if individual_count > 0 and micrograph_count > 0:
            overlap = np.sum(individual_outliers & outliers_image_identified_by_micrograph)
            logger.info(
                "  Individual-Micrograph overlap: %s (%.1f%% of individual)", overlap, overlap / individual_count * 100
            )
        if particle_count > 0 and micrograph_count > 0:
            ...
            logger.info(
                "  Particle-Micrograph overlap: %s (%.1f%% of micrographs)",
                overlap,
                overlap / np.sum(outliers_image_identified_by_micrograph) * 100,
            )
    ...
    if output_dir is not None and save_all_plots:
        ...
        plt.axvline(
            x=low_contrast_threshold,
            color="orange",
            linestyle="--",
            linewidth=2,
            label=f"Low threshold ({low_contrast_threshold})",
        )
        plt.axvline(
            x=high_contrast_threshold,
            color="orange",
            linestyle="--",
            linewidth=2,
            label=f"High threshold ({high_contrast_threshold})",
        )
        plt.xlabel("Contrast")
...
Individual outliers: {n_individual_outliers} ({n_individual_outliers / n_images * 100:.1f}%)"""
            ...
            if particle_to_tilts is not None and "particle_median_contrasts" in locals():
                ...
                if len(particle_contrast_values) > 0:
                    ...
                    axes[0].text(
                        0.5,
                        0.5,
                        "No particle contrast data available",
                ...
                axes[0].text(
                    0.5, 0.5, "No particle grouping available", transform=axes[0].transAxes, ha="center", va="center"
                )
                axes[0].set_title("Median Contrast per Particle")
            ...
            if micrographtilt_to_tilts is not None and "micrograph_median_contrasts" in locals():
                ...
                if len(micrograph_contrast_values) > 0:
                    ...
                    axes[1].text(
                        0.5,
                        0.5,
                        "No micrograph contrast data available",
                ...
                axes[1].text(
                    0.5, 0.5, "No micrograph grouping available", transform=axes[1].transAxes, ha="center", va="center"
                )
                axes[1].set_title("Median Contrast per Micrograph")

            plt.tight_layout()
            plt.savefig(
                os.path.join(contrast_plots_dir, "median_contrast_distributions.png"), dpi=300, bbox_inches="tight"
            )
            plt.close()
    ...
    combined_image_outliers = (
        individual_outliers | outliers_image_identified_by_particle | outliers_image_identified_by_micrograph
    )
...
def create_particle_outlier_visualization(
    all_particle_outliers, method_names, output_dir, zdim_key, total_particles, save_all_plots=False
):
    """
    ...
    for i in range(n_methods):
        for j in range(n_methods):
            text = ax1.text(j, i, str(overlap_matrix[i, j]), ha="center", va="center", color="black", fontweight="bold")
        ...
        for j in range(n_methods):
            if i == j:
                ...
                if percentage_matrix[i, j] > 0:
                    ...
                    text = ax2.text(j, i, "0%\n(0)", ha="center", va="center", color="black", fontweight="bold")
    ...
    plt.savefig(
        os.path.join(particle_overlap_plots_dir, "particle_outlier_method_overlap_matrix.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
            ...
            for j, other_method in enumerate(method_names):
                if i != j:
                    if overlap_matrix[i, i] > 0:
                        ...
                        f.write(
                            f"  Overlap with {other_method}: {overlap_matrix[i, j]} ({overlap_pct:.1f}% of {method} outliers)\n"
                        )
                    else:
...
def create_overlap_matrix_visualization(
    all_outliers, method_names, output_dir, zdim_key, total_particles, save_all_plots=False
):
    """
    ...
    logger.info(
        "Method overlap matrix saved to: %s", os.path.join(overlap_plots_dir, "outlier_method_overlap_matrix.png")
    )
...
def _run_outlier_detection(args):
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[RobustFileHandler(os.path.join(args.output_dir, "outlier_detection.log")), RobustStreamHandler()],
    )
    ...
    if is_tilt_series:
        ...
        logger.info(
            "Tilt series dataset: %s particles, %s total images",
            total_particles,
            len(image_halfsets[0]) + len(image_halfsets[1]),
        )
    else:
    ...
    logger.info(
        "Particles halfsets: %s + %s = %s particles",
        len(particles_halfsets[0]),
        len(particles_halfsets[1]),
        len(particles_halfsets[0]) + len(particles_halfsets[1]),
    )
    logger.info(
        "Image halfsets: %s + %s = %s images",
        len(image_halfsets[0]),
        len(image_halfsets[1]),
        len(image_halfsets[0]) + len(image_halfsets[1]),
    )
    ...
    plot_anomaly_detection_results(
        zs, np.sort(original_particle_indices), anomaly_output_dir, save_all_plots=save_all_plots
    )
    ...
    logger.info(
        "Anomaly detection completed. Found %s inliers and %s particle outliers.",
        len(anomaly_inliers),
        len(anomaly_outliers),
    )
    ...
    if zdim_key in pipeline_output.get_embedding_keys(contrast_entry):
        ...
        logger.info(
            "Contrast-based outlier detection completed. Found %s image inliers and %s image outliers.",
            len(contrast_image_inliers),
            len(contrast_image_outliers),
        )
    else:
        original_image_indices = None
        contrast_image_inliers = None
    ...
    original_particle_indices = np.sort(
        np.concatenate(pipeline_output.get("particles_halfsets")).astype(np.int32, copy=False)
    )
    if original_image_indices is None:
        original_image_indices = np.sort(np.concatenate(pipeline_output.get("halfsets")).astype(np.int32, copy=False))
    else:
        original_image_indices = np.sort(original_image_indices)
    ...
    if len(all_particle_outliers) > 0:
        create_particle_outlier_visualization(
            all_particle_outliers,
            particle_method_names,
            combined_output_dir,
            zdim_key,
    ...
    if anomaly_outliers is not None:
        ...
        if is_tilt_series:
            ...
            anomaly_image_outliers = map_particle_original_indexing_to_images_original_indexing(
                anomaly_outliers, original_image_indices, starfile
            )
        else:
    ...
    if junk_outliers is not None:
        ...
        if is_tilt_series:
            # For tilt series, map particle outliers to image outliers
            junk_image_outliers = map_particle_original_indexing_to_images_original_indexing(
                junk_outliers, original_image_indices, starfile
            )
        else:
            # For regular datasets, particle outliers = image outliers
    ...
    if is_tilt_series:
        particle_outliers_to_image_outliers = map_particle_original_indexing_to_images_original_indexing(
            combined_particle_outliers, original_image_indices, starfile
        )
    ...
    if save_all_plots:
        ...
        with open(breakdown_file, "w") as f:
            ...
            f.write(
                f"Combined image inliers: {len(combined_image_inliers)} ({len(combined_image_inliers) / original_image_indices.size * 100:.1f}%)\n\n"
            )
            if len(all_image_outliers) > 0:
                for i, (outliers, method) in enumerate(zip(all_image_outliers, image_method_names)):
                    f.write(
                        f"{method}: {len(outliers)} outliers ({len(outliers) / original_image_indices.size * 100:.1f}%)\n"
                    )
            else:
    ...
    if len(all_image_outliers) > 1:
        create_overlap_matrix_visualization(
            all_image_outliers,
            image_method_names,
            combined_output_dir,
            zdim_key,
            original_image_indices.size,
            save_all_plots=save_all_plots,
        )
    ...
    if len(all_particle_outliers) > 0:
        create_outlier_visualizations(
            ...
            all_particle_outliers,
            particle_method_names,
            combined_particle_outliers,
            combined_particle_inliers,
            args.output_dir,
            zdim_key,
            total_particles,
            is_tilt_series,
    ...
    logger.info(
        "Combined image outliers: %s (%.1f%%)",
        len(combined_image_outliers),
        len(combined_image_outliers) / original_image_indices.size * 100,
    )
    logger.info(
        "Combined particle outliers: %s (%.1f%%)",
        len(combined_particle_outliers),
        len(combined_particle_outliers) / original_particle_indices.size * 100,
    )
...
def map_particle_original_indexing_to_images_original_indexing(
    particle_indices_in_original_ordering, image_subset, starfile
):
...
def add_args(parser):
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        help="Output directory for results (default: pipeline_output_dir/outlier_detection)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-contrast", type=float, default=4.0, help="Maximum contrast value to consider (default: 4.0)"
    )
    parser.add_argument(
        "--particle-bad-fraction-threshold",
        type=float,
        default=0.7,
        help="Threshold for bad fraction in particle (default: 0.7)",
    )
    parser.add_argument(
        "--micrograph-bad-fraction-threshold",
        type=float,
        default=0.7,
    ...
    parser.add_argument(
        "--use-junk-detection", action="store_true", help="Run junk particle detection in addition to outlier detection"
    )
    parser.add_argument(
        "--junk-threshold", type=float, default=0.5, help="Threshold for junk particle detection (default: 0.5)"
    )
    parser.add_argument(
...
def create_outlier_visualizations(
    ...
    def create_method_plots(outlier_indices, inlier_indices, method_name, save_prefix):
        ...
        try:
            ...
            hb = ax.hexbin(
                zs[:, 0], zs[:, 1], gridsize=gridsize, cmap="Blues", alpha=0.4, mincnt=1, reduce_C_function=np.mean
            )
        except (ValueError, TypeError):
            pass
        ...
        try:
            gridsize = min(50, max(20, int(np.sqrt(len(zs) / 100))))
            hb = ax.hexbin(
                zs[:, 2], zs[:, 3], gridsize=gridsize, cmap="Blues", alpha=0.4, mincnt=1, reduce_C_function=np.mean
            )
        except (ValueError, TypeError):
            pass
        ...
        if contrast_values is not None:
            ...
            if outlier_image_indices.size + inlier_image_indices.size != original_image_indices.size:
                ...
                logger.info(
                    "Number of images in outlier indices + inlier indices: %s",
                    outlier_image_indices.size + inlier_image_indices.size,
                )
                raise ValueError(
                    "Number of images in outlier indices + inlier indices does not match original image indices"
                )
            ...
            if outlier_image_indices is not None and len(outlier_image_indices) > 0:
                ...
                ax.hist(
                    ...
                    label=f"Outliers (n={len(outlier_contrast):,})",
                    density=True,
                    edgecolor="black",
                    linewidth=0.8,
                    hatch="///",
            ...
            if inlier_image_indices is not None and len(inlier_image_indices) > 0:
                ...
                ax.hist(
                    ...
                    label=f"Inliers (n={len(inlier_contrast):,})",
                    density=True,
                    edgecolor="black",
                    linewidth=0.8,
                    hatch="\\\\\\",
        ...
        try:
            gridsize = min(50, max(20, int(np.sqrt(len(zs) / 100))))
            hb = ax.hexbin(
                zs[:, 0], zs[:, 2], gridsize=gridsize, cmap="Blues", alpha=0.4, mincnt=1, reduce_C_function=np.mean
            )
        except (ValueError, TypeError):
            pass
        ...
        try:
            gridsize = min(50, max(20, int(np.sqrt(len(zs) / 100))))
            hb = ax.hexbin(
                zs[:, 1], zs[:, 3], gridsize=gridsize, cmap="Blues", alpha=0.4, mincnt=1, reduce_C_function=np.mean
            )
        except (ValueError, TypeError):
            pass
        ...
        with open(stats_file, "w") as f:
            ...
            if contrast_values is not None:
                ...
                if outlier_image_indices is not None and len(outlier_image_indices) > 0:
                    ...
                    outlier_contrast = contrast_values[outlier_contrast_mask]
                    f.write(
                        f"Outlier contrast - Mean: {np.mean(outlier_contrast):.3f}, Std: {np.std(outlier_contrast):.3f}\n"
                    )
                ...
                if inlier_image_indices is not None and len(inlier_image_indices) > 0:
                    ...
                    inlier_contrast = contrast_values[inlier_contrast_mask]
                    f.write(
                        f"Inlier contrast - Mean: {np.mean(inlier_contrast):.3f}, Std: {np.std(inlier_contrast):.3f}\n"
                    )
    ...
    if len(combined_particle_outliers) > 0:
        create_method_plots(combined_particle_outliers, combined_particle_inliers, "Combined", f"combined_{zdim_key}")
