...
def outlier_detection_from_contrast(contrats, starfile, bad_contrast_threshold = 0.2):
    contrasts = contrasts < 0.2

    # Group contrasts by tomogram TILT
    tomogramtilts_to_tilts, tilts_to_tomogramtilts = tilt_dataset.parse_tomogramtilt_tilt(starfile)
    contrasts_grouped_by_tomogramtilts = None

    bad_groups = []
    for contrast_group in contrasts_grouped_by_tomogramtilts:
        bad_group = False
        if np.mean(contrast_group) < bad_contrast_threshold: # Probably bad?
            bad_group = True
            
        if np.mean(contrast_group < bad_contrast_threshold) > 0.5:
            bad_group = True


        if bad_group:
            bad_groups.append(bag_group)
        ## Make all these tilts bad tilts. Also, probably save which tomogram tilts have been declared bad?


    # If most tomogram tilts of a tomogram have been declared bad, it might make sense to declare the whole tomogram bad?
