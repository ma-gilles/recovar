"""Small student-facing RECOVAR synthetic PDB walkthrough.

The command intentionally does only the basics:

1. Generate a 1D PDB trajectory.
...
dataset, masks, pipeline output, and compute_state bandwidth/mask diagnostics.
"""
...
def _load_volume_stack(prefix: Path, n_states: int) -> np.ndarray:
    ...
    for idx in range(n_states):
        ...
        if not path.exists():
            raise FileNotFoundError(f"Missing expected trajectory volume {path}")
...
def _write_raw_pdb_volumes(args, out: Path, voxel_size: float) -> tuple[Path, np.ndarray]:
    """Generate the PDB trajectory into ``01_raw_volumes`` and return its prefix."""
    raw_dir = out / "01_raw_volumes"
    raw_prefix = raw_dir / "vol"
    if not Path(f"{raw_prefix}{args.n_states - 1:04d}.mrc").exists():
        raw_dir.mkdir(parents=True, exist_ok=True)
        logger.info("Generating PDB trajectory volumes in %s", raw_dir)
    ...
    return raw_prefix, _load_volume_stack(raw_prefix, args.n_states)


def _write_active_volumes(raw_volumes: np.ndarray, args, out: Path, voxel_size: float) -> tuple[Path, np.ndarray, dict]:
    """Write the volumes that will actually be used by the simulator."""
    active_dir = out / "02_active_volumes"
    active_prefix = active_dir / "vol"
    active_dir.mkdir(parents=True, exist_ok=True)

    if args.pc_project > 0:
        active_volumes, pca_meta = kb.project_volume_trajectory(raw_volumes, args.pc_project)
        logger.info("Using trajectory projected onto the first %d PC(s)", args.pc_project)
    else:
        active_volumes = np.asarray(raw_volumes, dtype=np.float32)
        pca_meta = kb.project_volume_trajectory(raw_volumes, 0)[1]
        logger.info("Using raw PDB trajectory volumes")
    ...
    _write_json(
        ...
        {
            "description": "These are the volumes used to simulate the particle images.",
...
def _middle_state_latent_point(pipeline_dir: Path, sim_info: dict, target_state: int, zdim: int) -> np.ndarray:
    """Use the RECOVAR embedding mean of particles assigned to the target state."""
...
def run_walkthrough(args, out: Path) -> dict:
    ...
    summary = {
        "output_dir": str(out),
        "raw_volumes": str(out / "01_raw_volumes"),
...
def _write_readme(out: Path, summary: dict) -> None:
    text = f"""# RECOVAR PDB Synthetic Walkthrough

This directory was generated by `recovar.commands.benchmark_kernel_bandwidth_1d`.
It is intentionally organized as a step-by-step experiment.

1. `01_raw_volumes/`: PDB trajectory volumes generated from 5nrl.
2. `02_active_volumes/`: volumes actually used by the simulator. If `pc_project > 0`, these are PCA-projected.
3. `03_dataset/`: simulated particle stack, STAR file, poses, CTF, and state assignments.
