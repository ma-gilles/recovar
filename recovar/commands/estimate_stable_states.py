...
def estimate_stable_states(density, latent_space_bounds, percent_top=1, n_local_maxs=3, file_path=None):
    ...
    latent_pts_z, latent_pts_grid = deconvolve_density.find_local_maxs_of_density(
        density, latent_space_bounds, percent_top=percent_top,
        n_local_maxs=n_local_maxs, plot_folder=file_path,
    )
    output.plot_over_density(density, points=latent_pts_grid, annotate=True,
                             plot_folder=density_dir, cmap='inferno')
