...
def compute_cluster_fsc_scores(
    pipeline_output,
    cluster_centers,
    cluster_indices,
    zdim_key,
    batch_size=100,
    n_particles_per_cluster=10,
    ...
    for cluster_idx in range(len(cluster_centers)):
        ...
        for i, zs_subset in enumerate(zs_subsets):
            ...
            logger.info(
                "Cluster %s: Using %s closest particles for half-map %s (min distance: %.3f, max distance: %.3f). Average distance over all particles: %.3f",
                cluster_idx,
                len(closest_local),
                i,
                distances[closest_local[0]],
                distances[closest_local[-1]],
                np.mean(distances),
            )
        ...
        if filter_resolution is not None:
            ...
            # Get frequency grid
            freq_grid = fourier_transform_utils.get_k_coordinate_of_each_pixel_3d(
                volume_shape, voxel_size=voxel_size, scaled=True
            )
            ...
            logger.info(
                "Cluster %s: Applied low-pass filter at %.1f Angstroms (%s Fourier shells)",
                cluster_idx,
                filter_resolution,
                filter_fourier_shells,
            )
        ...
        if save_reconstructions and output_folder is not None:
            ...
            individual_mrc_files.append(
                {"path": vol_combined_path, "type": "combined", "cluster": cluster_idx, "vol_index": vol_idx}
            )
            ...
            reconstructions[cluster_idx] = {
                ...
                "individual_mrc_files": {"combined": vol_combined_path},
            }
        ...
        fsc_auc_scores[cluster_idx] = {"halfmap_auc": fsc_auc, "vs_mean_auc": fsc_vs_mean_auc}
        ...
        logger.info(
            "Cluster %s: FSC=%.3f, AUC=%.3f, vs-mean_FSC=%.3f", cluster_idx, fsc_score, fsc_auc, fsc_vs_mean_score
        )
...
def plot_junk_detection_results(
    zs,
    cluster_centers,
    cluster_indices,
    fsc_scores,
    ...
    if save_all_plots:
        ...
        hb = ax.hexbin(freq_mesh.flatten(), curve_data.flatten(), gridsize=30, cmap="Blues", alpha=0.3, mincnt=1)
        ...
        sm = plt.cm.ScalarMappable(
            cmap=cm.viridis, norm=mcolors.Normalize(vmin=min(halfmap_fscs), vmax=max(halfmap_fscs))
        )
        ...
        plt.tight_layout()
        _safe_savefig(
            os.path.join(output_folder, "plots", f"all_halfmap_fsc_curves_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    if save_all_plots:
        ...
        plt.tight_layout()
        _safe_savefig(
            os.path.join(output_folder, "plots", f"all_vs_mean_fsc_curves_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    if save_all_plots:
        ...
        for i, cluster_idx in enumerate(top_10):
            ...
            ax.plot(
                freq_axis, all_fsc_curves[cluster_idx], "b-", linewidth=2, label=f"FSC={halfmap_fscs[cluster_idx]:.3f}"
            )
            ax.plot(
                freq_axis,
                all_vs_mean_curves[cluster_idx],
                "r-",
                linewidth=2,
                label=f"vs-Mean={vs_mean_fscs[cluster_idx]:.3f}",
            )
        ...
        plt.tight_layout()
        _safe_savefig(
            os.path.join(output_folder, "plots", f"top_10_clusters_fsc_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.tight_layout()
        _safe_savefig(
            os.path.join(output_folder, "plots", f"bottom_10_clusters_fsc_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    if save_all_plots:
        ...
        fig.suptitle(
            f"Junk Particle Detection Summary (zdim={zdim_key}, n_clusters={len(cluster_centers)})",
            fontsize=20,
            y=0.95,
            fontweight="bold",
        )
        ...
        _safe_savefig(
            os.path.join(output_folder, "plots", f"junk_detection_results_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    if save_all_plots and zs.shape[1] >= 2:
        ...
        ax.scatter(
            cluster_centers[top_5, 0],
            cluster_centers[top_5, 1],
            c=colors_top5,
            s=150,
            edgecolors="black",
            marker="x",
        ...
        ax.scatter(
            cluster_centers[bottom_5, 0],
            cluster_centers[bottom_5, 1],
            c=colors_bottom5,
            s=150,
            edgecolors="black",
            marker="x",
        ...
        bars = ax.bar(unique_counts, count_frequencies, color="skyblue", edgecolor="black", alpha=0.8, linewidth=1)
        ...
        plt.tight_layout(rect=[0, 0.03, 1, 0.93])
        _safe_savefig(
            os.path.join(output_folder, "plots", f"particle_usage_visualization_{zdim_key}.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    if save_all_plots:
        ...
        plt.tight_layout(rect=[0, 0.03, 1, 0.93])
        _safe_savefig(
            os.path.join(output_folder, "plots", f"fsc_analysis_{zdim_key}.png"), dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    logger.info(
        "  - Junk clusters: %s (%.1f%%)",
        len(halfmap_junk_clusters),
        len(halfmap_junk_clusters) / len(cluster_centers) * 100,
    )
    logger.info(
        "  - Junk particles: %s (%.1f%%)",
        len(halfmap_junk_particle_indices),
        len(halfmap_junk_particle_indices) / len(zs) * 100,
    )
    logger.info("")
    logger.info("vs-Mean FSC based detection:")
    logger.info(
        "  - Junk clusters: %s (%.1f%%)",
        len(vs_mean_junk_clusters),
        len(vs_mean_junk_clusters) / len(cluster_centers) * 100,
    )
    logger.info(
        "  - Junk particles: %s (%.1f%%)",
        len(vs_mean_junk_particle_indices),
        len(vs_mean_junk_particle_indices) / len(zs) * 100,
    )
    logger.info("")
    logger.info("Combined detection:")
    logger.info(
        "  - Junk clusters: %s (%.1f%%)",
        len(combined_junk_clusters),
        len(combined_junk_clusters) / len(cluster_centers) * 100,
    )
    logger.info(
        "  - Junk particles: %s (%.1f%%)",
        len(combined_junk_particle_indices),
        len(combined_junk_particle_indices) / len(zs) * 100,
    )
...
def plot_umap_visualization(
    zs, cluster_centers, cluster_indices, fsc_scores, fsc_auc_scores, output_folder, zdim_key, save_all_plots=False
):
    """
    ...
    for i, cluster_idx in enumerate(top_10_auc):
        ...
        ax.scatter(
            zs_umap[cluster_mask, 0],
            zs_umap[cluster_mask, 1],
            c=[colors_top10[i]],
            alpha=0.8,
            s=20,
    ...
    ax.scatter(
        cluster_centers_umap[top_10_auc, 0],
        cluster_centers_umap[top_10_auc, 1],
        c=colors_top10,
        s=150,
        marker="x",
        linewidth=3,
    ...
    ax.scatter(
        cluster_centers_umap[bottom_10_auc, 0],
        cluster_centers_umap[bottom_10_auc, 1],
        c=colors_bottom10,
        s=150,
        marker="x",
        linewidth=3,
    ...
    ax.hist(
        ...
        alpha=0.6,
        color="blue",
        label=f"Half-map FSC (μ={np.mean(halfmap_fscs):.3f})",
        edgecolor="black",
        linewidth=0.5,
    )
    ax.hist(
        ...
        alpha=0.6,
        color="red",
        label=f"vs-Mean FSC (μ={np.mean(vs_mean_fscs):.3f})",
        edgecolor="black",
        linewidth=0.5,
    )
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.93])
    _safe_savefig(
        os.path.join(output_folder, "plots", f"umap_visualization_{zdim_key}.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    ax.axvline(
        np.mean(halfmap_aucs), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(halfmap_aucs):.3f}"
    )
    ...
    ax.axvline(
        np.mean(vs_mean_aucs), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(vs_mean_aucs):.3f}"
    )
...
def detect_junk_clusters(
    ...
    for method_name in methods_results:
        if method_name == "consensus":
            ...
            methods_results[method_name] = apply_fraction_constraints(methods_results[method_name], combined_fsc)
        elif "fsc" in method_name:
            methods_results[method_name] = apply_fraction_constraints(methods_results[method_name], combined_fsc)
        else:  # auc methods
            methods_results[method_name] = apply_fraction_constraints(methods_results[method_name], combined_auc)
    ...
    logger.info(
        "Junk detection complete. Found %d junk clusters (%.1f%%) using method '%s'",
        len(junk_clusters),
        len(junk_clusters) / len(combined_fsc) * 100,
        method,
    )
...
def create_junk_detection_visualizations(
    halfmap_fscs,
    vs_mean_fscs,
    halfmap_aucs,
    vs_mean_aucs,
    ...
    fig, axes = plt.subplots(2, 3, figsize=(20, 12))
    fig.suptitle(f"Junk Detection Results (Method: {method}, zdim={zdim_key})", fontsize=18, y=0.95, fontweight="bold")
    ...
    if len(good_clusters) > 0:
        ax.hist(
            combined_fsc[good_clusters],
            bins=bins,
            alpha=0.8,
            color=colors["good"],
            label=f"Good clusters (n={len(good_clusters)})",
            edgecolor="black",
            linewidth=0.8,
            density=True,
            hatch="///",
    ...
    if len(junk_clusters) > 0:
        ax.hist(
            combined_fsc[junk_clusters],
            bins=bins,
            alpha=0.8,
            color=colors["junk"],
            label=f"Junk clusters (n={len(junk_clusters)})",
            edgecolor="black",
            linewidth=0.8,
            density=True,
            hatch="\\\\\\",
    ...
    ax.axvline(mean_fsc, color=colors["mean"], linestyle="--", linewidth=2.5, alpha=0.9, label=f"Mean: {mean_fsc:.3f}")
    ...
    ax.axvline(mean_auc, color=colors["mean"], linestyle="--", linewidth=2.5, alpha=0.9, label=f"Mean: {mean_auc:.3f}")
    ...
    try:
        ...
        if np.any(np.isnan(combined_fsc)) or np.any(np.isnan(combined_auc)):
            logger.warning(
                "NaN values found: FSC NaN count: %s, AUC NaN count: %s",
                np.sum(np.isnan(combined_fsc)),
                np.sum(np.isnan(combined_auc)),
            )
            hb = None
        elif np.any(np.isinf(combined_fsc)) or np.any(np.isinf(combined_auc)):
            logger.warning(
                "Infinite values found: FSC inf count: %s, AUC inf count: %s",
                np.sum(np.isinf(combined_fsc)),
                np.sum(np.isinf(combined_auc)),
            )
            hb = None
        elif combined_fsc.max() - combined_fsc.min() < 1e-10 or combined_auc.max() - combined_auc.min() < 1e-10:
            logger.warning(
                "No range in data: FSC range: %s, AUC range: %s",
                combined_fsc.max() - combined_fsc.min(),
                combined_auc.max() - combined_auc.min(),
            )
            hb = None
        else:
            hb = ax.hexbin(
                combined_fsc, combined_auc, gridsize=20, cmap="Blues", alpha=0.6, mincnt=1, reduce_C_function=np.mean
            )
    ...
    bars = ax.bar(
        range(len(sorted_scores)), sorted_scores, color=colors_bars, alpha=0.8, edgecolor="black", linewidth=0.5
    )
    ...
    legend_elements = [
        matplotlib.patches.Patch(facecolor=colors["good"], alpha=0.8, label="Good clusters"),
...
"""
    ...
    ax.text(
        ...
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.5", facecolor="lightgray", alpha=0.9, edgecolor="black", linewidth=1),
    )

    plt.tight_layout(rect=[0, 0.03, 1, 0.93])
    _safe_savefig(
        os.path.join(output_folder, "plots", f"junk_detection_summary_{zdim_key}.png"),
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    fig2.suptitle(f"Junk Detection Method Comparison (zdim={zdim_key})", fontsize=16, y=0.95, fontweight="bold")
    ...
    ax = axes2[0, 1]
    junk_fractions = [np.sum(methods_results[method]) / len(combined_fsc) * 100 for method in method_names]

    bars = ax.bar(method_names, junk_fractions, color="skyblue", edgecolor="black", alpha=0.8, linewidth=1)
    ...
    for i, method_name in enumerate(method_names):
        ...
        if np.sum(is_junk) > 0:
            ax.hist(
                combined_fsc[is_junk],
                bins=20,
                alpha=0.7,
                label=f"{method_name} (junk)",
                density=True,
    ...
    if performance_data:
        ...
        bars = ax.bar(methods, separations, color="lightgreen", edgecolor="black", alpha=0.8, linewidth=1)
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.93])
    _safe_savefig(
        os.path.join(output_folder, "plots", f"junk_detection_methods_{zdim_key}.png"),
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
def junk_particle_detection(
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[RobustStreamHandler(), RobustFileHandler(os.path.join(output_folder, "junk_detection.log"))],
    )
    ...
    if filter_resolution is not None:
        logger.info(
            "Combined reconstructions will be filtered to %.1f Angstroms with %s Fourier shells",
            filter_resolution,
            filter_fourier_shells,
        )
    ...
    if save_pipeline_indices:
        ...
        with open(summary_file, "w") as f:
            ...
            if output_format in ["both", "junk_only"]:
                f.write(
                    f"  python -m recovar.commands.pipeline particles.star --poses poses.pkl --ctf ctf.pkl --ind {os.path.basename(junk_pipeline_file)} -o output_dir\n\n"
                )
            f.write("  # Run pipeline with only good particles:\n")
            if output_format in ["both", "good_only"]:
                f.write(
                    f"  python -m recovar.commands.pipeline particles.star --poses poses.pkl --ctf ctf.pkl --ind {os.path.basename(good_pipeline_file)} -o output_dir\n"
                )
    ...
    logger.info(
        "Particle mapping completed. Found %s junk particles and %s good particles.",
        len(junk_particles),
        len(good_particles),
    )
    ...
    try:
        ...
        summary_dict = {
            "fsc_aucs": np.array([info.get("halfmap_fsc_auc", 0) for info in junk_info.values()])
            if junk_info
            else np.array([]),
...
def add_args(parser):
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        help="Output directory for results (default: recovar_result_dir/junk_detection_zdim)",
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, help="Batch size for reconstruction (auto: based on GPU memory and grid size)"
    )
    parser.add_argument(
        "--n-particles-per-cluster",
        type=int,
        help="Number of particles per halfmap (auto: min(100, max(10, n_particles/n_clusters)))",
    )
    ...
    parser.add_argument(
        "--filter-resolution", type=float, help="Resolution in Angstroms to filter combined reconstructions"
    )
    parser.add_argument(
        "--filter-fourier-shells",
        type=int,
        default=10,
        help="Number of Fourier shells to keep when filtering (default: 10)",
    )
    parser.add_argument(
        "--junk-detection-method",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=25.0,
        help="Percentile threshold for percentile method (default: 25.0)",
    )
    parser.add_argument(
        "--std-threshold",
        type=float,
        default=2.0,
        help="Standard deviation threshold for std_based method (default: 2.0)",
    )
    parser.add_argument(
        "--min-junk-fraction",
        type=float,
        default=0.1,
        help="Minimum fraction of clusters that can be classified as junk (default: 0.1)",
    )
    parser.add_argument(
        "--max-junk-fraction",
        type=float,
        default=0.8,
        help="Maximum fraction of clusters that can be classified as junk (default: 0.8)",
    )
    parser.add_argument(
    ...
    with job_context(args, "junk_particle_detection") as ctx:
        ...
        if args.batch_size is None or args.n_particles_per_cluster is None:
            ...
            n_particles_per_cluster = (
                ...
                if args.n_particles_per_cluster is not None
                else auto_n_particles_per_cluster
            )

            logger.info(
                "Auto-calculated: batch_size=%s, n_particles_per_cluster=%s", batch_size, n_particles_per_cluster
            )
        else:
...
def create_particle_classification_visualizations(
    zs,
    cluster_indices,
    junk_particles,
    good_particles,
    ...
    if zs.shape[1] >= 2:
        ...
        if len(good_particles) > 0:
            ax.scatter(
                ...
                alpha=0.7,
                s=10,
                label=f"Good particles (n={len(good_particles)})",
            )
        if len(junk_particles) > 0:
            ax.scatter(
                ...
                alpha=0.7,
                s=10,
                label=f"Junk particles (n={len(junk_particles)})",
            )
        ...
        ax.text(0.5, 0.5, "Insufficient dimensions for 2D plot", transform=ax.transAxes, ha="center", va="center")
        ax.set_title("Particle Classification in Latent Space")
    ...
    if len(junk_cluster_sizes) > 0:
        ax.hist(
            ...
            alpha=0.7,
            color="red",
            label=f"Junk clusters (n={len(junk_cluster_sizes)})",
            density=True,
        )
    if len(good_cluster_sizes) > 0:
        ax.hist(
            ...
            alpha=0.7,
            color="green",
            label=f"Good clusters (n={len(good_cluster_sizes)})",
            density=True,
        )
    ...
    try:
        ...
        if len(good_particles) > 0:
            ax.scatter(
                zs_umap[good_particles, 0],
                zs_umap[good_particles, 1],
                c="green",
                alpha=0.7,
                s=10,
                label=f"Good particles (n={len(good_particles)})",
        ...
        if len(junk_particles) > 0:
            ax.scatter(
                zs_umap[junk_particles, 0],
                zs_umap[junk_particles, 1],
                c="red",
                alpha=0.7,
                s=10,
                label=f"Junk particles (n={len(junk_particles)})",
        ...
        ax.text(0.5, 0.5, "UMAP not available", transform=ax.transAxes, ha="center", va="center")
        ax.set_title("Particle Classification in UMAP Space")
    ...
    if zs.shape[1] >= 1:
        ...
        ax.text(0.5, 0.5, "No latent dimensions available", transform=ax.transAxes, ha="center", va="center")
...
"""
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.93])
    _safe_savefig(
        os.path.join(output_folder, "plots", f"particle_classification_{zdim_key}.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.93])
    _safe_savefig(
        os.path.join(output_folder, "plots", f"particle_analysis_detailed_{zdim_key}.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def save_particle_classifications(
    junk_particles,
    good_particles,
    particle_stats,
    cluster_indices,
