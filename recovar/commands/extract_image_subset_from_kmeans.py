...
def extract_image_subset_from_kmeans(path_to_centers, kmeans_indices, inverse, output_path):
    ...
    logger.info(
        "Saved %d out of %d indices of subset of images to %s. Total dataset has %d images (or very close to that).",
        indices.size,
        n_valid,
        path_to_centers,
        labels.size,
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the output .pkl file containing the indices of subset of images",
    )
