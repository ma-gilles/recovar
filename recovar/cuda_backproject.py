...
def _lib_missing_required_symbols(lib_path: pathlib.Path) -> str | None:
    ...
    ``PerImageBackproject``, and today's ``cuda_backproject.py`` registers FFI
...
def _lib_is_stale(lib_path: pathlib.Path) -> bool:
    ...
    Catches:
      - mtime skew: user installed before a kernel/Makefile fix landed (e.g.
        issue #131's Blackwell widening): without this check,
      ...
      - binary skew: a cached ``.so`` from an older branch doesn't export a
        symbol the current source requires (e.g. ``PerImageBackproject``).
        Without this check ``_ensure_ffi`` would crash inside ``ctypes`` and
        ``cuda_available()`` would silently fall back to the slow JAX path.
    """
    try:
...
_FFI_REGISTRATIONS: tuple[tuple[str, str], ...] = (
    ...
    (_TARGET_PER_IMAGE_BACKPROJECT, "PerImageBackproject"),
