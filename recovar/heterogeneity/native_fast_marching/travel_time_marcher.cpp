...
double travelTimeMarcher::updatePointOrderTwo(int i, std::set<int>avoid_dim)
{
  ...
  try {
    ...
    if(static_cast<int>(avoid_dim.size()) == dim_) return std::numeric_limits<double>::infinity(); //end of the recursion, use inf so that it is discarded selecting the minimum
