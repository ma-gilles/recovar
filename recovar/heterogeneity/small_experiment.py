...
def subsample_coo_columns(sparse_mat, right_indices):
    ...
    return sparse.BCOO(
        (sparse_mat.data[good_indices], new_indices_all), shape=(sparse_mat.shape[0], right_indices.size)
    )
...
def generate_cryo_like_experiment(
    grid_size, m, b, snr, eig_decay=0.75, random_sampling=False, voxel_size=4, Bfactor=60, return_mat=False
):
        ...
        if random_sampling == "cryoemlike":
            ...
            for _ in range(m_batch):
                ...
                sampled_indices = jax.random.choice(
                    subkey, jnp.arange(volume_size), shape=(samples_per_image,), replace=False
                )
...
def high_d_PCA_by_nystrom_covar(
    y,
    indices,
    noise_variance,
    covar_indices,
    ...
    C = covar_estimate_again(
        y, indices, noise_variance, n, covar_indices, covar_regularization=covar_regularization, chunk_size=chunk_size
    )
...
def high_d_PCA_by_projected_covar(
    y, indices, noise_variance, covar_indices, n, covar_regularization=None, chunk_size=1024
):
...
def high_d_PCA_by_low_rank_completion(y, indices, n, mu=0.8, U=None):
    ...
    Xpg = pyproximal.optimization.primal.ProximalGradient(
        f, g, np.zeros(mat_size), acceleration="vandenberghe", tau=1.0, niter=100, show=True
    )
