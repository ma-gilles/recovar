...
def get_coords_in_basis_and_contrast_3(
    ...
    for (
        ...
        if shared_label:
            ...
            # malformed. batch_image_ind is the validated int32 host copy;
            # particle_ids went through an extra JAX-to-NumPy path that can
            # be unreliable under GPU memory pressure (see #71).
...
@eqx.filter_jit
def _compute_batch_coords_explicit(
    config: ForwardModelConfig,
    batch,
    rotation_matrices,
    translations,
    ...
    if opts.shared_label and not opts.contrast_shared_across_tilt_series:
        ...
        def refine_contrast(i, contrast_est):
            ...
            _xs_repeat = jnp.repeat(_xs, axis=0, repeats=batch.shape[0])
...
@eqx.filter_jit
def _compute_grouped_shared_batch_coords_explicit(
    config: ForwardModelConfig,
    batch,
    rotation_matrices,
    translations,
