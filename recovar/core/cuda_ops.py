...
Each function here wraps a CUDA kernel pair (project / backproject) so that
JAX's autodiff can differentiate through them.  The three variants differ only
in whether the volume and/or image are stored in rfft-packed (half-spectrum)
layout:

  - :func:`cuda_slice_full`         : full volume  → full image
  - :func:`cuda_slice_to_half_image`: full volume  → half image (rfft)
  - :func:`cuda_slice_from_half_vol`: half volume (rfft) → full image

Import these into ``slicing.py``; do not import ``cuda_backproject`` directly
from user-facing modules.
"""
