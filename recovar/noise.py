...
#     kernel_sq_sum =0 
#     for batch, particles_ind, batch_ind in data_generator:
#         image_PS = regularization.batch_average_over_shells(jnp.abs(batch)**2, experiment_dataset.image_shape, 0)
...
#     masked_image_PS, _, image_PS = estimate_noise_variance_from_outside_mask_v2(experiment_dataset, volume_mask, batch_size, disc_type = 'linear_interp')
#     return mean_fn(masked_image_PS, axis =0), np.std(masked_image_PS, axis =0), mean_fn(image_PS, axis =0), np.std(image_PS, axis =0)


# def estimate_image_PS(experiment_dataset, batch_size):
#     data_generator = experiment_dataset.get_dataset_generator(batch_size=batch_size)
#     sum_PS = 0
#     count = 0
#     for batch, _, _ in data_generator:
#         processed = experiment_dataset.image_stack.process_images(batch)
#         # Compute the power spectrum for each processed image
#         image_PS = jnp.abs(processed) ** 2
#         # Collapse the 2D power spectra to a radial profile
#         averaged_PS = regularization.average_over_shells(image_PS, experiment_dataset.image_shape)
#         sum_PS += averaged_PS
...
def get_masked_noise_variance_from_noise_variance(image_masks, image_cov_noise, image_shape):
    ...
    image_cov_noise_ft = jnp.fft.fft2(image_cov_noise.reshape(-1, *image_shape))
    masked_noise_variance = jnp.fft.ifft2( f_mask * image_cov_noise_ft )
