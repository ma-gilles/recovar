...
from recovar import output, simulator, utils, dataset, synthetic_dataset
from recovar import regularization, noise, homogeneous, covariance_estimation, principal_components, metrics
...
def generate_dataset(dataset_info, grid_size, n_images, noise_level, output_folder, seed=42, batch_size=1000):
    ...
    image_stack, sim_info = simulator.generate_synthetic_dataset(
        output_folder, voxel_size, fixed_vol_dir + "/", n_images,
        outlier_file_input=None, grid_size=grid_size,
        volume_distribution=None, dataset_params_option="uniform",
        noise_level=noise_level, noise_model="white",
        put_extra_particles=False, percent_outliers=0.00,
        volume_radius=0.7, trailing_zero_format_in_vol_name=False,
        noise_scale_std=0, contrast_std=0, disc_type='nufft', n_tilts=-1
    )
    ...
    noise_var_image = utils.make_radial_image(noise_variance, cryos[0].image_shape)
...
def run_single_scale(dataset_name, grid_size, n_pcs, strategy, scale, n_iter,
                     results_dir, results_output=None):
    ...
    U_gt = jnp.array(U_gt)
    s_gt = jnp.array(s_gt)
    # L1 uses PCA W as init (no L2 warmstart when running in parallel)
    if strategy == 'l2_no_whiten':
        results = run_l2_sweep(
            cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
            volume_shape, [scale], n_iter, use_whitening=False
        )
    elif strategy == 'l2_cz':
        results = run_l2_sweep(
            cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
            volume_shape, [scale], n_iter, use_whitening=True, whitening_mode='cz'
        )
    elif strategy == 'l2_proj_ls':
        results = run_l2_sweep(
            cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
            volume_shape, [scale], n_iter, use_whitening=True, whitening_mode='proj_ls'
        )
    elif strategy == 'l1_no_whiten':
        results = run_l1_sweep(
            cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
            volume_shape, [scale], n_iter, use_whitening=False
        )
    elif strategy == 'l1_cz':
...
def load_single_run_results(results_dir, dataset_name, results_output=None):
    ...
    groups = {
        'l1_cz': [], 'l1_proj_ls': [], 'l1_no_whiten': [],
        'l2_cz': [], 'l2_proj_ls': [], 'l2_no_whiten': [],
    }
    for path in files:
    ...
    return (
        groups['l1_cz'], groups['l1_proj_ls'], groups['l1_no_whiten'],
        groups['l2_cz'], groups['l2_proj_ls'], groups['l2_no_whiten'],
    )


def combine_and_plot_single_runs(results_dir, dataset_name, plots_dir, n_pcs,
                                 results_output=None, run_id=None):
    ...
    l1_cz, l1_proj_ls, l1_no_whiten, l2_cz, l2_proj_ls, l2_no_whiten = \
        load_single_run_results(results_dir, dataset_name, results_output)
    ...
    dataset_dir = os.path.dirname(results_output) if os.path.basename(results_output).startswith('run_') else results_output
    ...
    if not os.path.isfile(npz_path):
        ...
        data = np.load(npz_path, allow_pickle=True)
        pca_results = data['pca_results'].flat[0] if 'pca_results' in data and data['pca_results'].size > 0 else {'rel_var': 0.0}
    ...
    plot_whitening_comparison(
        l1_cz, l1_proj_ls, l1_no_whiten, l2_cz, l2_proj_ls, l2_no_whiten,
        dataset_name, pca_results, plots_dir, n_pcs
    )
...
def warmstart_from_pca(cryos, means, gt_results, basis_size, batch_size=100, gpu_memory=40):
    ...
    w_gt_averaged = regularization.batch_average_over_shells(
        jnp.abs(W_gt.T)**2, volume_shape, 0
    )
    ...
    u, s, _, _, _ = principal_components.estimate_principal_components(
        cryos, options, means, means['prior'], volume_mask, dilated_volume_mask,
        valid_idx, batch_size, gpu_memory_to_use=gpu_memory/4,
        covariance_options=covariance_options, variance_estimate=None
    )
...
def run_l1_sweep(cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt, volume_shape,
                 scales, n_iter=20, use_whitening=True, whitening_mode="cz"):
    ...
    for scale in scales:
        ...
        reload(ppca)
        out = ppca.EM(
            cryos, mean_estimate, W_init.copy(), W_prior_base,
            U_gt=U_gt, S_gt=s_gt**2, EM_iter=n_iter,
            use_whitening=use_whitening, whitening_mode=whitening_mode, sparse_PCA=True,
            l1_sigma=sigma,
            disc_type_mean='cubic', disc_type='linear_interp',
            return_iteration_data=True
        )
        u, s, W, ez, sm, iteration_data = out
        ...
        result = {
            'scale': scale, 'rel_var': rv, 'rel_var_per_pc': list(rel_var),
            'W_norm': W_norm, 'z_var': z_var, 'sparsity': avg_sparsity, 'W': np.array(W),
            'trace_Cz': trace_Cz, 'constraint_violation': constraint_violation,
            'C_z': C_z, 'iteration_data': iteration_data,
        }
        results.append(result)
        print(f"    RelVar: {rv:.4f}, Sparsity: {avg_sparsity:.2%}, W_norm: {W_norm:.2f}")
    return results


def run_l2_sweep(cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt, volume_shape,
                 scales, n_iter=20, use_whitening=True, whitening_mode="cz"):
    ...
    for scale in scales:
        ...
        out = ppca.EM(
            ...
            U_gt=U_gt, S_gt=s_gt**2, EM_iter=n_iter,
            use_whitening=use_whitening, whitening_mode=whitening_mode, sparse_PCA=False,
            disc_type_mean='cubic', disc_type='linear_interp',
            return_iteration_data=True
        )
        u, s, W, ez, sm, iteration_data = out
...
def plot_sweep_results(l1_whiten, l1_no_whiten, l2_whiten, l2_no_whiten, dataset_name, pca_results, plots_dir, n_pcs):
    ...
    fig.suptitle(f'{dataset_name}: L1 vs L2 PPCA Scale Sweep (With/Without Whitening)\n({n_pcs} PCs, warmstarted from Cov-PCA)', fontsize=14)
    ...
    ax = axes[1, 0]
    ax.semilogx(l1w_scales, [s*100 for s in [r['sparsity'] for r in l1_whiten]], 'bo-', linewidth=2, markersize=6, label='L1 + whiten')
    ax.semilogx(l1w_scales, [s*100 for s in [r['sparsity'] for r in l1_no_whiten]], 'b^--', linewidth=1.5, label='L1 no whiten')
    ax.semilogx(l2w_scales, [s*100 for s in [r['sparsity'] for r in l2_whiten]], 'gs-', linewidth=2, markersize=6, label='L2 + whiten')
    ax.semilogx(l2w_scales, [s*100 for s in [r['sparsity'] for r in l2_no_whiten]], 'g^--', linewidth=1.5, label='L2 no whiten')
    ax.set_xlabel('Scale Parameter', fontsize=12)
    ...
    ax.bar(pc_indices, l1_best['rel_var_per_pc'], width, label=f'L1+whiten (s={l1w_scales[best_idx]:.4f})', color='steelblue', edgecolor='black')
    ax.bar(pc_indices + width, l2_best['rel_var_per_pc'], width, label=f'L2+whiten (s={l2w_scales[best_idx_l2]:.4f})', color='lightgreen', edgecolor='black')
...
def plot_whitening_comparison(l1_cz, l1_proj_ls, l1_no_whiten, l2_cz, l2_proj_ls, l2_no_whiten,
                              dataset_name, pca_results, plots_dir, n_pcs):
    ...
    ax.semilogx(scales_l1, [r['rel_var'] for r in l1_no_whiten], 'k^--', linewidth=1.5, markersize=5, label='L1 no whiten')
    ...
    ax.semilogx(scales_l2, [r['rel_var'] for r in l2_no_whiten], 'k^--', linewidth=1.5, markersize=5, label='L2 no whiten')
    ax.axhline(y=pca_baseline, color='r', linestyle=':', linewidth=1.5, label=f'Cov-PCA ({pca_baseline:.3f})')
    ...
    ax.semilogx(scales_l1, [r['sparsity']*100 for r in l1_proj_ls], 'g-s', linewidth=2, markersize=5, label='L1 + proj_ls')
...
def plot_em_convergence_all_scales(results_list, strategy_label, dataset_name, plots_dir):
    ...
    for (scale, idat), color in zip(entries, colors):
        ...
        if any(v is not None for v in relvars):
            axes[0, 1].plot(iters, [v if v is not None else 0 for v in relvars], color=color, label=f'{scale}', markersize=2)
        ...
        axes[1, 1].plot(iters, [d.get('trace_E_mean_outer', 0) for d in idat], color=color, label=f'{scale}', markersize=2)
...
def plot_em_convergence_all_methods(l1_cz, l1_proj_ls, l1_no_whiten, l2_cz, l2_proj_ls, l2_no_whiten,
                                    dataset_name, plots_dir):
...
def plot_em_convergence(iteration_data, title, plots_dir, prefix):
    ...
    ax.plot(iters, [d.get('trace_E_mean_outer', 0) for d in iteration_data], 'orange', marker='o', markersize=4, label='tr(E[μμ^T])')
    ax.plot(iters, [d.get('norm_E_mean_outer_minus_I', 0) for d in iteration_data], 'brown', marker='s', markersize=4, label='||E[μμ^T]-I||')
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Value')
...
def plot_scale_diagnostics(l1_cz, l1_proj_ls, l1_no_whiten, l2_cz, l2_proj_ls, l2_no_whiten,
                           dataset_name, plots_dir):
    ...
    ax.semilogx(scales_l1, [r.get('trace_Cz', 0) for r in l1_proj_ls], 'g-s', linewidth=2, markersize=5, label='proj_ls')
    ...
    ax = axes[0, 2]
    ax.semilogx(scales_l1, [r.get('constraint_violation', 0) for r in l1_cz], 'b-o', linewidth=2, markersize=5, label='Cz')
    ax.semilogx(scales_l1, [r.get('constraint_violation', 0) for r in l1_proj_ls], 'g-s', linewidth=2, markersize=5, label='proj_ls')
    ax.semilogx(scales_l1, [r.get('constraint_violation', 0) for r in l1_no_whiten], 'k^--', linewidth=1.5, label='no whiten')
    ax.set_xlabel('L1 Scale')
    ax.set_ylabel('||Ĉ_z - I||')
    ...
    ax.semilogx(scales_l2, [r.get('trace_Cz', 0) for r in l2_proj_ls], 'g-s', linewidth=2, markersize=5, label='proj_ls')
    ...
    ax = axes[1, 2]
    ax.semilogx(scales_l2, [r.get('constraint_violation', 0) for r in l2_cz], 'b-o', linewidth=2, markersize=5, label='Cz')
    ax.semilogx(scales_l2, [r.get('constraint_violation', 0) for r in l2_proj_ls], 'g-s', linewidth=2, markersize=5, label='proj_ls')
    ax.semilogx(scales_l2, [r.get('constraint_violation', 0) for r in l2_no_whiten], 'k^--', linewidth=1.5, label='no whiten')
    ax.set_xlabel('L2 Scale')
    ax.set_ylabel('||Ĉ_z - I||')
...
def plot_scale_diagnostics_simple(l1_whiten, l1_no_whiten, l2_whiten, l2_no_whiten,
                                  dataset_name, plots_dir):
    ...
    ax = axes[0, 2]
    ax.semilogx(scales_l1, [r.get('constraint_violation', 0) for r in l1_whiten], 'b-o', linewidth=2, markersize=5, label='whiten')
    ax.semilogx(scales_l1, [r.get('constraint_violation', 0) for r in l1_no_whiten], 'k^--', linewidth=1.5, label='no whiten')
    ax.set_xlabel('L1 Scale')
    ax.set_ylabel('||Ĉ_z - I||')
    ax.set_title('L1: Constraint Violation vs Scale')
    ...
    ax = axes[1, 2]
    ax.semilogx(scales_l2, [r.get('constraint_violation', 0) for r in l2_whiten], 'g-s', linewidth=2, markersize=5, label='whiten')
    ax.semilogx(scales_l2, [r.get('constraint_violation', 0) for r in l2_no_whiten], 'k^--', linewidth=1.5, label='no whiten')
    ax.set_xlabel('L2 Scale')
    ax.set_ylabel('||Ĉ_z - I||')
    ax.set_title('L2: Constraint Violation vs Scale')
...
def run_benchmark(dataset_info, grid_size, n_images, noise_level, n_pcs,
                  l1_scales, l2_scales, n_iter, seed, results_dir, plots_dir, whitening_mode="cz"):
    ...
    l2_whiten = run_l2_sweep(
        cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l2_scales, n_iter, use_whitening=True, whitening_mode=whitening_mode
    )
    print("\n[4/7] Running L2 PPCA scale sweep (NO whitening)...")
    l2_no_whiten = run_l2_sweep(
        cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l2_scales, n_iter, use_whitening=False
    )
    ...
    l1_whiten = run_l1_sweep(
        cryos, mean_estimate, W_l2_best_whiten, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l1_scales, n_iter, use_whitening=True, whitening_mode=whitening_mode
    )
    print("\n[6/7] Running L1 PPCA scale sweep (NO whitening, warmstart from L2)...")
    l1_no_whiten = run_l1_sweep(
        cryos, mean_estimate, W_l2_best_no_whiten, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l1_scales, n_iter, use_whitening=False
    )

    print("\n[7/7] Creating plots...")
    plot_sweep_results(
        l1_whiten, l1_no_whiten, l2_whiten, l2_no_whiten,
        dataset_name, pca_results, plots_dir, n_pcs
    )
    ...
    if idat:
        path = plot_em_convergence(
            idat, f'{dataset_name}: EM Convergence (L1+whiten, scale={best_l1["scale"]:.4f})',
            plots_dir, f'{dataset_name}_l1_whiten'
        )
        if path:
    ...
    summary = {
        'dataset': dataset_name, 'n_volumes': dataset_info['n_volumes'],
        'grid_size': grid_size, 'n_images': n_images, 'noise_level': noise_level,
        'n_pcs': n_pcs, 'seed': seed, 'whitening_mode': whitening_mode,
        'pca_results': {k: v for k, v in pca_results.items() if k != 'W_init'},
        'l1_whiten': strip_W(l1_whiten), 'l1_no_whiten': strip_W(l1_no_whiten),
        'l2_whiten': strip_W(l2_whiten), 'l2_no_whiten': strip_W(l2_no_whiten),
        'best_l1_scale': l1_best_whiten['scale'], 'best_l1_relvar': l1_best_whiten['rel_var'],
        'best_l2_scale': l2_best_whiten['scale'], 'best_l2_relvar': l2_best_whiten['rel_var'],
        'W_best_l1': l1_best_whiten['W'], 'W_best_l2': l2_best_whiten['W'],
        'W_pca': np.array(W_init), 'volume_shape': volume_shape,
    }
    ...
    return {
        'l1_whiten': l1_whiten, 'l1_no_whiten': l1_no_whiten,
        'l2_whiten': l2_whiten, 'l2_no_whiten': l2_no_whiten,
        'pca_results': pca_results, 'summary': summary,
    }


def run_benchmark_compare_whitening(dataset_info, grid_size, n_images, noise_level, n_pcs,
                                    l1_scales, l2_scales, n_iter, seed, results_dir, plots_dir):
    ...
    print(f"  Strategies: Cz, proj_ls, no whitening")
    print(f"{'='*80}")
    ...
    l2_cz = run_l2_sweep(
        cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l2_scales, n_iter, use_whitening=True, whitening_mode="cz"
    )
    print("\n[5/8] Running L2 PPCA scale sweep (proj_ls whitening)...")
    l2_proj_ls = run_l2_sweep(
        cryos, mean_estimate, W_init, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l2_scales, n_iter, use_whitening=True, whitening_mode="proj_ls"
    )
    ...
    l1_cz = run_l1_sweep(
        cryos, mean_estimate, W_l2_best_cz, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l1_scales, n_iter, use_whitening=True, whitening_mode="cz"
    )
    print("\n[8/8] Running L1 PPCA scale sweep (proj_ls whitening)...")
    l1_proj_ls = run_l1_sweep(
        cryos, mean_estimate, W_l2_best_proj_ls, W_prior_base, U_gt, s_gt,
        gt_results.volume_shape, l1_scales, n_iter, use_whitening=True, whitening_mode="proj_ls"
    )
    ...
    summary = {
        'dataset': dataset_name, 'n_volumes': dataset_info['n_volumes'],
        'grid_size': grid_size, 'n_images': n_images, 'noise_level': noise_level,
        'n_pcs': n_pcs, 'seed': seed, 'compare_whitening': True,
        'pca_results': {k: v for k, v in pca_results.items() if k != 'W_init'},
        'l1_cz': strip_W(l1_cz), 'l1_proj_ls': strip_W(l1_proj_ls), 'l1_no_whiten': strip_W(l1_no_whiten),
        'l2_cz': strip_W(l2_cz), 'l2_proj_ls': strip_W(l2_proj_ls), 'l2_no_whiten': strip_W(l2_no_whiten),
        'best_l1_cz_relvar': best_l1_cz['rel_var'], 'best_l1_cz_scale': best_l1_cz['scale'],
        'best_l1_ls_relvar': best_l1_ls['rel_var'], 'best_l1_ls_scale': best_l1_ls['scale'],
        'best_l2_cz_relvar': best_l2_cz['rel_var'], 'best_l2_cz_scale': best_l2_cz['scale'],
        'best_l2_ls_relvar': best_l2_ls['rel_var'], 'best_l2_ls_scale': best_l2_ls['scale'],
    ...
    return {
        'l1_cz': l1_cz, 'l1_proj_ls': l1_proj_ls, 'l1_no_whiten': l1_no_whiten,
        'l2_cz': l2_cz, 'l2_proj_ls': l2_proj_ls, 'l2_no_whiten': l2_no_whiten,
        'pca_results': pca_results, 'summary': summary,
    }
...
def main():
    ...
    parser.add_argument('--base-dir', type=str, default='/home/mg6942/mytigress/cryobench2',
    ...
    parser.add_argument('--plots-dir', type=str, default='ppca_plots',
    ...
    parser.add_argument('--whitening-mode', type=str, default='cz', choices=['cz', 'proj_ls'],
                        help='Whitening mode when use_whitening=True (default: cz); ignored if --compare-whitening')
    parser.add_argument('--compare-whitening', action='store_true',
                        help='Run both Cz and proj_ls whitening and compare (L1/L2 sweeps for each + no whiten)')
    parser.add_argument('--l1-scales', type=str, default='0.01,0.03,0.1,0.3,1.0,3.0,10.0',
                        help='Comma-separated L1 scale values')
    parser.add_argument('--l2-scales', type=str, default='0.01,0.1,1.0,10.0',
                        help='Comma-separated L2 scale values')
    ...
    parser.add_argument('--combine-and-plot', action='store_true',
    ...
    if args.combine_and_plot:
        ...
        combine_and_plot_single_runs(
            results_root, args.dataset, plots_root, args.n_pcs,
            results_output=None, run_id=run_id
        )
        return
    ...
    if args.setup_only:
        ...
        if args.results_dir is not None:
            ...
            results_root = os.path.join(args.base_dir, "ppca_sweep_results_10pc")
        run_setup_only(
            datasets[0], args.grid_size, args.n_images, args.noise_level,
            args.n_pcs, args.seed, results_root
        )
        print(f"Setup done. Submit single-run jobs with --single-run --strategy STRAT --scale SCALE")
        return
    ...
    if args.single_run:
        ...
        run_single_scale(
            args.dataset, args.grid_size, args.n_pcs, args.strategy, args.scale,
            args.em_iters, results_root, results_output=results_output
        )
        return
        ...
        try:
            if args.compare_whitening:
                result = run_benchmark_compare_whitening(
                    dataset_info, args.grid_size, args.n_images, args.noise_level,
                    args.n_pcs, l1_scales, l2_scales, args.em_iters, args.seed,
                ...
                result = run_benchmark(
                    dataset_info, args.grid_size, args.n_images, args.noise_level,
                    args.n_pcs, l1_scales, l2_scales, args.em_iters, args.seed,
                    results_root, plots_root, whitening_mode=args.whitening_mode
                )
