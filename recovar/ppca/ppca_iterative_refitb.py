"""Iterative PPCA + RefitB — harness wrapper.

The actual algorithm now lives inside :func:`recovar.ppca.ppca.EM` and is
selected with the ``refitb_every`` / ``refitb_start`` / ``refitb_inner_iters``
/ ``refitb_kappa`` parameters. This module only contains the harness glue
(:func:`run_iterative_ppca_refitb`) that wires a ``PipelineOutput`` to
``ppca.EM`` and writes the result back to a PipelineOutput-compatible
directory. Mirror of :mod:`recovar.ppca.ppca_iterative_projcov`.
"""
...
def run_iterative_ppca_refitb(
    ...
    """Run iterative PPCA + RefitB from a ``PipelineOutput`` and save results.

    Loads the dataset and initial W from the PPCA result, calls
    :func:`recovar.ppca.ppca.EM` with the requested ``refitb_every`` /
    ``refitb_kappa`` settings, then re-derives per-image embeddings in the
    final orthonormal basis (matching the projcov harness convention).
    """
    ...
    logger.info(
        "Running ppca.EM(refitb_every=%d, refitb_start=%d, inner=%d, kappa=%.2f) zdim=%d n_iters=%d",
        refitb_every, refitb_start, refitb_inner_iters, kappa, zdim, n_iters,
    )

    # ── Run EM with RefitB refinement ────────────────────────────────────
