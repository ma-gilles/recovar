...
class WaveletL1(L1):
    ...
    def prox(self, x, tau):
        ...
        if debug:   
            ...
            print(f"WaveletL1.prox: {total_time:.4f}s (forward: {wavelet_forward_time:.4f}s, prox: {prox_time:.4f}s, backward: {wavelet_backward_time:.4f}s)")
...
def generate_data_from_ground_truth(gt, noise_level=0.1):
    ...
    return (np.array(lhs), np.array(rhs), np.array(A_matrices), 
            np.array(b_clean), np.array(b_noisy))
...
def analyze_wavelet_coefficient_variance(x, volume_shape, wavelet_type='db1', level_names=None):
    ...
    print(f"Analyzing wavelet coefficient variance...")
...
def test_admm_wavelet_function():
    ...
    for i, result in enumerate(results):
        if result["success"]:
            print(f"{result['name']:<25} {'✅':<8} {result['reconstruction_error']:<12.6f} {result['final_sparsity']:<8.3f}")
        else:
...
def main():
    ...
    reconstruction_error = np.linalg.norm(test_volume.flatten() - reconstructed[0]) / np.linalg.norm(test_volume.flatten())
    ...
    print(f"Inferred Solution Losses:")
    ...
    print(f"{'Total Loss':<20} {gt_total_loss:<15.6f} {init_total_loss:<15.6f} {rec_total_loss:<15.6f} {total_improvement:<14.1f}%")
    print(f"{'Data Fit Loss':<20} {gt_data_loss:<15.6f} {init_data_loss:<15.6f} {rec_data_loss:<15.6f} {data_improvement:<14.1f}%")
    print(f"{'Regularization Loss':<20} {gt_reg_loss:<15.6f} {init_reg_loss:<15.6f} {rec_reg_loss:<15.6f} {reg_improvement:<14.1f}%")
...
class TimingADMM:
    ...
    def run_admm(self, x0, tau, niter=10, gfirst=False, show=False):
        ...
        if show:
            print('ADMM with Timing\n'
                  '---------------------------------------------------------\n'
                  'Proximal operator (f): %s\n'
                  'Proximal operator (g): %s\n'
                  'tau = %10e\tniter = %d\n' % (type(self.proxf), type(self.proxg),
                                                tau, niter))
            ...
            if show:
                if iiter < 10 or niter - iiter < 10 or iiter % (niter // 10) == 0:
                    ...
                    msg = '%6g  %12.5e  %10.3e  %10.3e  %10.3e  %10.4f  %10.4f' % \
                          (iiter + 1, np.real(x[0]),
                           pf, pg, pf + pg, proxf_time, proxg_time)
                    print(msg)
    ...
    def print_timing_summary(self):
        ...
        print(f"Proxf percentage: {100 * np.sum(self.timing_data['proxf_times']) / np.sum(self.timing_data['iteration_times']):.1f}%")
        print(f"Proxg percentage: {100 * np.sum(self.timing_data['proxg_times']) / np.sum(self.timing_data['iteration_times']):.1f}%")
