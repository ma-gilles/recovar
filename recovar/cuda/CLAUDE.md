...
## Auto-compilation
`recovar/cuda_backproject.py` auto-compiles the kernel on first import if `.so` is missing. No manual build needed for normal use.

## Interface
The kernel is exposed via JAX XLA FFI (custom call). `adjoint_slice_volume()` in `core/slicing.py` dispatches to CUDA when available, falling back to pure JAX otherwise.
