...
class CryoEMDataset:
        ...
        if image_source is not None:
            ...
            if dataset_indices is None:
                ...
                if source_layout.n_images != dataset_indices.size:
                    raise ValueError("dataset_indices must have the same length as the image_source")
                if np.array_equal(
                    dataset_indices,
    ...
    def tilt_particles(self):
        ...
        if not self.tilt_series_flag:
            return None
        return [
            np.asarray(local_images, dtype=np.int32) for local_images in self._index_layout.group_local_image_indices
        ]
    ...
    def get_proj(self, X, to_real=np.real, axis=0, hide_padding=True):
        im = to_real(
            fourier_transform_utils.get_idft2(jnp.take(X.reshape(self.volume_shape), self.grid_size // 2, axis=axis))
        )
        if hide_padding:
            im = im[
                self.hpad : self.image_source.unpadded_D + self.hpad,
                self.hpad : self.image_source.unpadded_D + self.hpad,
            ]
        return im
    ...
    def get_image_real(self, i, tilt_idx=None, to_real=np.real, hide_padding=True):
        ...
        if hide_padding:
            return to_real(
                fourier_transform_utils.get_idft2(self.get_image(i, tilt_idx))[
                    hpad : self.image_shape[0] - hpad, hpad : self.image_shape[1] - hpad
                ]
            )
        else:
    ...
    def get_denoised_image(self, i, tilt_idx=None, to_real=np.real, hide_padding=True, weiner_param=1):
        ...
        if tilt_idx is not None:
            ...
            CTFs = self.ctf_evaluator(
                self.CTF_params[image_ind[tilt_idx]][None], self.image_shape, self.voxel_size
            )  # Compute CTF
        else:
    ...
    def get_image_mask(self, indices, mask, binary=True, soften=5):
        ...
        from recovar.heterogeneity import (
            covariance_core,
        )  # Not sure I want this depency to exist... Could make some circular imports

        mask = covariance_core.get_per_image_tight_mask(
            mask,
            self.rotation_matrices[indices],
            self.image_source.mask,
            self.volume_mask_threshold,
            self.image_shape,
            self.volume_shape,
            self.grid_size,
            self.padding,
    ...
    def halfset_original_image_indices(self, halfset_id: int):
        return self._index_layout.original_image_indices_for_local(self.halfset_local_image_indices(halfset_id))

    def halfset_local_group_indices(self, halfset_id: int):
        return self._index_layout.local_group_indices_from_local_images(self.halfset_local_image_indices(halfset_id))

    def halfset_original_group_indices(self, halfset_id: int):
        return self._index_layout.original_group_indices_from_local_images(self.halfset_local_image_indices(halfset_id))
...
def _load_poses(particles_file, poses_file, D, n_images, dataset_indices):
    ...
    if poses_file is not None and poses_file.endswith(".pkl"):
        ...
        if dataset_indices is not None:
            ...
            raise ValueError(
                f"Pose count ({rots_raw.shape[0]}) must match loaded image count ({n_images}) when ind is not provided"
            )
        rots = rots_raw
        trans = trans_frac * D
...
def _apply_tilt_ctf_augmentation(ctf_params, tilt_dataset, tilt_series_ctf, dose_per_tilt, angle_per_tilt):
    ...
    if tilt_series_ctf == "from_star":
        ...
        ctf_eval = core.CTFEvaluator(
            mode=core.CTFMode.TILT_SERIES, dose_per_tilt=dose_per_tilt, angle_per_tilt=angle_per_tilt
        )
...
def reorder_to_original_indexing_from_halfsets(arr, halfsets, num_images=None):
    ...
    arr_reorder = (
        np.ones(arr_reorder_shape) * np.nan
    )  # nan things which are not in halfsets. They have been filtered out.
