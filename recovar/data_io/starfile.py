...
def _parse_star_file(filepath: str) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
    ...
    for block_name, block_data in data_blocks.items():
        ...
        if row_lengths[0] != len(cols):
            raise ValueError(f"Column count mismatch in {block_name}: {row_lengths[0]} values vs {len(cols)} headers")
    ...
    logger.debug(
        "Parsed %s: %d particles, %d columns, optics=%s",
        filepath,
        len(main_df),
        len(main_df.columns),
...
class StarFile:
    ...
    def set_optics_values(self, field: str, values: Union[float, List, np.ndarray]) -> None:
        ...
        if len(values) not in valid_lengths:
            raise ValueError(f"Values length {len(values)} not in valid set {valid_lengths}")
        ...
        if self.has_optics and field in self.data_optics.columns:
            if "_rlnOpticsGroup" in self.df.columns:
                ...
                if len(values) != 1:
                    raise ValueError(f"Without optics groups, can only set single value (got {len(values)})")
        ...
        # Update main data table
            ...
            if self.has_optics and len(values) == len(self.data_optics):
                ...
                mapped_values = [values[self.data_optics["_rlnOpticsGroup"].tolist().index(g)] for g in optics_groups]
        ...
        if image_name is not None:
            try:
                ...
                if not os.path.isabs(mrcs_path) and hasattr(self, "_star_path"):
                    mrcs_path = os.path.join(os.path.dirname(self._star_path), mrcs_path)
