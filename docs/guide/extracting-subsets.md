...
recovar extract_image_subset \
    --input-dir output/output/analysis_10/centers/vol0003 \
...
The `--input-dir` should point to a volume directory generated by `analyze`, `compute_state`, or `compute_trajectory`. For example:

```
output/output/analysis_10/centers/vol0000/
```
...
recovar extract_image_subset_from_kmeans \
    output/output/analysis_10/centers.pkl \
    subset_indices.pkl \
    0,3
...
|----------|-------------|
| `path_to_centers` | Path to `centers.pkl` from analyze |
