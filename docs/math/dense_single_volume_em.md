...
This document describes the stable algorithmic core, not the current transient API boundaries.
...
1. regularize the filter with the prior variance $\tau$,
2. divide $\text{Ft\_y}$ by the regularized $\text{Ft\_ctf}$,
...
2. apply $\text{CTF} / \text{noise\_variance}$,
3. generate all translated versions of the images,
4. flatten $(\text{image}, \text{translation})$ into one long batch,
5. multiply by $\text{projections}^\top$ to obtain all cross terms,
6. separately multiply $\text{CTF}^2 / \text{noise\_variance}$ by $|\text{projections}|^2{}^\top$ to obtain all norm terms,
...
3. flatten to $(\text{image} \times \text{translation})$,
...
6. backproject it into the 3D Fourier volume to accumulate $\text{Ft\_y}$,
7. separately sum probabilities over translations and backproject the $\text{CTF}^2 / \text{noise\_variance}$ weights to accumulate $\text{Ft\_ctf}$.
...
3. accumulates $\text{Ft\_y}$ and $\text{Ft\_ctf}$.
...
$\text{Ft\_y}$ and $\text{Ft\_ctf}$ are sums over images. This makes the single-volume dense path naturally compatible with:
...
4. **$\text{Ft\_y}$ and $\text{Ft\_ctf}$ are additive over image batches and therefore over devices.**
...
$\text{Ft\_y}$ and $\text{Ft\_ctf}$ are crucial iteration-local statistics. Their initialization, accumulation, and reset should be explicit in the public dense homogeneous API.
...
3. **Mean-statistics accumulation**
   - accumulate $\text{Ft\_y}$,
   - accumulate $\text{Ft\_ctf}$
