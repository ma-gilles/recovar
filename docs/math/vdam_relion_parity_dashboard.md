...
| Native atomic + coarse batched lanes `13284729/13284897` | **LANE-ONLY ONE-ITERATION MATH/RUNTIME PASS; DEFAULT OFF.** Holding atomic math fixed, warm wall -8.89%, expectation -11.45%, pass 1 -11.14%, coarse sum -8.71%, and coarse union -10.87%; launches are 48 instead of the earlier per-particle scheduler's 1,000. | All 3,000 STAR rows and discrete metadata are exact; cold/warm cross maps remain inside repeat envelopes with negligible bias and scale drift. The experimental source contains unmerged infrastructure, so it cannot be cherry-picked wholesale. | Manually port only the atomic batched-lane topology onto the tracking-derived shared kernel, repeat the focused GPU and atomic A-B-B-A live gates there, then run one true-200-iteration same-H100 no-growth/basin trajectory if the material gain survives. |
...
| **LANE-ONLY ONE-ITERATION PASS / TRACKING PORT OPEN** | Native atomic + coarse batched lanes `13284729/13284897` | The combined canonical control improves warm wall 6.99%; holding atomic math fixed still improves warm wall 8.89%, expectation 11.45%, and coarse union 10.87%. All 3000/3000 STAR rows/discrete outputs are exact and maps are repeat-bounded. Lane-only report SHA-256 `0a21380af341ba8159acf8401e71d9b9f5aaac9688c01df97f7d1920a518a512`. | Manually port only the atomic batched-lane topology onto the tracking-derived shared kernel, repeat the focused GPU and atomic A-B-B-A live gates there, then run one true-200-iteration same-H100 no-growth/basin trajectory if the material gain survives. |
...
| **SEALED COMPILE PROFILE / SHARED FIXED EXECUTOR TARGET** | Iterations 47--80 | `local.run_local_em_exact` accounts for 53.547 of 57.833 s XLA compile (92.6%). Big-JIT plus fused x-half alone consume 37.404 s. The compile-only forecast is 10.1% of the 423 s run, before packed-work savings. | Add a default-off fixed-capacity packed whole-local executor to the mature shared EM path. Preserve outer-call chronology, candidate sets, physical particle order, and radix buckets; pass valid images, row offsets/counts, and logical cutoff dynamically; keep preprocessing, fine posterior, noise, and x-half M-step physical shapes stable. Sealed analysis SHA-256 `fd33935f77c275615249e6149abfdf9abf50d4b1c8463aa4dcbe11f2ea176def`; runtime report SHA-256 `5ffa9ae565466aa924eccd1a836a3a5af1613df40cf4c584b344ec3dd4494165`. |

Invalid jobs `13270868` and `13270984` stopped in preflight before science and are not evidence. All listed lanes are diagnostic and default-off/unwired, with **no impact** on frozen correctness 2/20 or runtime 0/20. Forced nondefault native-texture path remains rejected after 120/120 strict artifacts differed.
...
1. Manually port only the contiguous batched-lane topology onto the tracking-derived shared atomic kernel. Incremental gate 13284897 cuts warm wall 8.89% with exact discrete outputs and repeat-bounded maps; repeat the focused GPU and atomic A-B-B-A live gates on the clean port before one true-200-iteration no-growth trajectory.
...
4. Continue the sealed profile's larger shared lever: a fixed-capacity packed whole-local executor plus pipelined data/layout and coarse-to-fine preparation. Target the measured 3.344 s excess idle and 53.547 s local compile churn while preserving shared EM/VDAM math and call chronology.
