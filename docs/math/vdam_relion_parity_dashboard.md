...
|:---:|---|---|
| 🟢 | What improved? | The systematic GF47 iteration-4 split is now a **live causal chain**, and the Wavg/BPref host edge is measured in both engines. RELION repeat 2 takes **`4,082 ns`** to enqueue Wavg, **`233 ns`** from Wavg return to BPref call entry, and **`3,613 ns`** inside BPref setup/enqueue. RECOVAR's first trace measured **`178 ns`** only to BPref `cuLaunchKernel` entry at iteration 4, exposing that the original intervals were not apples-to-apples. Complete enqueue/return tracing is now implemented. |
...
| 🟡 | Why can both be true? | Every candidate map is inside the four-repeat native direct relative-L2 envelope at all **201/201** checkpoints, but RECOVAR's repeat distribution is much narrower. A two-arm **233 ns** replay leaves RECOVAR repeats only **`1.10e-7`** apart while the paired native repeats span **`6.278e-5`**; one candidate/native arm is **`0.00950x`** the native diameter and the alternate native arm remains **`0.99988x`** away. Exact per-arm worker ownership also misses the alternate native map by **`6.2783e-5`** in its sealed arm. Simple host delay and worker identity do not restore reverse native-mode coverage. |
| 🔴 | First open operation | The serialized iteration-3 maps differ by **`6.29176e-5`** relative L2 and generate the full **`5.77441e-5`** PPref difference. The live intervention proves this state is sufficient, reconnecting the early mode split to the iteration-1 shared multi-particle BPref accumulation boundary. Exact PTX, clean-process ownership, observable CUDA context/stream state, simple launch delay, per-arm worker ownership, and RELION's exact Wavg-to-BPref same-stream chain are rejected as sufficient. The remaining boundary is the complete host/device enqueue/event chronology or an as-yet uncaptured launch operand before shared accumulation. |
| ➡️ | What is next? | Measure matching Wavg enqueue, pre-BPref gap, BPref enqueue, and Wavg-return-to-BPref-return intervals with complete trace head `2ac3dc735`. Then seal all per-particle launch operands and stream-event chronology at iteration 4 and iteration 1, diff them fail-closed, and replay only the first differing operand or synchronization edge. Run focused 0--4/0--20 repeats before another 0--200 qualification. |
| 🟢 | What finished? | Native gap **`13086700`**, candidate trace **`13087189`**, 233 ns panel **`13087232`**, and exact-worker arm **`13087339/a`** join context **`13084397`**, clean-process **`13084892`**, exact Wavg **`13086085`**, and exact PTX **`13083892`**. Setup-only **`13087180`** ran no science; `13087339/b` produced a report but no terminal seal and is excluded. Complete-interval CUDA build from `2ac3dc735` is active. |
