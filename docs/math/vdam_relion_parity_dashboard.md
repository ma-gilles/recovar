...
| 🔴 | What still fails? | The frozen single-realization 0--200 score remains **2/20 strict**. In the first fresh GF47 repeat, one particle first selects a different pose at iteration **27**, the point map gate first fails at **75**, minimum cross-engine FSC-AUC is **`0.756025`**, and minimum RECOVAR-minus-RELION ground-truth FSC-AUC is **`-0.0031605`**. Same-H100 runtime is **`2151.9 / 350.3 s = 6.14x`** RELION. K>1, real-data, and final CLI/GUI qualification have not started. |
...
| 🟡 | What is running? | Current-head job **`13073643`** has completed and sealed repeat **1/4** and is running repeat 2. Dependent distribution job **`13075917`** will audit the 4x4 map/state/schedule/runtime panel. Legacy analyzer backtest **`13076002`** verifies fail-closed handling of older metadata after focused tests passed. |
