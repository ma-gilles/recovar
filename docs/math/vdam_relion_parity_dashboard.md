...
| ➡️ | What is next? | Test persistent per-device callback scratch/texture ownership independently of persistent streams. This targets the `cudaFree`/setup overhead exposed by Nsight while keeping the accepted shared numerical functions and serial particle/rotation oracle unchanged. Only a green exact-H100 1--20 direct audit advances to repeats/profiling. |
| 🟢 | What finished? | Dense exact control `13154000` passes every checkpoint in `334 s`; bounded compact scoring and compact posterior are exact but no faster. Worker-private BPref reaches `192--212 s` but misses `1723@19`. Dynamic shared-BPref claiming reaches `181--187 s`; the phase-aligned expansion rejects it at **4/5** with the historical `286@4 / 2903@16 / 903@18` branch. The bitwise CUDA Wavg and persistent-stream arms reach **299 s / 297 s**, respectively, but both recreate the same branch. No-global-blocking remains rejected at **10/11**, launch-blocked at **6/7**, and EM's projection cache at **0/2**. |
