...
| Performance lanes | non-scoring | Cache-only is retained for repeat/scale; chunking is rejected. The shared eight-stream coarse scheduler is mathematically accepted and measurably faster, but held below the runtime target. The 65--128 single-lane specialization is inapplicable to GF46's actual T=29 coarse call; the clean tracking-derived native-atomic T=29 batched-lane port cuts warm wall about 9%, but its frozen two-repeat maximum-envelope smoke gate fails narrowly. It remains default-off while the separately predeclared replicated roundoff-equivalence panel is active; no long trajectory is authorized yet. |
...
| EM reuse | shared production primitives | The remaining boundary is execution topology/variability, not duplicate projector or scorer math. The default-off shared fixed-capacity host seam is at Phase 1d (`41c1cdbd10`), with no production or GPU invocation yet. |
...
| **CLEAN PORT: RUNTIME PASS / STRICT N=2 FAIL / REPLICATED GATE RUNNING** | Native atomic + coarse batched lanes `13285438/13285647/13286397` | The tracking-derived shared kernel improves warm wall 9.03%, expectation 10.26%, GPU union 10.45%, and coarse union 10.83%. All 3000/3000 STAR rows/discrete outputs are exact, but the frozen two-repeat max-envelope rule fails narrowly. Prior experimental lane-only job `13284897` remains supporting, non-production evidence. Immutable clean-port report SHA-256 `b46b54d168c9cf4febf7fb6527d2633e7c8c4b83f4569f0b3cfd2d2e801f9506`. | Monitor sealed 8+8 fresh-process, same-H100 blocked repeat-panel job 13286397 at overlay fb9aaf271 (acceptance SHA-256 4f266733a4a7ffbc79c06d340918a4389c79d7c5cf34401031d07a64c38f258d). Keep the frozen n=2 strict result visibly failed. Do not start the true-200-iteration trajectory unless every replicated roundoff-equivalence and material-runtime criterion passes and the report is reviewed. |
...
| **SEALED COMPILE PROFILE / SHARED FIXED EXECUTOR TARGET** | Iterations 47--80 | `local.run_local_em_exact` accounts for 53.547 of 57.833 s XLA compile (92.6%). Big-JIT plus fused x-half alone consume 37.404 s. The compile-only forecast is 10.1% of the 423 s run, before packed-work savings. Phase 1d `41c1cdbd10` seals the default-off shared host bundle (109 passed; 10 passed, 17 deselected) but has no GPU/production wiring. | Factor the mature run_local_bucket_big_jit call into one shared helper, then execute one default-off score-only fixed-capacity call through that same helper and compare scores, logZ, argmax, and posterior diagnostics before widening to all calls. Sealed analysis SHA-256 `fd33935f77c275615249e6149abfdf9abf50d4b1c8463aa4dcbe11f2ea176def`; runtime report SHA-256 `5ffa9ae565466aa924eccd1a836a3a5af1613df40cf4c584b344ec3dd4494165`. |
...
1. Keep the clean tracking-derived batched-lane port default-off. Its 9.03% warm-wall gain is material, but immutable job 13285647 remains a frozen two-repeat strict FAIL. Run sealed overlay job `13286397` at overlay `fb9aaf2717` / acceptance `4f266733a4a7ffbc79c06d340918a4389c79d7c5cf34401031d07a64c38f258d` as the predeclared 8+8 blocked, fresh-process roundoff-equivalence panel; only a full pass may authorize one true-200-iteration no-growth/basin trajectory.
...
4. Continue the sealed profile's larger shared lever from default-off Phase 1d `41c1cdbd10`: factor the mature big-JIT call into one shared helper and run one score-only fixed-capacity call through it. Target the measured 3.344 s excess idle and 53.547 s local compile churn while preserving shared EM/VDAM math and call chronology.
