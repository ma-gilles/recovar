...
| What is the active theory? | Multi-particle CUDA **within-particle block admission / atomic order** is now the leading axis. Exact native per-particle grid counts still fail: issue+count raw data are **`9.12e-6 / 8.50e-6`**, weights **`3.35e-6 / 2.93e-6`**; timing+count is similarly non-passing. Although global all-block atomic rank is about `0.99985`, particle launch order dominates that number: median native/candidate atomic rank *inside one particle* is only **`0.50 / 0.65`** for issue/timing arms. |
| What is next? | Replay RELION's captured within-particle block-start order on top of exact global issue or timing and native grid counts. Jobs **`13071700 / 13071701`** are live. Chronology schema v2 now reports matched block/atomic sets and within-particle rank distributions directly. |
...
| Within-particle native-order discriminator | local science `92214d9ec`; 52 focused tests pass; jobs `13071700 / 13071701` live | composes exact global particle issue/timing with captured native block-start order and exact grid counts; chronology v2 makes the newly exposed within-particle atomic rank a durable progress metric |
...
|---:|---|---|---|---|
| 1 | GF47 production BPref accumulation boundary @58 | exact particle counts, global issue/timing, and physical grid counts are insufficient; within-particle atomic-rank median remains only `0.50 / 0.65` | native within-particle block-order 2x2 is live: issue+grid `13071700`, timing+grid `13071701` | none yet; promote only a robust aggregate correction, then rerun 0--200 |
