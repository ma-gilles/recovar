...
| 🔴 | What still fails? | The frozen single-realization 0--200 score remains **2/20 strict**. Fresh repeat 1 first fails particle/map @**27/75**, with minimum FSC-AUC **`0.756025`** and runtime **`6.14x`**; repeat 2 first fails @**4/13**, with minimum FSC-AUC **`0.515764`** and runtime **`5.75x`**. K>1, real-data, and final CLI/GUI qualification have not started. |
...
| 🟡 | Current theory | There is no remaining evidence for a deterministic raw-BPref formula, topology, or instruction-order bug at iteration 58. In the fresh 2x2 state panel, both RECOVAR schedules match native mode 1 through iteration 62 while native mode 2 is already uncovered @4; a RECOVAR translation mode absent from the first two fresh native samples @70 exactly matches an older independent RELION repeat. The leading blocker is **stochastic mode support/coverage**, followed separately by runtime. |
...
| 🟡 | What is running? | Current-head job **`13073643`** has sealed repeats **1--2/4** and is running repeat 3. Same-GPU interim map audit **`13076857`** and final dependent 4x4 distribution audit **`13076833`** use exact science head `f5e7d74ade251...` and analyzer head `dca338dcf35f...`. |
