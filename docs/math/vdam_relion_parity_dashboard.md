...
| 🔴 | What still fails? | The frozen single-realization 0--200 score remains **2/20 strict**. Fresh repeats 1--3 first fail particle/map @**27/75**, **4/13**, and **71/115**, with minimum FSC-AUC **`0.756025 / 0.515764 / 0.897837`** and runtime **`6.14x / 5.75x / 5.55x`**. K>1, real-data, and final CLI/GUI qualification have not started. |
...
| 🟡 | Current theory | There is no remaining evidence for a deterministic raw-BPref formula, topology, or instruction-order bug at iteration 58. Native repeat 3 exactly resolves the apparent candidate particle/schedule failures @27/@70 from the 2x2 panel, proving those RECOVAR states were native modes missing from the first two samples. All three RECOVAR candidates still choose the same early mode, leaving native repeat 2 uncovered @4. The leading blocker is **stochastic mode support/coverage**, followed separately by runtime. |
| ➡️ | What is next? | Finish all four independent GF47 production 0--200 pairs and compare candidate validity, reverse native-mode coverage, particle states, complete schedule modes, maps, and runtime. Only after that evidence defines a stable native envelope will the frozen K=1 matrix be rerun; thresholds and the 20-case denominator remain unchanged meanwhile. |
| 🟡 | What is running? | Current-head job **`13073643`** has sealed repeats **1--3/4**; repeat 4 RELION is complete and RECOVAR is running. Final dependent 4x4 audit **`13077225`** uses exact science head `f5e7d74ade251...` and analyzer head `ac79e3348c1b...`. |
