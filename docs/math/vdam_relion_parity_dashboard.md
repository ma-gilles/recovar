...
|:---:|---|---|
| 🟢 | What improved? | Both formerly causal controller boundaries are now closed. The shared canonical reduction gives the native iteration-4 coarse mask (**100/100, 0/16,704 mismatches**), and STAR-seeded trial poses restore RELION expected accuracy and translation topology at iteration 20: `2.391 deg / 1.88275 A`, `2.824125 A`, and 148 translations. The complete 0--20 map trajectory passes with minimum FSC-AUC `0.999999972881`. |
...
| 🔴 | What still fails? | Strict particle parity remains red: focused job `13108972` first finds one low-posterior row at iteration **4** (`Pmax 0.002694` versus `0.001884`, alternate orientation), later peaking at two divergent stored rows. The continuous optimal-offset-change audit misses once at iteration **18** by `0.006156 A`. The frozen score remains **2/20 strict** and runtime remains **0/20** at **4.91--11.58x** RELION. |
| 🟡 | Why can both be true? | Map, controller, and grid parity can pass while a near-flat particle posterior selects another discrete orientation. The new expected-accuracy call changes host/GPU chronology even though it does not change the early grid; downstream shared atomic reconstruction can therefore realize a nearby map and expose another cutoff-sensitive particle. This is the leading theory, not yet a closed cause. |
...
| ➡️ | What is next? | Repeat the immutable shared-wrapper build to measure whether the one-particle iteration-4 choice is systematic or inside native/candidate realization width, then capture that particle's coarse/fine cutoff operands. Keep the gate strict; only after 0--20 particle closure proceed to 0--200 and the unchanged K=1 matrix. |
| 🟢 | What finished? | Focused `13108972` closes expected accuracy and translation topology at iteration 20 and passes every 0--20 map checkpoint, but fails closed on particle/schedule diagnostics. InitialModel now imports the authoritative EM scorer, significance, sparse pass-2, local refinement, layout, and expected-accuracy binding by object identity; shared-wrapper commit is `193e31b01`. |
