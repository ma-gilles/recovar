...
| ➡️ | What is next? | Qualify EM/RELION-style persistent worker-stream lifetime inside the existing fused BPref callback. Commit `04742df83` and Hopper binary SHA `3580d11aa7ef...` pass focused source checks **3/3** and focused GPU checks **2/2** (job `13167391`). Exact-H100 trajectory array `13167503` is queued behind the job currently occupying the frozen device; a green 1--20 direct audit advances to repeats/profiling, while a red audit rejects the arm without changing defaults. |
| 🟢 | What finished? | Dense exact control `13154000` passes every checkpoint in `334 s`; bounded compact scoring and compact posterior are exact but no faster. Worker-private BPref reaches `192--212 s` but misses `1723@19`. Dynamic shared-BPref claiming reaches `181--187 s`; the phase-aligned expansion rejects it at **4/5** with the historical `286@4 / 2903@16 / 903@18` branch. The bitwise CUDA Wavg arm reaches **299 s** but recreates the same branch. No-global-blocking remains rejected at **10/11**, launch-blocked at **6/7**, and EM's projection cache at **0/2**. |
