...
|:---:|---|---|
| 🟢 | What improved? | The shared canonical coarse reduction is now live, not only an offline replay. GF46 is exact through iteration 19: the iteration-4 coarse mask is **100/100 with 0/16,704 mismatches**, all 3,000 particle states match at every checkpoint through iteration 19, and the iteration-4 map has FSC-AUC `0.999999999965`. |
...
| 🔴 | What still fails? | GF46 first departs at iteration **20**, when InitialModel skips RELION expected-accuracy estimation and retains sentinel `acc_rot=0`, `acc_trans=999`. It consequently builds a `3.0 A / 116` translation grid instead of native `2.824125 A / 148`, and all 200 active particle states split. The frozen score remains **2/20 strict** and runtime remains **0/20** at **4.91--11.58x** RELION. |
| 🟡 | Why can both be true? | The shared EM scorer/posterior is now demonstrated exact on the previously causal boundary. The remaining failure is outside that engine: VDAM's changing subset can contain particles without a stored prior pose, so its separate expected-accuracy wrapper returns unavailable before the E-step. RELION and mature supplied-map EM retain valid trial orientations and compute finite accuracy values. |
| 🔴 | First systematic departure | With canonical reduction enabled, GF46's first operative departure moves from iteration 4 to the iteration-20 controller update. Candidate/native offset range still matches at `9.255374 A`, but offset step and translation topology split at `3.0 A / 116` versus `2.824125 A / 148`. The original frozen candidate has the same sentinel/controller values, proving this is a pre-existing boundary newly exposed by the iteration-4 correction. |
| ➡️ | What is next? | Seed InitialModel particle orientations from input STAR metadata, as RELION does, then qualify the exact composed fix through 0--20. Once green, remove VDAM's duplicate binding wrapper in favor of the mature EM expected-accuracy module, requalify, and extend to 0--200 plus the unchanged K=1 matrix. |
| 🟢 | What finished? | Production commit `b90f22b1c` passed H100 build/test jobs `13107529 / 13107668`. Live GF46 0--4 job `13107898` has zero coarse-mask and particle-state mismatches and ran in 52 s versus the prior 56 s short control. Live 0--20 job `13108111` is exact through 19 and isolates the controller split at 20. |
