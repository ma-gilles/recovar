...
| 🟡 | Current theory | The first unresolved candidate-support boundary is now exact: at iteration **58**, particle **`1021@particles.128.mrcs`**, RECOVAR repeats 1--2 choose one orientation while repeats 3--4 choose the RELION-1/3/4 orientation. In the captured repeat, the latter wins by only **`0.0001220703125`** (**256 float32 ULPs**) because a **`+0.418545`** raw-score advantage nearly cancels a **`-0.418423`** rotation-prior disadvantage; translation priors are identical. This is not an exact-tie or posterior tie-break bug. The leading correctness theory is a small fine-score reproducibility/order/precision difference that flips a near-cancelled orientation mode; native stochastic mode coverage remains an alternative until the independent native/candidate score capture closes it. Runtime is a separate blocker. |
| ➡️ | What is next? | Complete the independent native/candidate score capture at the exact iteration-58 particle, compare its raw fine-score components with the first capture, and use the 20-repeat early panel to estimate mode frequencies. Patch only the first reproducible score operand that lies outside RELION's own repeat envelope, then rerun the focused K=1 trajectories and expanded outlier/noise/pose/scale matrix. Thresholds and the 20-case denominator remain unchanged. |
| 🟡 | What is running? | Four full GF47 pairs are sealed. Final 4x4 map/state distribution audit **`13077225`** is running; exact-H100 independent native/candidate score capture **`13078944`** is running; the 20-repeat early-mode panel **`13078176`** is queued behind it. Science remains `f5e7d74ade251...`; analyzer heads are `ac79e3348c1b...` and `f4305b23f146...`. |
...
|---:|---|---|---|---|
| 1 | GF47 fine-score mode boundary @58, particle 1021 | four independent panels put **32/32** raw M-step data/weight arms inside the native repeat floor; four full trajectories are sealed, and the first unsupported particle mode is a `0.0001220703125` near-cancellation between raw score and rotation prior | independent native/candidate score capture **`13078944`**, 4x4 map/state audit **`13077225`**, then 20-repeat early-mode panel **`13078176`** | no frozen score change; M-step boundary is green, E-step mode selection and trajectory remain red |
