...
| 🟡 | Current theory | The iteration-58 particle-1021 branch is reproducible but inherited: the second RECOVAR capture again selects the RELION-1/3/4 orientation with only **`0.00006103515625`** margin, while native RELION separates the same modes by **`0.0059356689453125`**. RECOVAR loses **`0.003300667`** in raw-score advantage and **`0.002573967`** in orientation-prior advantage versus native; translation cancels. The upstream discriminator is now iteration **4**, particle **`1085@particles.128.mrcs`**: all **20/20** same-H100 RECOVAR repeats choose native mode 1/3/4 and **0/20** choose native mode 2. The leading theory is a systematic early E-step score/prior bias, not insufficient candidate sampling or a tie-break. Runtime remains separate. |
| ➡️ | What is next? | Capture particle 1085 from native repeat 2's exact iteration-3 optimiser and a fresh RECOVAR full-schedule run, then decompose the iteration-4 coarse/fine scores and priors. Patch only the first reproducible operand outside RELION's native envelope, then rerun the focused K=1 trajectories and expanded outlier/noise/pose/scale matrix. Thresholds and the 20-case denominator remain unchanged. |
| 🟡 | What is running? | Exact-H100 iteration-4 particle-1085 paired capture **`13079981`** is running from native repeat 2; final 4x4 map/state distribution audit **`13077225`** is running. Four full GF47 pairs, independent iteration-58 capture `13078944`, and the 20-repeat early panel `13078176` are sealed. Science remains `f5e7d74ade251...`; analyzers are `ac79e3348c1b...`, `306a2a7a5...`, and `ef8fd3dfc...`. |
...
|---:|---|---|---|---|
| 1 | GF47 systematic mode boundary @4, particle 1085 | all 20 same-H100 candidates occupy native mode 1/3/4 and none occupies native mode 2; the later iteration-58 gap contains both raw-score and orientation-prior inheritance | paired native-repeat-2/candidate coarse+fine capture **`13079981`**; 4x4 map/state audit **`13077225`** | no frozen score change; M-step boundary is green, early E-step mode coverage and trajectory remain red |
