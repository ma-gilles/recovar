...
| 🔴 | First open operation | The serialized iteration-3 maps differ by **`6.29176e-5`** relative L2 and generate the full **`5.77441e-5`** PPref difference. The live intervention proves this state is sufficient, reconnecting the early mode split to the iteration-1 shared multi-particle BPref accumulation boundary. Exact PTX, clean-process ownership, observable context state, simple delay, Runtime-versus-Driver launch, per-arm worker ownership, and exact same-stream Wavg are rejected as sufficient. The remaining boundary is an uncaptured per-particle launch operand or stream/event dependency before shared accumulation. |
| ➡️ | What is next? | Byte-seal every immutable BPref launch operand and the accumulator state seen at the launch edge, beginning with the iteration-4 causal particle and then iteration 1. Pair this with per-particle stream/event chronology, diff both engines fail-closed, and replay only the first actual difference. Run focused 0--4/0--20 repeats before another 0--200 qualification. |
| 🟢 | What finished? | Complete candidate trace **`13087597`**, Runtime trace **`13087838`**, and Runtime repeat panel **`13087881`** join native gap **`13086700`**, 233 ns panel **`13087232`**, and exact-worker arm **`13087339/a`**. Runtime science head is `b60152741`, binary SHA is `41a2fe4d31c2...`, and focused guards pass **8/8**. No productive job is currently running. |
