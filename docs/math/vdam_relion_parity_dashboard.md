...
| Native-atomic T=29 x eight streams `13281836/13283759` | **ONE-ITERATION MATH/RUNTIME PASS; DEFAULT OFF.** Warm wall -6.93%, expectation -11.03%, pass 1 -13.87%, GPU union -15.99%, and coarse union -17.94%. | All 3000/3000 STAR rows/columns and discrete metadata are exact; warm cross-map rel-L2 `2.332e-09` is below repeat envelope `2.519e-09`. Long-run no-growth is untested and per-image launches still inflate coarse work 28.91%. | Do not enable by default. First qualify the scientifically distinct coarse-grained batched-lane scheduler, which removes the current 1000-launch 28.91% coarse-work inflation; then run the larger winner through a repeat/no-growth representative trajectory and multi-dataset basin gates. |
...
| **ONE-ITERATION MATH/RUNTIME PASS; DEFAULT OFF** | Native-atomic x eight streams `13281836/13283759` | Warm wall improves 6.93%, expectation 11.03%, and coarse union 17.94%; all 3,000 rows/decisions are exact and maps remain inside non-directional repeat noise. Long-run growth remains untested. | Do not enable by default. First qualify the scientifically distinct coarse-grained batched-lane scheduler, which removes the current 1000-launch 28.91% coarse-work inflation; then run the larger winner through a repeat/no-growth representative trajectory and multi-dataset basin gates. |
...
4. Implement the sealed profile's large shared lever: approximately eight coarse-grained batched lanes plus pipelined data/layout and coarse-to-fine preparation. Target the measured 3.344 s excess idle while avoiding both six serial monolithic kernels and 1000 work-inflating per-image launches.
