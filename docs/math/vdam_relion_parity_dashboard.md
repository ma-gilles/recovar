...
| Persistent linear callback scratch (`cb953f7cf`) | focused source **3/3**, GPU **2/2** | **195 s** (`-40.7%`) | **0/3,000 divergent particles at all 20 checkpoints** | 🟡 repeat qualification |
...
| ➡️ | What is next? | Run the persistent-linear-scratch arm through the same **16 clean-process** exact-H100 1--20 qualification as the serialized oracle. If all 16 direct audits remain exact, profile it and advance to a complete 200-iteration trajectory; any repeat failure leaves the default unchanged and returns to worker-private deterministic reduction. |
| 🟢 | What finished? | Persistent scratch job `13168384_4` is the first combined speed/correctness positive: **195 s**, focused GPU **2/2**, and no divergent particle across all 20 checkpoints (audit SHA `ca29d2c4669...`). Dense exact control `13154000` passes in `334 s`. Worker-private BPref reaches `192--212 s` but misses `1723@19`; dynamic shared-BPref reaches `181--187 s` but fails the historical branch. CUDA Wavg and persistent streams reach **299 s / 297 s** but also fail that branch. |
