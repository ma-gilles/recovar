...
`13132833` is queued on the pinned node with a one-GPU fail-closed UUID gate.
Two-GPU submissions `13132648 / 13132747` were cancelled while pending so the
first could not run the intrusive coarse observer and the second would not
reserve an unused H100; setup
