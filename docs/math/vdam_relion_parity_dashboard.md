...
| What is the active theory? | Multi-particle CUDA atomic timing is now directly isolated. Under captured global issue order, candidate/native first-start and first-atomic ranks match at **`0.999994 / 0.999992`**, but RECOVAR compresses the 200-particle device window to **`21.2M` cycles**, only **`6.65%`** of RELION's `319.2M`. Candidate gaps are nearly uniform around `104k` cycles while RELION's p90 gap is about `7.1M`; the remaining narrow axis is temporal overlap, not particle membership, owner, per-worker order, or global rank order. |
| What is next? | Replay RELION's captured per-particle start offsets with a lightweight GPU timeline gate, trace the resulting candidate chronology, and test whether restoring the native overlap window closes both raw accumulator halves before any 0--200 qualification. |
...
| Candidate/native particle chronology | local science `15a685150`; compile-level gate passes 58 focused tests; paired traced job `13068548` completes in 402 s | all 200 launch sequences and workers match exactly; first-start/atomic ranks match at `0.999994 / 0.999992`, but candidate start/atomic spans are only `0.0665x` native. This directly isolates temporal spacing/overlap as the next discriminator; score unchanged |
...
Last scientific update: **2026-08-27 19:20 ET**
...
|---:|---|---|---|---|
| 1 | GF47 production BPref accumulation boundary @58 | exact particles, operands, isolated scatter, worker owners, global issue order, and device rank order are closed; candidate device span is only `0.0665x` native | replay captured per-particle GPU start offsets and remeasure raw accumulators plus device chronology | none yet; promote only a robust aggregate correction, then rerun 0--200 |
