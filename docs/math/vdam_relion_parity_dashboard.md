...
| What is the active theory? | Multi-particle CUDA **physical grid shape / occupancy** is now the leading axis. A passive target-58 trace showed the ordinary candidate window is only **`3.36M / 319.22M = 1.053%`** of native. Replaying native per-particle launch offsets restored **`99.9815%`** of the native span but made all four raw accumulators worse, decisively rejecting elapsed launch timing alone. Particle membership, operands, isolated scatter, owner, per-worker order, global issue order, rank order, and simple host timing are closed or rejected as sufficient. |
| What is next? | Run a same-RELION-arm 2x2 discriminator: captured issue order versus captured timing, each combined with the exact native per-particle physical block counts. Jobs **`13070934 / 13070935`** are live. The runner now captures worker and block chronology at target iteration 58 and fails closed if those seals do not match the candidate arm. |
...
| Same-arm native-count discriminator | runner fix `35025330f`; 10 focused contract tests pass; jobs `13070934 / 13070935` live | each arm now captures its own iteration-58 worker schedule and block chronology before replaying exact per-particle native block counts; this is the current decisive test |
...
|---:|---|---|---|---|
| 1 | GF47 production BPref accumulation boundary @58 | exact particles, operands, isolated scatter, worker owners, issue/rank order, and elapsed launch timing are closed or rejected; passive candidate span is `0.01053x` native and timing replay at `0.999815x` worsens parity | same-arm exact-native-block-count 2x2 is live: issue+count `13070934`, timing+count `13070935` | none yet; promote only a robust aggregate correction, then rerun 0--200 |
