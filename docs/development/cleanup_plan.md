...
| Follower dispatch and history | `relion_worker_scale.py`, `helpers/iteration_history.py` and controller dispatch helpers | PR180 supplies setup and history owners. Review moving the remaining follower dispatch helpers to the scale owner with explicit precision and iteration inputs, preserving replay replacement, mutation order and telemetry. Keep scheduling decisions in the controller. |
