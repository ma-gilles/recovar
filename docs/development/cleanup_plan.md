...
| Real K1 repeatability | Two unchanged PR158 runs on the same H100 fail the direct-map gate from iteration 8. Saved accumulators already differ in iteration 1; support/Pmax first differ at iteration 2. | Capture the existing pre-/post-half-join boundary on unchanged source, then replay identical operands at the first divergent operation. Keep autonomous convergence and fixed-state arithmetic as separate results. |
