...
| Sparse scoring | `helpers/sparse_pass2_bucketed.py`, `helpers/bpref_diagnostics.py` | BPref capture context, membership selectors/counter/rotation-mass writers, and contribution validation have one diagnostic owner with direct callers. Continue separating candidate planning, kernel calls and sufficient-statistic reduction one boundary at a time. Preserve packed shapes, reduction order, casts and ownership; other score/noise diagnostic writers remain in the sparse module. |
