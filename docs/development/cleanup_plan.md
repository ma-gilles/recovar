...
| Real K1 repeatability | Two unchanged PR158 runs on the same H100 fail the direct-map gate from iteration 8. Shortened controls locate different accumulators before the first half join; support/Pmax first differ at iteration 2. | Capture upstream operands and replay identical per-particle accumulation inputs with the production support radius and fused launch boundaries. Keep autonomous convergence and fixed-state arithmetic as separate results. |
...
| Half/class outputs | `HalfScoreResult`, `PerHalfOutputs`, `relion_replay._RelionHalfInputState` and local-search dispatch | The output containers now document half-local ordering, mutable list ownership and separately populated class fields. Before combining output unpacking, test unequal half sizes and K greater than 1. |
