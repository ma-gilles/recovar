...
| Half/class outputs | `HalfScoreResult`, `PerHalfOutputs`, `relion_replay._RelionHalfInputState` and local-search dispatch | Dense/local scoring and class-result scatter now receive the existing output container. Exact comparisons cover unequal half sizes and K1/K2/K4/K8/K16 without changing list identities. Next simplify common result unpacking while preserving per-branch assignments and update order. |
