...
| Half/class outputs | `dense/score_outputs.py`, `refinement/half_inputs.HalfInputState`, `helpers.types` and local-search dispatch | Dense/local scoring, sparse pass-2 and class-result scatter use named results. Sparse pass-2 producers return `SparsePass2Output` directly; its optional-field sentinel, tuple builder and positional decoder are removed. The local kernel returns `LocalEMResult`; the positional packer and both decoders are removed, with all sixteen return-flag combinations and existing caller cases preserved. Continue reviewing state transitions and finalization boundaries, preserving optional captures, class mass, array ownership and update order. |
