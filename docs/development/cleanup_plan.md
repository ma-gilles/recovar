...
| Follower dispatch and history | `relion_worker_scale.py` and `helpers/iteration_history.py` | Setup, dispatch and input remapping now share the scale owner. The controller supplies precision, numbered iteration and logger. Existing tests and independent comparisons cover replay replacement, mutation order and telemetry. Review the remaining controller-side scale-update/finalization boundary while keeping scheduling decisions in the controller. |
