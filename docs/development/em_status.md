...
and `dense_adapter.py` into owners is the natural next phase. Running the whole `tests/unit/initial_model` directory (added to the move gates on September 11) shows six failures that pre-date the cleanup packages (they fail at `11bc4f0c2`): the four `test_m_step_halfset_averaging` cases (the SSNR recorder the test installs is never called; a stale test or a lost halfset-weight route, for the VDAM owner to adjudicate), `test_local_mstep_donation_ab::test_control_and_donated_wrappers_share_one_sealed_numeric_source` (the sealed static-argument drift already recorded at `67b184b39`) and `test_vdam_parity_dashboard` (a dashboard text assertion). They are deselected by name in the move gates and listed in each receipt directory's `known_initial_model_failures.txt`; the VDAM merge guards that read a fixed list of InitialModel files now read the whole package. The historical native trace-order failure at `11bc4f0c2` was resolved in `05a07d680`: the clean instrumented RELION source `c770287` confirms that the clock sample belongs before interpolation. The existing assertion now passes, with 50 trace/build tests passing and all non-tracing device functions unchanged in an isolated CUDA build. [Source, build and PTX evidence](/scratch/gpfs/CRYOEM/gilleslab/em_work/codex/cuda_trace_timestamp_20260912/result.json). A short current-source A100 trace execution now passes: one active row produces a valid chronological record and traced/untraced outputs are bitwise equal, with the loaded read-only binary hash unchanged. [Execution receipt](/scratch/gpfs/CRYOEM/gilleslab/em_work/codex/cuda_trace_execution_20260912/result.json). This checks one trace specialization; broader replay variants and trajectory qualification remain open. A seventh pre-existing failure sits outside `initial_model`: `tests/unit/test_sparse_pass2_relion_f32_posterior.py::test_relion_f32_fine_posterior_is_default_for_xhalf` asserts the VDAM default (`relion_x_half_f32_fine_posterior_enabled()` true without the env switch) and passes at its VDAM parent `8ab1a44be`, while the merge `c38fc62f0` (VDAM execution paths onto the PR179 checkpoint) deliberately kept the PR179 production default (`default=False`, "enable this separately qualified arithmetic path explicitly"); the test and the production default disagree, and which one stands is a scientific-default decision for the user, not a cleanup change. The pass-2 move gates deselect it by name (`known_pass2_failures.txt`). Two findings from the duplicate scan are deliberately not packages. The long
