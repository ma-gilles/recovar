...
def test_relion_gradient_runtime_max_significants_uses_3d_per_class_cap():
    do_grad = relion_metadata.relion_do_grad_for_iteration(
    ...
    assert (
        relion_metadata.relion_active_max_significants(
            -1,
            do_grad=do_grad,
            n_classes=4,
...
def test_relion_runtime_max_significants_preserves_argument_and_firstiter_cc():
    ...
    assert not relion_metadata.relion_do_grad_for_iteration(
        gradient_refine=True,
        has_converged=False,
        iteration=1,
        number_iterations=8,
