...
def test_half_staging_preserves_current_crop_mask_and_scaling(
    ...
    monkeypatch.setattr(p, "relion_projector_half_to_texture_full",
...
def test_nonqualified_geometry_keeps_existing_staging(
    ...
    monkeypatch.setattr(p, "project_half_spectrum", lambda *a, **kw:
                        jnp.zeros((1, output_size * (output_size // 2 + 1)), dtype=jnp.complex64))
    p._project_relion_projector_texture(slab, rows, (32, 32), r_max=16,
        projector_output_size=output_size, padding_factor=padding_factor)
    assert calls == ["full"]
...
def test_gpu_half_staging_matches_previous_full_staging(padding_factor, compact, mask_disk):
    ...
    angles = np.array([0., .137, -.219, np.pi / 2], dtype=np.float32)
    rotations = np.stack([np.array([[np.cos(a), -np.sin(a), 0.],
                                  [np.sin(a), np.cos(a), 0.], [0., 0., 1.]], dtype=np.float32)
                          for a in angles])
