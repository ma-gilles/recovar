...
def test_convergence_distance_minimizes_over_relion_symmetry_mates(label):
    ...
    from recovar.em.dense_single_volume.helpers.convergence import (
        relion_angular_distance_per_particle,
    )
...
def test_refinement_symmetry_options_canonicalize_at_construction(requested, canonical):
    from recovar.em.dense_single_volume.refinement_options import SymmetryOptions
...
def test_firstiter_cc_fine_grid_uses_reduced_asymmetric_unit_ids(label):
    ...
    actual = _build_firstiter_cc_pass2_grids(
        coarse,
...
def _write_iteration_debug_metadata(
    ...
    *,
    ...
    from recovar.em.dense_single_volume.debug_dumps import (
        _K1_INTERNAL_FOURIER_DUMP_DIR_ENV,
        _K1_INTERNAL_FOURIER_DUMP_ITERATION_ENV,
        _save_iteration_intermediates,
    )
    from recovar.output import output as output_module

    monkeypatch.delenv(_K1_INTERNAL_FOURIER_DUMP_DIR_ENV, raising=False)
    monkeypatch.delenv(_K1_INTERNAL_FOURIER_DUMP_ITERATION_ENV, raising=False)
