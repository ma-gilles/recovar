...
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "..", "fixtures", "relion5_tomo_subset")
...
def test_conversion_row_count(fixture_paths, tmp_path):
    ...
    assert len(out_df) == len(ref_df), f"Row count mismatch: got {len(out_df)}, expected {len(ref_df)}"
...
def test_conversion_string_columns_exact(fixture_paths, tmp_path):
    ...
    out_df = out_df.sort_values(["_rlnGroupName", "_rlnMicrographPreExposure"]).reset_index(drop=True)
    ref_df = ref_df.sort_values(["_rlnGroupName", "_rlnMicrographPreExposure"]).reset_index(drop=True)

    for col in ["_rlnGroupName", "_rlnImageName", "_rlnMicrographName"]:
        assert (out_df[col].values == ref_df[col].values).all(), f"Column {col} mismatch"
...
def test_conversion_numerical_regression(fixture_paths, tmp_path):
    ...
    key_cols = ["_rlnDefocusU", "_rlnDefocusV", "_rlnDefocusAngle", "_rlnAngleRot", "_rlnAngleTilt", "_rlnAnglePsi"]
...
def test_conversion_dose_sorted(fixture_paths, tmp_path):
    ...
    for group, sub in out_df.groupby("_rlnGroupName"):
        ...
        assert (np.diff(doses) >= 0).all(), f"Group {group} not sorted by dose"
...
def test_conversion_random_subset_preserved(fixture_paths, tmp_path):
    ...
    for group, sub in out_df.groupby("_rlnGroupName"):
        ...
        assert len(set(subsets)) == 1, f"Group {group} has mixed random subsets: {set(subsets)}"
