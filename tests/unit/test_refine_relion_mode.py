...
def test_run_local_em_exact_external_log_evidence_scales_posterior(rng):
    dataset = MockDataset(1, rng)
...
class TestRelionModeSmokeTest:
    ...
    def test_relion_k_class_defaults_to_fixed_iteration_schedule(
        self,
        half_datasets,
        init_volume,
        rotations,
        ...
        """RELION Class3D/VDAM does not run auto-refine convergence checks."""
        original_update = iteration_loop_module.update_refinement_state
        ...
        assert len(result["current_sizes"]) == 3
    ...
    def test_relion_mode_skips_initial_global_significant_support_for_os0_replay(
        ...
        assert captured["sig_calls"] == 0
        assert captured["local_pass2_calls"] == 0
        assert captured["run_em_calls"] == 2
        assert captured["prior_centers"] == []
        assert captured["normalization_log_z"] == []
