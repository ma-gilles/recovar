...
def test_slice_volume_matches_full():
    ...
    out_full = np.asarray(core_slicing.slice_volume(
        volume_ft, rotation_matrices, image_shape, volume_shape, "nearest"
    ))
    out_half = np.asarray(core_slicing.slice_volume(
...
def test_slice_volume_half_image_jax(monkeypatch):
    ...
    ref_half = np.asarray(core_slicing.slice_volume(
        vol_ft, rots, image_shape, volume_shape, "linear_interp", half_image=True,
    ))

    # New path: project from half volume directly to half image
    out_half = np.asarray(core_slicing.slice_volume(
        half_vol, rots, image_shape, volume_shape, "linear_interp",
...
def test_batch_slice_volume_jax(monkeypatch):
    ...
    batch_out = np.asarray(core_slicing.batch_slice_volume(
        half_vols, rots, image_shape, volume_shape, "linear_interp",
        half_volume=True, half_image=False,
    ))
    for i in range(n_volumes):
        single = np.asarray(core_slicing.slice_volume(
            half_vols[i], rots, image_shape, volume_shape, "linear_interp",
            half_volume=True, half_image=False,
        ))
        np.testing.assert_allclose(batch_out[i], single, atol=1e-5, rtol=1e-5)

    # With half_image=True: batch == vmap of single
    batch_out_hi = np.asarray(core_slicing.batch_slice_volume(
        half_vols, rots, image_shape, volume_shape, "linear_interp",
        half_volume=True, half_image=True,
    ))
    for i in range(n_volumes):
        single_hi = np.asarray(core_slicing.slice_volume(
            half_vols[i], rots, image_shape, volume_shape, "linear_interp",
            half_volume=True, half_image=True,
        ))
...
def test_slice_from_half_volume_to_half_image_multiple_shapes_jax(monkeypatch):
    ...
    for volume_shape in [(8, 8, 8), (8, 8, 9), (8, 10, 12)]:
        ...
        ref = np.asarray(core_slicing.slice_volume(
            vol_ft, rots, image_shape, volume_shape, "linear_interp", half_image=True
        ))
        out = np.asarray(core_slicing.slice_volume(
            half_vol, rots, image_shape, volume_shape, "linear_interp", half_volume=True, half_image=True
        ))
        np.testing.assert_allclose(
...
def test_slice_from_half_volume_to_half_image_nearest_jax(monkeypatch):
    ...
    ref = np.asarray(core_slicing.slice_volume(
        vol_ft, rots, image_shape, volume_shape, "nearest", half_image=True
    ))
    out = np.asarray(core_slicing.slice_volume(
        half_vol, rots, image_shape, volume_shape, "nearest", half_volume=True, half_image=True
    ))
...
def test_slice_from_half_volume_to_half_image_vjp_vs_reference_jax(monkeypatch):
    ...
    def f_ref(hv):
        ...
        return core_slicing.slice_volume(
            full_vol, rots, image_shape, volume_shape, "linear_interp", half_image=True
        )
...
def test_batch_slice_from_half_volume_to_half_image_vs_full_vol_batch_jax(monkeypatch):
    ...
    ref = np.asarray(core_slicing.batch_slice_volume(
        jnp.array(full_vols), rots, image_shape, volume_shape, "linear_interp", half_image=True
    ))
    # New path: batch project from half volumes to half images
    out = np.asarray(core_slicing.batch_slice_volume(
        jnp.array(half_vols), rots, image_shape, volume_shape, "linear_interp", half_volume=True, half_image=True
    ))
...
def test_slice_from_half_volume_to_half_image_cuda_vs_jax(gpu_device):
    ...
    for volume_shape in [(8, 8, 8), (8, 10, 12)]:
        ...
        with jax.default_device(device):
            ...
            ref_cuda = np.asarray(core_slicing.slice_volume(
            ...
            out_cuda = np.asarray(core_slicing.slice_volume(
                jax.device_put(jnp.array(half_vol)), jax.device_put(rots),
                image_shape, volume_shape, "linear_interp", half_volume=True, half_image=True,
            ))
        np.testing.assert_allclose(
...
def test_slice_from_half_volume_to_half_image_cuda_vjp_vs_ref(gpu_device):
    ...
    with jax.default_device(device):
        ...
        def f_ref(v):
            ...
            return core_slicing.slice_volume(
                full_vol, rots_d, image_shape, volume_shape, "linear_interp", half_image=True
            )
...
def test_batch_slice_from_half_volume_to_half_image_cuda_vs_jax(gpu_device):
    ...
    with jax.default_device(device):
        # Reference: CUDA full_vol → half_img (tested separately)
        ref_cuda = np.asarray(core_slicing.batch_slice_volume(
            jax.device_put(jnp.array(full_vols)), jax.device_put(rots),
            image_shape, volume_shape, "linear_interp", half_image=True,
        ))
        # Under test: CUDA half_vol → half_img (direct kernel)
        out_cuda = np.asarray(core_slicing.batch_slice_volume(
...
# ── Half-image JAX path equivalence tests ─────────────────────────────
# Verify the new efficient half-image path (direct half-image coords)
# produces the same results as the old path (full image → extract half).


def _make_hermitian_volume(rng, volume_shape):
    """Create a Hermitian-symmetric Fourier volume from a real-space volume."""
    real_vol = rng.standard_normal(volume_shape).astype(np.float32)
    vol_ft = np.asarray(fourier_transform_utils.get_dft3(jnp.array(real_vol)))
    return vol_ft.reshape(-1)


@pytest.mark.parametrize("order", [0, 1])
def test_jax_half_image_forward_matches_old_path(monkeypatch, order):
    """slice_volume(half_image=True) via new JAX path matches old expand-then-extract."""
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    ...
    image_shape = (6, 8)
    n_images = 4
    rots = np.concatenate([np.eye(3, dtype=np.float32)[None], _random_rotations(rng, n_images - 1)])
    vol = jnp.array(_make_hermitian_volume(rng, volume_shape))

    # Old path: full image → extract half
    full_images = np.asarray(core_slicing._jax_slice(vol, jnp.array(rots), image_shape, volume_shape, order))
    old_half = np.asarray(fourier_transform_utils.full_image_to_half_image(jnp.array(full_images), image_shape))

    # New path: direct half-image coordinates
    new_half = np.asarray(core_slicing._jax_slice_half_image(vol, jnp.array(rots), image_shape, volume_shape, order))

    np.testing.assert_allclose(new_half, old_half, atol=1e-5, rtol=1e-5,
                               err_msg=f"order={order}")


def test_jax_half_image_forward_cubic_matches_old_path(monkeypatch):
    """Cubic slice_volume(half_image=True) via new path matches old extract."""
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    import recovar.core.cubic_interpolation as cubic_interpolation
    ...
    volume_shape = (8, 8, 8)
    image_shape = (6, 8)
    rots = np.concatenate([np.eye(3, dtype=np.float32)[None], _random_rotations(rng, 3)])
    vol_ft = jnp.array(_make_hermitian_volume(rng, volume_shape))
    coeffs = cubic_interpolation.calculate_spline_coefficients(vol_ft.reshape(volume_shape))

    # Old: full cubic slice → extract half
    full_images = np.asarray(core_slicing._jax_slice(coeffs, jnp.array(rots), image_shape, volume_shape, 3))
    old_half = np.asarray(fourier_transform_utils.full_image_to_half_image(jnp.array(full_images), image_shape))

    # New: direct half-image cubic slice
    new_half = np.asarray(core_slicing._jax_slice_half_image(coeffs, jnp.array(rots), image_shape, volume_shape, 3))

    np.testing.assert_allclose(new_half, old_half, atol=1e-4, rtol=1e-4)


@pytest.mark.parametrize("disc_type", ["nearest", "linear_interp"])
@pytest.mark.parametrize("half_image", [False, True])
def test_slice_volume_jax_half_image(monkeypatch, disc_type, half_image):
    """slice_volume with use_cuda=False matches full→extract for nearest and linear."""
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    ...
    # Reference: full images
    ref_full = np.asarray(core_slicing.slice_volume(
        vol_ft, jnp.array(rots), image_shape, volume_shape, disc_type,
        half_image=False, use_cuda=False,
    ))

    # Under test
    result = np.asarray(core_slicing.slice_volume(
        vol_ft, jnp.array(rots), image_shape, volume_shape, disc_type,
        half_image=half_image, use_cuda=False,
    ))

    if half_image:
        ref_half = np.asarray(fourier_transform_utils.full_image_to_half_image(jnp.array(ref_full), image_shape))
        np.testing.assert_allclose(result, ref_half, atol=1e-4, rtol=1e-4,
                                   err_msg=f"disc_type={disc_type}")
    else:
...
def test_adjoint_jax_all_flag_combos(monkeypatch, disc_type, half_image, half_volume):
    """adjoint_slice_volume with use_cuda=False: all flag combos.

    For half_image=False: dot-product adjoint test <A*y, x> == <y, Ax>.
    For half_image=True: verify that adjoint(half_image=True) matches
    "expand half→full, then adjoint(half_image=False)" since the adjoint uses
    Hermitian expansion semantics (matching CUDA), not the mathematical adjoint
    of "project to half-image".
    """
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    ...
    image_shape = (8, 8)
    n_images = 3
    rots = jnp.array(_random_rotations(rng, n_images).astype(np.float32))

    if half_image:
        # Test: adjoint(half_imgs, half_image=True) == adjoint(expand(half_imgs), half_image=False)
        full_images = _random_complex_images(rng, n_images, image_shape)
        half_imgs = jnp.array(np.asarray(
            fourier_transform_utils.full_image_to_half_image(jnp.array(full_images), image_shape)
        ))
        expanded = jnp.array(np.asarray(
            fourier_transform_utils.half_image_to_full_image(half_imgs, image_shape)
        ))

        result = np.asarray(core_slicing.adjoint_slice_volume(
            half_imgs, rots, image_shape, volume_shape, disc_type,
            half_image=True, half_volume=half_volume,
        ))
        ref = np.asarray(core_slicing.adjoint_slice_volume(
            expanded, rots, image_shape, volume_shape, disc_type,
            half_image=False, half_volume=half_volume,
        ))
        np.testing.assert_allclose(result, ref, atol=1e-5, rtol=1e-5,
                                   err_msg=f"disc_type={disc_type}, half_image=True, half_volume={half_volume}")
    else:
        # Dot-product adjoint test: <A*y, x> == <y, Ax>
        vol_full = _make_hermitian_volume(rng, volume_shape)
        if half_volume:
            vol_in = jnp.array(np.asarray(fourier_transform_utils.full_volume_to_half_volume(
                jnp.array(vol_full), volume_shape
            )))
        else:
            vol_in = jnp.array(vol_full)

        fwd = np.asarray(core_slicing.slice_volume(
            vol_in, rots, image_shape, volume_shape, disc_type,
            half_volume=half_volume, half_image=False, use_cuda=False,
        ))

        img_in = jnp.array(_random_complex_images(rng, n_images, image_shape))

        adj = np.asarray(core_slicing.adjoint_slice_volume(
            img_in, rots, image_shape, volume_shape, disc_type,
            half_image=False, half_volume=half_volume,
        ))
        ...
        np.testing.assert_allclose(lhs, rhs, atol=1e-3, rtol=1e-3,
                                   err_msg=f"disc_type={disc_type}, half_image=False, half_volume={half_volume}")
...
def test_cubic_slice_from_coeffs_half_image(monkeypatch):
    """slice_from_cubic_half_coefficients(half_image=True) matches full→extract."""
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    ...
    full_out = np.asarray(core_slicing.slice_from_cubic_half_coefficients(
        coeffs, jnp.array(rots), image_shape, volume_shape, half_image=False
    ))
    half_out = np.asarray(core_slicing.slice_from_cubic_half_coefficients(
        coeffs, jnp.array(rots), image_shape, volume_shape, half_image=True
    ))

    ref_half = np.asarray(fourier_transform_utils.full_image_to_half_image(jnp.array(full_out), image_shape))
    np.testing.assert_allclose(half_out, ref_half, atol=1e-4, rtol=1e-4)
