...
def test_slice_volume_cubic_with_precomputed_spline_coefficients():
    ...
    slices = np.asarray(
        core_slicing.slice_volume(
            core_slicing.VolumeRepr(coeffs, disc_type="cubic"),
            rots,
            image_shape=image_shape,
            volume_shape=volume_shape,
            max_r=None,
...
def test_batch_slice_volume_cubic_volume_repr_matches_single(monkeypatch):
    monkeypatch.setattr(core_slicing, "_on_gpu", lambda: False)
    ...
    for i in range(coeffs.shape[0]):
        single_out = np.asarray(
            core_slicing.slice_volume(
                core_slicing.VolumeRepr(coeffs[i], disc_type="cubic"),
                rots,
                image_shape,
                volume_shape,
                half_image=True,
...
def test_slice_volume_defaults_half_image_from_half_volume_repr():
    ...
    wrapped = core_slicing.VolumeRepr(half_vol, disc_type="linear_interp", half_volume=True)

    default_out = np.asarray(core_slicing.slice_volume(wrapped, rots, image_shape, volume_shape))
    explicit_out = np.asarray(core_slicing.slice_volume(wrapped, rots, image_shape, volume_shape, half_image=True))

    assert default_out.shape == explicit_out.shape
    np.testing.assert_allclose(default_out, explicit_out, atol=1e-6, rtol=1e-6)


def test_adjoint_slice_volume_cubic_volume_repr_accumulates_in_coefficient_space():
    ...
    out = np.asarray(
        core_slicing.adjoint_slice_volume(
            ...
            image_shape,
            volume_shape,
            volume=core_slicing.VolumeRepr(np.zeros_like(coeffs), disc_type="cubic"),
        )
    )
    ref = np.asarray(
...
def test_slice_volume_matches_full():
    ...
    out_full = np.asarray(core_slicing.slice_volume(volume_ft, rotation_matrices, image_shape, volume_shape, "nearest"))
    out_half = np.asarray(
        core_slicing.slice_volume(
            half_volume,
            rotation_matrices,
...
def test_slice_from_half_volume_to_half_image_multiple_shapes_jax(monkeypatch):
    ...
    for volume_shape in [(8, 8, 8), (8, 8, 9), (8, 10, 12)]:
        ...
        ref = np.asarray(
            core_slicing.slice_volume(vol_ft, rots, image_shape, volume_shape, "linear_interp", half_image=True)
        )
        out = np.asarray(
            core_slicing.slice_volume(
...
def test_slice_from_half_volume_to_half_image_nearest_jax(monkeypatch):
    ...
    ref = np.asarray(core_slicing.slice_volume(vol_ft, rots, image_shape, volume_shape, "nearest", half_image=True))
    out = np.asarray(
        core_slicing.slice_volume(
...
def test_slice_from_half_volume_to_half_image_vjp_vs_reference_jax(monkeypatch):
    ...
    def f_ref(hv):
        ...
        return core_slicing.slice_volume(full_vol, rots, image_shape, volume_shape, "linear_interp", half_image=True)
...
def test_slice_from_half_volume_to_half_image_cuda_vs_jax(gpu_device):
    ...
    for volume_shape in [(8, 8, 8), (8, 10, 12)]:
        ...
        with jax.default_device(device):
            ...
            ref_cuda = np.asarray(
                core_slicing.slice_volume(
                    jax.device_put(jnp.array(vol_ft)),
                    jax.device_put(rots),
                    image_shape,
                    volume_shape,
                    "linear_interp",
...
def test_batch_slice_from_half_volume_to_half_image_cuda_vs_jax(gpu_device):
    ...
    with jax.default_device(device):
        ...
        ref_cuda = np.asarray(
            core_slicing.batch_slice_volume(
                jax.device_put(jnp.array(full_vols)),
                jax.device_put(rots),
                image_shape,
                volume_shape,
                "linear_interp",
