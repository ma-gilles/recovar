...
def test_deferred_plan_preserves_geometry(monkeypatch, order, oversampling, perturbation):
    ...
    dense = driver._build_sampling_plan(opts, iteration=3)
    ...
    sparse = driver._build_sampling_plan(opts, iteration=3, defer_fine_rotations=True)
...
def test_deferred_plan_cannot_enter_dense_execution(monkeypatch):
    ...
    opts = driver.NativeInitialModelOptions(fn_img="particles.star", healpix_order=0)
    plan = driver._build_sampling_plan(opts, defer_fine_rotations=True)
