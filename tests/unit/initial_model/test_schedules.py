...
class TestStepSizeSchedule:
    ...
    def test_3d_initial_model_default_scheme_decays_0p9_to_0p5(self):
        ...
        # RELION uses integer division in `float a = grad_inbetween_iter/2`,
        # so the midpoint is b + a/2 = 60 + 25.
    ...
    def test_short_4_iter_tau2_fudge_matches_relion_integer_division_nan(self):
        """RELION's `grad_inbetween_iter/4` is integer division.

        For a 4-iter InitialModel run, grad_inbetween_iter=3 so the tau2
        sigmoid length is zero; iter 1 evaluates 0/0 and writes NaN.
        """
        p = compute_phase_lengths(4, 0.3, 0.2)
        assert p.grad_ini_iter == 1
        assert p.grad_inbetween_iter == 3
        assert math.isnan(
            compute_tau2_fudge(
                iter=1,
                phase_lengths=p,
...
class TestTau2FudgeSchedule:
    ...
    def test_3d_initial_model_default_scheme_grows_1_to_4(self):
        ...
        # Midpoint iter = b + a/2 = grad_ini_iter + (grad_inbetween_iter / 4) / 2
        # using RELION's integer division: 60 + 12.5, so 72 and 73 straddle it.
