...
# ---------------------------------------------------------------------------
# Test 7: slice_volume_by_map + custom_vjp CUDA path (forward & backward)
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("N", SIZES)
@pytest.mark.parametrize("n_images", N_IMAGES_LIST)
def test_slice_volume_by_map_vjp_consistency(N, n_images):
    """Verify VJP of slice_volume_by_map matches direct adjoint."""
    ...
    # Forward
    from recovar.core.slicing import slice_volume_by_map, adjoint_slice_volume_by_map
    projected = slice_volume_by_map(vol, rots, image_shape, volume_shape, "linear_interp")
    assert projected.shape == (n_images, N * N)

    # Adjoint via VJP
    f = lambda v: slice_volume_by_map(v, rots, image_shape, volume_shape, "linear_interp")
    _, u = jax.vjp(f, jnp.zeros(vol_size, dtype=jnp.complex64))
    vjp_result = u(images)[0]

    # Direct adjoint
    direct_result = adjoint_slice_volume_by_map(images, rots, image_shape, volume_shape, "linear_interp")
...
def test_half_volume_vjp_consistency(N, n_images):
    ...
    Since slice_volume_by_map_from_half_volume = project(expand(hv)), the VJP is:
      expand'*(project'*(g))
    ...
    f_full = lambda v: slice_volume_by_map(v, rots, image_shape, volume_shape, "linear_interp")
    ...
    f_half = lambda hv: slice_volume_by_map_from_half_volume(hv, rots, image_shape, volume_shape, "linear_interp")
