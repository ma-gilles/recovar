...
def test_deferred_check_only_queues_concrete_results(monkeypatch, traced):
    monkeypatch.setattr(cuda, "_RELION_PREPROCESS_PENDING_CHECKS", [])
    seen = []
    def native(images, *args):
        seen.append(args[-1])
        return images, images, jnp.zeros((1,), dtype=jnp.int32)
    monkeypatch.setattr(cuda, "_relion_preprocess_real_f32_jit", native)
    def call(images):
        return cuda.relion_preprocess_real_f32(images, None, None, 1., 1., deferred_finite_check=True)[1]
    ...
    assert seen == [traced]
    assert cuda.pending_relion_preprocess_checks() == (0 if traced else 1)
    assert cuda.drain_relion_preprocess_checks() == (0 if traced else 1)


def test_deferred_invalid_status_fails_and_drains(monkeypatch):
    monkeypatch.setattr(cuda, "_RELION_PREPROCESS_PENDING_CHECKS", [])
    cuda._queue_relion_preprocess_check(jnp.asarray([2],dtype=jnp.int32))
    with pytest.raises(RuntimeError, match="2 image"):
        cuda.drain_relion_preprocess_checks()
    assert cuda.pending_relion_preprocess_checks() == 0
