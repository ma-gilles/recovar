...
def test_local_result_route_is_opt_in_and_single_class(monkeypatch, n_classes, host_finalize, flag):
    ...
    def engine(_dataset, mean, *_args, **kwargs):
        ...
        return (
            np.zeros(mean.shape, dtype=np.complex64),
            np.zeros(mean.shape, dtype=np.float32),
...
def test_actual_local_engine_publishes_exact_host_statistics():
    ...
    assert all(isinstance(value, np.ndarray) for value in candidate[3])
    assert isinstance(candidate[4].wsum_norm_correction, np.ndarray)
