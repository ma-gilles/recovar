...
def _write_capture(path, *, corrupt_lane=False, production_offset=0):
    ...
    squared = np.add(
        np.multiply(pixels["diff_real"], pixels["diff_real"], dtype=np.float32),
        np.multiply(pixels["diff_imag"], pixels["diff_imag"], dtype=np.float32),
        dtype=np.float32,
    )
    pixels["contribution"] = np.multiply(
