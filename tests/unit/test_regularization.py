...
def test_streamed_fsc_preserves_requested_output_precision(monkeypatch, output_dtype):
    ...
    expected = regularization.compute_relion_fsc_from_backprojector(*data, *weights, shape, **kwargs)
    monkeypatch.setenv("RECOVAR_RELION_FSC_PACKED_STREAM_MIN_ELEMENTS", "0")
    actual = regularization.compute_relion_fsc_from_backprojector(*data, *weights, shape, **kwargs)
