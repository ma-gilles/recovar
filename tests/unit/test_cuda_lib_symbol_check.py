...
an older ``.cu`` (e.g. lacking ``PerImageBackproject``) was silently accepted
...
def test_missing_required_symbols_returns_sentinel_for_missing_file(tmp_path):
...
def test_lib_is_stale_flags_so_missing_symbols(tmp_path, monkeypatch):
    ...
    be flagged as stale. This is the regression case that bit us: a cached
    .so from an older branch, mtime newer than current .cu source, but
    missing ``PerImageBackproject``.
    """
