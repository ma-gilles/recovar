...
def test_sampling_points_and_volume_shapes():
    sampling_points = np.asarray(
        locres.get_sampling_points(grid_size=16, locres_sampling=4, locres_maskrad=4, voxel_size=1.0)
    )
...
def test_masked_noisy_error_split_over_shells_v1_runs():
    ...
    result = np.asarray(
        locres.masked_noisy_error_split_over_shells(diff, noise_var, offset, maskrad_pix, 2, multiplier)
    )
...
def test_masked_noisy_error_split_over_shells_v2_runs():
    ...
    offset = jnp.array([0, 0, 0], dtype=jnp.int32)
    result = np.asarray(
        locres.masked_noisy_error_split_over_shells_v2(diff, noise_var, offset, maskrad_pix, 2, multiplier)
    )
    assert result.ndim == 1
    assert np.all(np.isfinite(result))
    assert np.all(result >= 0)
...
def test_expensive_local_error_with_cov_gpu(gpu_device):
    ...
    cpu_out = np.asarray(
        locres.expensive_local_error_with_cov(
            map1=map1,
            map2=map2,
            voxel_size=1.5,
            noise_variance=noise_variance,
