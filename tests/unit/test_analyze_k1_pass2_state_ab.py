...
def _write_capture(
    ...
    *,
    ...
    np.savez(
        ...
        local_index=np.int64(local_index),
...
def test_analyze_separates_coordinates_and_attributes_candidate_only_mass(tmp_path):
    reference = tmp_path / "reference.npz"
    candidate = tmp_path / "candidate.npz"
    _write_capture(
        reference,
    ...
    assert report["summary"]["first_unequal"] == {
        "stage": "identity_and_topology",
        "field": "iteration",
    }
    assert report["summary"]["first_scientific_unequal"] == {
        "stage": "identity_and_topology",
        "field": "candidate_mask",
    }
    topology = report["summary"]["candidate_topology_delta"]
    assert topology["candidate_only_count"] == 1
    assert topology["reference_only_count"] == 0
    assert topology["candidate_only_posterior_mass"] == 0.2
    assert topology["candidate_common_renormalized_pmax"] == 1.0
    assert topology["pmax_gap_explained_by_candidate_only_fraction"] == 1.0
