...
Three behaviors:
1. ``--gpu-gb N`` is accepted by analyze / compute_state / compute_trajectory
...
def test_gpu_memory_arg_in_shared_downstream_args():
    """--gpu-gb must be accepted by all downstream commands."""
    from recovar.utils import parser_args

    parser = argparse.ArgumentParser()
    parser_args.standard_downstream_args(parser)
    args = parser.parse_args(["/tmp/results", "--gpu-gb", "8.0"])
...
def test_gpu_memory_arg_in_analyze():
    """analyze.py must accept --gpu-gb via standard_downstream_args."""
    from recovar.commands import analyze

    parser = argparse.ArgumentParser()
    analyze.add_args(parser)
    args = parser.parse_args(["--zdim", "4", "--gpu-gb", "12.5", "/dummy/path"])
...
def test_gpu_memory_arg_in_compute_state():
    ...
    args = parser.parse_args(["--gpu-gb", "16", "--latent-points", "/dev/null", "/dummy/path"])
...
def test_gpu_memory_arg_in_compute_trajectory():
    ...
    args = parser.parse_args(["--gpu-gb", "4", "/dummy/path"])
    assert args.gpu_memory == 4.0


# ---------------------------------------------------------------------------
# --gpu-gb on every other heavy-GPU command
# ---------------------------------------------------------------------------


def _commands_that_need_gpu_memory():
    """Commands that do heavy GPU work and therefore must accept --gpu-gb.
...
def test_gpu_memory_arg_in_command(cmd_name):
    """Every heavy-GPU command must accept --gpu-gb.
    ...
    assert "--gpu-gb" in combined, (
        f"recovar.commands.{cmd_name} does NOT expose --gpu-gb. "
