...
def test_resident_hypothesis_tables_match_host_device_index_build(monkeypatch):
    ...
    def capture(*args, **kwargs):
        calls.append(kwargs.get("resident") is not None)
        return original(*args, **kwargs)
    monkeypatch.setattr(sba, "compact_pair_index_arrays_device", capture)
    ...
    with jax.default_device(jax.devices("cpu")[0]):
        for image_indices in (chunk, np.concatenate([chunk[:3], special_images[:1]])):
            ...
            host = sba._build_compact_pair_bucket_arrays_from_per_image_inputs(bucket, fast, capacity_rows=8, device_index=True)
            monkeypatch.setenv(sba.RESIDENT_HYPOTHESIS_TABLES_ENV, "1")
            resident = sba._build_compact_pair_bucket_arrays_from_per_image_inputs(bucket, fast, capacity_rows=8, device_index=True)
...
def test_preparation_and_resident_tables_preserve_fused_results(monkeypatch, noise):
    ...
    kwargs.update(accumulate_noise=noise, relion_f32_fine_posterior=True)
    expected = _fused_kclass_result_arrays(engine.compute_k_class_pass2_stats_sparse_fused(**kwargs))
    calls = []
    original = sba.padded_rotations_from_resident_device
    def capture(*args, **kwargs):
        calls.append(1)
        return original(*args, **kwargs)
    monkeypatch.setattr(sba, "padded_rotations_from_resident_device", capture)
    for flag in flags:
