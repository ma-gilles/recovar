...
from scripts import summarize_em_k1_norm_roundtrip_scorecard as scorecard


@pytest.mark.unit
def test_validates_fixed_normalization_roundtrip_panels() -> None:
...
def test_renders_checked_case_tables() -> None:
    ...
    assert "| Preprocessing exactness | 0/2 | 2/2 | +2 |" in rendered
    assert "| Geometry identity | 5/5 | 5/5 | +0 |" in rendered
    assert "| Score/map gates | 17/21 | 17/21 | +0 |" in rendered
    assert "| [x] | `norm-roundtrip-preprocess-correction` | fail | pass | improved |" in rendered
    assert "| [ ] | `norm-roundtrip-map-gt-merged` | fail | fail | unchanged-fail |" in rendered
