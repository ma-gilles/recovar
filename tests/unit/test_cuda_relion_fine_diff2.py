...
@pytest.mark.gpu
def test_fused_relion_coarse_pass1_matches_sequential_bookkeeping_bitwise(
    monkeypatch,
    custom_cuda_lib,
    gpu_device,
):
    import recovar.cuda_backproject as cuda_backproject
    from recovar.em.dense_single_volume.helpers import significance
    from recovar.em.dense_single_volume.helpers.scoring import _update_logsumexp
    ...
    images = (
        rng.normal(0.0, 0.02, (batch_size, compact_pixel_count))
        + 1j * rng.normal(0.0, 0.02, (batch_size, compact_pixel_count))
    ).astype(np.complex64)
    ...
    pixel_weight = rng.uniform(0.1, 3.0, images.shape).astype(np.float32)
    initial_diff2 = rng.uniform(10.0, 20.0, batch_size).astype(np.float32)
    lookup = np.arange(compact_pixel_count, dtype=np.int32)
    rotation_prior = rng.normal(0.0, 0.1, rotation_count).astype(np.float32)
    translation_prior = rng.normal(
        0.0,
        0.1,
        (batch_size, translation_count),
    ).astype(np.float32)
    class_prior = np.float32(-0.125)
    ...
    with jax.default_device(gpu_device):
        ...
        expected_raw = -diff2
        expected_raw = jnp.where(
            jnp.arange(rotation_count)[None, :, None] < valid_rotation_count,
            expected_raw,
            -jnp.inf,
        )
        expected_scores = expected_raw + class_prior
        expected_scores = expected_scores + jnp.asarray(rotation_prior)[None, :, None]
        expected_scores = expected_scores + jnp.asarray(translation_prior)[:, None, :]
        initial_max = jnp.full(batch_size, -jnp.inf, dtype=jnp.float32)
        initial_sum = jnp.zeros(batch_size, dtype=jnp.float64)
        expected_class_max, expected_class_sum = _update_logsumexp(
            initial_max,
            initial_sum,
            expected_scores,
        )
        expected_global_max, expected_global_sum = _update_logsumexp(
            initial_max,
            initial_sum,
            expected_scores,
        )
        actual = significance._fused_relion_coarse_score_priors_logsumexp_block(
            jnp.asarray(projector),
            jnp.asarray(rotations),
            jnp.asarray(images),
            jnp.asarray(translation_angles),
            ...
            class_prior,
            jnp.asarray(valid_rotation_count, dtype=jnp.int32),
            initial_max,
            initial_sum,
            initial_max,
            initial_sum,
    ...
    for actual_value, expected_value in zip(actual, expected, strict=True):
        actual_np = np.asarray(actual_value)
        expected_np = np.asarray(expected_value)
        np.testing.assert_array_equal(
            actual_np.view(np.uint8),
            expected_np.view(np.uint8),
        )
