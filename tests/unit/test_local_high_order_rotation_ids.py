...
from recovar.em.dense_single_volume import local_em_engine, local_layout
...
def test_projection_cache_rows_map_large_ids_and_padding():
    cache = local_em_engine._LocalRelionProjectionCache(
        projections=None,
        id_map=None,
        enabled=True,
        row_count=3,
    ...
    rows = local_em_engine._relion_projection_cache_rows_for_bucket(cache, ids)
    ...
    with pytest.raises(RuntimeError, match="missing from the RELION projection cache"):
        local_em_engine._relion_projection_cache_rows_for_bucket(cache, np.array([[6]], dtype=np.int64))
