...
def test_relion_fused_x_half_wrapper_uses_mixed_aliases_and_native_square(monkeypatch):
    ...
    assert observed["target"] == em_cuda_kernels._TARGET_RELION_FUSED_X_HALF_BP
...
def test_ordinary_indexed_signature_cuda_source_copies_after_production_launch():
    # pipeline library only: the next launcher of its unit follows (relax split S4)
    text = read_cuda_source()
    start = text.index("cudaError_t launch_backproject_indexed_with_signature(")
    launch = text[start : text.index("cudaError_t launch_batch_backproject_indexed(", start)]
...
def test_relion_fused_x_half_particle_grid_preserves_particle_axis_in_native_attrs(
    ...
    assert observed["target"] == em_cuda_kernels._TARGET_RELION_FUSED_X_HALF_BP_PARTICLE_GRID
    assert observed["options"]["input_output_aliases"] == {4: 0, 5: 1}
